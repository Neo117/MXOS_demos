...
ret_code_t auth_init(ali_init_t const *p_init, tx_func_t tx_func)
{
    ...
    ikm_init(p_init);
    ret = _os_timer_new(&g_auth.timer, on_timeout, &g_auth, BZ_AUTH_TIMEOUT);
    return ret;
}
    ...
    switch (g_auth.state) {
        case AUTH_STATE_RAND_SENT:
            ...
            if (cmd == BZ_CMD_AUTH_REQ &&
                ...
                err_code = _os_timer_start(&g_auth.timer);
                if (err_code != BZ_SUCCESS) {
...
int auth_calc_adv_sign(uint32_t seq, uint8_t *sign)
{
    ...
    sha256_update(&context, (const BYTE*)DEVICE_NAME_STR, strlen(DEVICE_NAME_STR));
    sha256_update(&context, (const BYTE*)g_auth.device_name, g_auth.device_name_len);

    sha256_update(&context, (const BYTE*)DEVICE_SECRET_STR, strlen(DEVICE_SECRET_STR));
    sha256_update(&context, (const BYTE*)g_auth.secret, DEVICE_SECRET_LEN);

    sha256_update(&context, (const BYTE*)PRODUCT_KEY_STR, strlen(PRODUCT_KEY_STR));
    sha256_update(&context, (const BYTE*)g_auth.product_key, PRODUCT_KEY_LEN);

    sha256_update(&context, (const BYTE*)SEQUENCE_STR, strlen(SEQUENCE_STR));
