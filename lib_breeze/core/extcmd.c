...
static const char m_sdk_version[] = ":" BZ_VERSION;
...
static void network_signature_calculate(uint8_t *p_buff)
{
    ...
    sha256_update(&context, (const BYTE*)CLIENTID_STR, strlen(CLIENTID_STR));
    sha256_update(&context, str_id, sizeof(str_id));

    sha256_update(&context, (const BYTE*)DEVICE_NAME_STR, strlen(DEVICE_NAME_STR)); /* "deviceName" */
    sha256_update(&context, g_extcmd.p_device_name, g_extcmd.device_name_len);

    sha256_update(&context, (const BYTE*)DEVICE_SECRET_STR, strlen(DEVICE_SECRET_STR)); /* "deviceSecret" */
    sha256_update(&context, g_extcmd.p_secret, g_extcmd.secret_len);

    sha256_update(&context, (const BYTE*)PRODUCT_KEY_STR, strlen( PRODUCT_KEY_STR)); /* "productKey" */
...
static ret_code_t ext_cmd06_rsp(uint8_t *p_buff, uint8_t *p_blen, const uint8_t *p_data, uint8_t dlen)
{
    ...
    *p_blen = sizeof(rsp);
    ...
    if (ready_flag & ALL_READY) {
        event_notify(BZ_EVENT_APINFO, (uint8_t *)&comboinfo, sizeof(comboinfo));
    }
...
#ifdef CONFIG_AIS_SECURE_ADV
extern void set_adv_sequence(uint32_t seq);
static ret_code_t ext_cmd07_rsp(uint8_t *p_buff, uint8_t *p_blen, const uint8_t *p_data, uint8_t dlen)
{
    ret_code_t err_code = BZ_SUCCESS;
    ...
    BREEZE_LOG_INFO("The sequence hex to be saved is %08lx\r\n", seq);
...
static void get_os_info(void)
{
    ...
    g_extcmd.tlv_01_rsp[suffix_len] = '\0';
#endif
    BREEZE_LOG_DEBUG("---%*s---\r\n", suffix_len, g_extcmd.tlv_01_rsp);
