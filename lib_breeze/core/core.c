...
static void update_seq(void *arg1, void *arg2)
{
    ...
    _os_timer_start(&g_secadv_timer);
}
...
static void init_seq_number(uint32_t *seq)
{
    ...
    _os_timer_new(&g_secadv_timer, update_seq, NULL, AIS_SEQ_UPDATE_FREQ);
    _os_timer_start(&g_secadv_timer);
}
#endif
...
ret_code_t core_init(ali_init_t const *p_init)
{
    ...
    if (p_init->user_adv_len > 0) {
        ...
        if (size < p_init->user_adv_len) {
            BREEZE_LOG_ERR("Warning: no space for user adv data (expected %ld but"
                   " only %ld left). No user adv data set!!!\r\n",
