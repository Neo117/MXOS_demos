...
static void reset_tx(void)
{
    ...
    if (g_transport.timeout != 0) {
        _os_timer_stop(&g_transport.tx.timer);
    }
}
...
static void reset_rx(void)
{
    ...
    if (g_transport.timeout != 0) {
        _os_timer_stop(&g_transport.rx.timer);
    }
}
...
static ret_code_t send_fragment(void)
{
    ...
    if ((bytes_left != 0) && (g_transport.timeout != 0)) {
        _os_timer_start(&g_transport.tx.timer);
    }
...
ret_code_t transport_init(ali_init_t const *p_init)
{
    ...
    if (g_transport.timeout != 0) {
        _os_timer_new(&g_transport.tx.timer, on_tx_timeout, &g_transport, g_transport.timeout);
        _os_timer_new(&g_transport.rx.timer, on_rx_timeout, &g_transport, g_transport.timeout);
    }
    return BZ_SUCCESS;
}
...
void transport_rx(uint8_t *p_data, uint16_t length)
{
    ...
    if (!rx_frames_left()) {
        ...
        if (g_transport.timeout != 0) {
            _os_timer_start(&g_transport.rx.timer);
        }
    }
}
