 ...
 * \code{.c}
    ...
    void app_gap_vendor_callback(uint8_t cb_type, void *p_cb_data)
    {
        ...
        app_log("app_gap_common_callback: cb_type = %d", cb_type);
        switch (cb_type)
        {
        case GAP_MSG_VENDOR_CMD_RSP:
            app_log("GAP_MSG_VENDOR_CMD_RSP: command 0x%x, cause 0x%x, is_cmpl_evt %d, param_len %d",
        ...
        case GAP_MSG_VENDOR_EVT_INFO:
            app_log("GAP_MSG_VENDOR_EVT_INFO: param_len %d",
