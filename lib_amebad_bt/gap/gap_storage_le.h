 ...
 * \code{.c}
    T_APP_RESULT gap_service_callback(T_SERVER_ID service_id, void *p_para)
    {
        ...
        app_log("gap_service_callback conn_id = %d msg_type = %d\n", p_gap_data->conn_id,
                       p_gap_data->msg_type);
        ...
        else if (p_gap_data->msg_type == SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION)
        {
            ...
            {
                app_log("GATT_SERVICE_CHANGE_CCCD_ENABLE");
            }
        }
        return result;
    }
 ...
 * \code{.c}
    ...
    {
        app_log("app_handle_authen_state_evt:conn_id %d", conn_id);
        switch (new_state)
        {
        ...
        case GAP_AUTHEN_STATE_COMPLETE:
           {
               app_log("GAP_MSG_LE_AUTHEN_STATE_CHANGE:(GAP_AUTHEN_STATE_COMPLETE) status 0x%x",
                               status);
               if (status == 0)
               {
                   app_log("GAP_MSG_LE_AUTHEN_STATE_CHANGE pair success");
                   {
                       uint8_t addr[6];
                       ...
                       if (bd_type == GAP_REMOTE_ADDR_LE_RANDOM)
                       {
                           ...
                           {
                               app_log("GAP_AUTHEN_STATE_COMPLETE: resolved_addr %s, resolved_addr_type %d",
                                               TRACE_BDADDR(resolved_addr), resolved_bd_type);
                           }
                           else
                           {
                               app_log("GAP_AUTHEN_STATE_COMPLETE: resolved addr failed");
                           }
                       }
                   }
               }
               ...
               {
                   app_log("GAP_MSG_LE_AUTHEN_STATE_CHANGE pair failed");
               }
           }
           break;
           ...
    ...
    void test(void)
    {
        ...
        if(le_get_conn_addr(conn_id, bd_addr, &remote_addr_type))
        {
            ...
            if (p_entry)
            {
                ...
                {
                    app_log("Get cccd data: len %d, data %s", p_cccd_data->data_length,
 ...
 * \code{.c}
    void test(void)
    {
        ...
        {
            app_log("Generate bond device success");
        }
