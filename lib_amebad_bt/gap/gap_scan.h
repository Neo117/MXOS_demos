 ...
 * @retval  GAP_CAUSE_INVALID_STATE: Operation failure, invalid device state.
  ...
  * \code{.c}
    ...
    {
        app_log("app_handle_dev_state_evt: init state = %d scan state = %d adv state = %d conn state = %d cause = 0x%x",
    ...
    T_APP_RESULT app_gap_callback(uint8_t cb_type, void *p_cb_data)
    {
        ...
        case GAP_MSG_LE_SCAN_INFO:
            app_log("GAP_MSG_LE_SCAN_INFO:adv_type 0x%x, bd_addr %s, remote_addr_type %d, rssi %d, data_len %d",
