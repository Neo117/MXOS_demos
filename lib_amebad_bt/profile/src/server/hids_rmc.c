...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "rmc", M, ##__VA_ARGS__)
                               ...
                               uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    *p_length = 0;
    ...
    app_log("hids_attr_read_cb attrib_index = %d offset %x", attrib_index, offset);
                                ...
                                uint16_t length, uint8_t *p_value, P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    if (!p_value)
    {
        app_log("hids_attr_write_cb: p_value %p length= 0x%x", p_value, length);
...
#if RCU_VOICE_EN
    case GATT_SRV_HID_VOICE_OUTPUT_INDEX:
        app_log("--> host write voice output 0x%x ", *p_value);
    ...
    case GATT_SVC_HID_PROTOCOL_MODE_INDEX:
        {
            ...
            app_log("--> host write hids_protocol_mode %d ",
                            hids_protocol_mode);
        }
        break;
    case GATT_SVC_HID_CONTROL_POINT_INDEX:
        {
            ...
            cause = APP_RESULT_SUCCESS;
            app_log("--> host write hids_suspand_mode %d",
                            hids_suspand_mode);
        }
        break;
    case GATT_SRV_HID_KB_INPUT_INDEX:
 ...
 * @return None
*/
void hids_rmc_cccd_update_cb(uint8_t conn_id, T_SERVER_ID service_id, uint16_t index, uint16_t ccc_bits)
{
    T_HID_CALLBACK_DATA callback_data;
    callback_data.msg_type = SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION;
    bool handle = true;

    app_log("bas_cccd_update_cb index = %d ccc_bits %x", index, ccc_bits);
    switch (index)
    {
...
const T_FUN_GATT_SERVICE_CBS hids_cbs =
{
    ...
    hids_rmc_cccd_update_cb  // Authorization callback function pointer
};
...
uint8_t hids_add_service(void *p_func)
{
    ...
    if (false == server_add_service(&service_id,
                                    ...
                                    hids_cbs))
    {
        app_log("hids_add_service: ServiceId %d", service_id);
        service_id = 0xff;
    }
