...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "kb", M, ##__VA_ARGS__)
 ...
 * \endcode
 */
bool hids_kb_set_parameter(T_HIDS_PARAM_TYPE param_type, uint8_t length, void *value_ptr)
{
    bool ret = true;
...
void hids_kb_cccd_update_cb(uint8_t conn_id, T_SERVER_ID service_id, uint16_t index, uint16_t ccc_bits)
{
    bool cause = true;
    T_HID_CALLBACK_DATA callback_data;
    callback_data.conn_id = conn_id;
    ...
    app_log("hids_cccd_update_cb index = %d ccc_bits %x", index, ccc_bits);
...
bool hids_send_report(uint8_t conn_id, T_SERVER_ID service_id, uint16_t index, uint8_t *p_data,
                      uint16_t data_len)
{
    app_log("hids_send_report data_len %d", data_len);
...
const T_FUN_GATT_SERVICE_CBS hids_cbs =
{
    ...
    hids_kb_cccd_update_cb,    // Authorization callback function pointer
};
