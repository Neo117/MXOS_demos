...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "sbs", M, ##__VA_ARGS__)
...
bool simp_ble_service_set_parameter(T_SIMP_PARAM_TYPE param_type, uint16_t len, void *p_value)
{
    ...
    if (!ret)
    {
        app_log("simp_ble_service_set_parameter failed");
    }
                                            ...
                                            uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    {
    default:
        app_log("simp_ble_service_attr_read_cb, Attr not found, index %d", attrib_index);
...
{
    (void)p_value;
    app_log("simple_write_post_callback: conn_id %d, service_id %d, attrib_index 0x%x, length %d",
                                            ...
                                            P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    TSIMP_CALLBACK_DATA callback_data;
    T_APP_RESULT  cause = APP_RESULT_SUCCESS;
    app_log("simp_ble_service_attr_write_cb write_type = 0x%x", write_type);
    *p_write_ind_post_proc = simple_write_post_callback;
    ...
    {
        app_log("simp_ble_service_attr_write_cb Error: attrib_index 0x%x, length %d",
                         attrib_index,
                         length);
        cause = APP_RESULT_ATTR_NOT_FOUND;
    }
...
bool simp_ble_service_send_v4_indicate(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                       uint16_t length)
{
    app_log("simp_ble_service_send_v4_indicate");
                                     ...
                                     uint16_t cccbits)
{
    ...
    app_log("simp_ble_service_cccd_update_cb: index = %d, cccbits 0x%x", index, cccbits);
    switch (index)
    {
