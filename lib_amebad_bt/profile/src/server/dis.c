...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "dis", M, ##__VA_ARGS__)
...
bool dis_set_parameter(T_DIS_PARAM_TYPE param_type, uint8_t length, void *p_value)
{
    ...
    if (!ret)
    {
        app_log("DIS parameter set failed");
    }
                              ...
                              uint16_t offset, uint16_t *length_ptr, uint8_t **pp_value)
{
    ...
    *length_ptr = 0;
    ...
    {
    default:
        app_log("dis_attr_read_cb: attrib_index %d", attrib_index);
    ...
    app_log("dis_attr_read_cb: attrib_index %d, *length_ptr %d",
                    attrib_index,
                    *length_ptr);
    return (cause);
}
...
T_SERVER_ID dis_add_service(void *p_func)
{
    ...
    if (false == server_add_service(&service_id,
                                    ...
                                    dis_cbs))
    {
        app_log("dis_add_service: service_id %d", service_id);
        service_id = 0xff;
    }
