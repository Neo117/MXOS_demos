...
bool bas_set_parameter(T_BAS_PARAM_TYPE param_type, uint8_t length, uint8_t *p_value)
{
    ...
    default:
        {
            ret = false;
            app_log("bas_set_parameter failed");
        }
        break;
                              ...
                              uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    *p_length = 0;
    ...
    app_log("bas_attr_read_cb attrib_index = %d offset %x", attrib_index, offset);

    switch (attrib_index)
    {
    default:
        {
            app_log("bas_attr_read_cb attrib_index = %d not found", attrib_index);
...
T_SERVER_ID bas_add_service(void *p_func)
{
    ...
    if (false == server_add_service(&service_id,
                                    ...
                                    bas_cbs))
    {
        app_log("bas_add_service: service_id %d", service_id);
        service_id = 0xff;
    }
