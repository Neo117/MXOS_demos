...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "gcs", M, ##__VA_ARGS__)
...
T_GAP_CAUSE gcs_all_primary_srv_discovery(uint8_t conn_id)
{
    ...
    {
        app_log("gcs_all_primary_srv_discovery: failed, conn_id %d, cause %d",
                         conn_id, cause);
    }
    else
    {
...
T_GAP_CAUSE gcs_by_uuid128_srv_discovery(uint8_t conn_id, uint8_t *p_uuid128)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_by_uuid128_srv_discovery: failed, conn_id %d, cause %d, p_uuid128 %p",
...
T_GAP_CAUSE gcs_by_uuid_srv_discovery(uint8_t conn_id, uint16_t uuid16)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_by_uuid_srv_discovery: failed, conn_id %d, cause %d, uuid16 0x%x",
...
T_GAP_CAUSE gcs_all_char_discovery(uint8_t conn_id, uint16_t start_handle, uint16_t end_handle)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_all_char_discovery: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x",
                                       ...
                                       uint16_t end_handle, uint16_t uuid16)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_by_uuid_char_discovery: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x, uuid16 0x%x",
                                          ...
                                          uint16_t end_handle, uint8_t *p_uuid128)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_by_uuid128_char_discovery: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x, p_uuid128 %p",
                                              ...
                                              uint16_t end_handle)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_all_char_descriptor_discovery: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x",
                         conn_id, cause, start_handle, end_handle);
    }
    else
    {
...
T_GAP_CAUSE gcs_attr_read(uint8_t conn_id, uint16_t  handle)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_attr_read: failed, conn_id %d, cause %d, handle 0x%x",
                                       ...
                                       uint16_t end_handle, uint16_t uuid16)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_attr_read_using_uuid16: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x, uuid16 0x%x",
                         conn_id, cause, start_handle, end_handle, uuid16);
    }
    return cause;
}
                                        ...
                                        uint16_t end_handle, uint8_t *p_uuid128)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_attr_read_using_uuid128: failed, conn_id %d, cause %d, start_handle 0x%x, end_handle 0x%x, p_uuid128 %p",
                         conn_id, cause, start_handle, end_handle, p_uuid128);
    }
    return cause;
}
...
T_GAP_CAUSE gcs_attr_ind_confirm(uint8_t conn_id)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_attr_ind_confirm: failed, conn_id %d, cause %d",
                         conn_id, cause);
    }
    return cause;
}
                           ...
                           uint16_t length, uint8_t *p_data)
{
    ...
    if (cause != GAP_CAUSE_SUCCESS)
    {
        app_log("gcs_attr_write: failed, conn_id %d, cause %d, write_type %d, handle 0x%x, length %d",
                                         ...
                                         T_DISCOVERY_RESULT_DATA result_data)
{
    ...
    {
        app_log("gcs_client_add_discov_result: invalid conn_id %d", conn_id);
        return false;
    }
    p_gcs_link = &gcs_table[conn_id];
    if (p_gcs_link->result_num >= gcs_max_discov_table_num)
    {
        app_log("gcs_client_add_discov_result: failed, gcs_max_discov_table_num %d",
...
static void gcs_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("gcs discovery_state %d", discovery_state);
...
static void gcs_client_discover_result_cb(uint8_t conn_id,  T_DISCOVERY_RESULT_TYPE result_type,
                                          T_DISCOVERY_RESULT_DATA result_data)
{
    app_log("gcs_client_discover_result_cb: result_type = %d", result_type);
...
{
    T_GCS_CLIENT_CB_DATA cb_data;
    app_log("gcs_client_read_result_cb: handle 0x%x, cause 0x%x", handle, cause);
                                       ...
                                       uint8_t credits)
{
    (void)credits;
    T_GCS_CLIENT_CB_DATA cb_data;
    app_log("gcs_client_write_result_cb: handle 0x%x, cause 0x%x", handle, cause);
                           ...
                           uint16_t max_discov_table_size)
{
    ...
    {
        app_log("gcs_add_client: invalid link_num %d", link_num);
        return 0xff;
    }
    ...
    if (gcs_table == NULL)
    {
        app_log("gcs_add_client: allocate link table mem fail");
        gcs_cl_id = 0xff;
        return gcs_cl_id;
    }
    ...
    for (i = 0; i < gcs_link_num; i++)
    {
        ...
        {
            app_log("gcs_add_client: allocate discov table mem fail");
            gcs_cl_id = 0xff;
            return gcs_cl_id;
        }
    }
    if (false == client_register_spec_client_cb(&gcs_cl_id, &gcs_client_cbs))
    {
        ...
        app_log("gcs_add_client: register fail");
        return gcs_cl_id;
    }

    app_log("gcs_add_client: client id %d", gcs_cl_id);
    return gcs_cl_id;
}
#endif
