...
#define app_log(format, ...)  //custom_log("hid", format, ##__VA_ARGS__)
...
T_HIDS_CLIENT_REPORT *hids_report_add(uint8_t conn_id, uint16_t hdl_report_value)
{
    ...
    app_log("[HIDS]: hids_report_add: conn_id %d, reprot value handle 0x%x", conn_id,
                        hdl_report_value);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_report_add: failed, invalid conn_id %d", conn_id);
        return NULL;
    }
    p_hids_link = &hids_client_table[conn_id];
    if (p_hids_link->report_num >= HIDS_CLIENT_MAX_REPORT_NUM)
    {
        app_log("[HIDS]: hids_report_add: failed, exceed HIDS_CLIENT_MAX_REPORT_NUM");
        return NULL;
    }
    ...
    app_log("[HIDS]: hids_report_add: report_num 0x%x", p_hids_link->report_num);
                                                 ...
                                                 uint16_t handle)
{
    ...
    T_HIDS_CLIENT_LINK *p_hids_link;
    T_HIDS_CLIENT_REPORT *p_report = NULL;
    app_log("[HIDS]: hids_report_find_by_handle: conn_id %d", conn_id);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_report_find_by_handle: failed, invalid conn_id %d", conn_id);
        return NULL;
    }
    p_hids_link = &hids_client_table[conn_id];
    ...
    app_log("[HIDS]: hids_report_find_by_handle: not found");
    return NULL;
}
                                             ...
                                             T_HIDS_REPORT_TYPE type)
{
    ...
    T_HIDS_CLIENT_LINK *p_hids_link;
    T_HIDS_CLIENT_REPORT *p_report = NULL;
    app_log("[HIDS]: hids_report_find_by_id: conn_id %d", conn_id);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_report_find_by_id: failed, invalid conn_id %d", conn_id);
        return NULL;
    }
    p_hids_link = &hids_client_table[conn_id];
    ...
    app_log("[HIDS]: hids_report_find_by_id: not found");
    return NULL;
}
...
T_HIDS_CLIENT_REPORT *hids_report_check_desc_handle(uint8_t conn_id, uint16_t handle)
{
    ...
    T_HIDS_CLIENT_LINK *p_hids_link;
    T_HIDS_CLIENT_REPORT *p_report = NULL;
    app_log("[HIDS]: hids_report_check_desc_handle: conn_id %d, handle 0x%x", conn_id,
                        handle);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_report_check_desc_handle: failed, invalid conn_id %d", conn_id);
        return NULL;
    }
    p_hids_link = &hids_client_table[conn_id];
    ...
    app_log("[HIDS]: hids_report_check_desc_handle: not found");
    return NULL;
}
...
{
    app_log("[HIDS]: hids_start_discovery");
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hid_start_discovery: failed invalid conn_id %d", conn_id);
        return false;
    }
    /* First clear handle cache. */
    if (hids_client_table[conn_id].p_report_map != NULL)
    {
        free(hids_client_table[conn_id].p_report_map);
    }
...
bool hids_enable_all_cccd(uint8_t conn_id)
{
    ...
    app_log("[HIDS]: hids_enable_all_cccd");
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_enable_all_cccd: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (hids_client_table[conn_id].disc_state != DISC_HIDS_DONE ||
        hids_client_table[conn_id].enable_cccd_proc)
    {
        app_log("[HIDS]: hids_enable_all_cccd: failed invalid state %d",
    ...
    for (i = 0; i < hids_client_table[conn_id].report_num; i++)
    {
        if (hids_client_table[conn_id].report[i].hdl_report_cccd != 0)
        {
            ...
            {
                app_log("[HIDS]: hids_enable_all_cccd: call client_attr_write failed");
                return false;
            }
        }
    }
                       ...
                       uint16_t length, uint8_t *p_value, T_GATT_WRITE_TYPE type)
{
    ...
    app_log("[HIDS]: hids_write_report");
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_write_report: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    if (p_report != NULL)
    {
        ...
        {
            app_log("[HIDS]: hids_write_report: call client_attr_write failed");
            return false;
        }
    }
...
bool hids_read_saved_data(uint8_t conn_id, T_HIDS_READ_TYPE read_type, T_HIDS_READ_VALUE *p_value)
{
    ...
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_read_saved_data: failed invalid conn_id %d", conn_id);
        return false;
    }
    switch (read_type)
    {
    ...
    if (ret == false)
    {
        app_log("[HIDS]: hids_read_saved_data failed: read_type %d", read_type);
    }
...
{
    int8_t i;
    app_log("[HIDS]: hids_print_infos: conn_id %d", conn_id);
    if (conn_id >= hids_client_link_num)
    {
        return;
    }
    ...
    {
        app_log("[HIDS]: hdl_cache[%d]: 0x%x", i, hids_client_table[conn_id].hdl_cache[i]);
    }
    for (i = 0; i < hids_client_table[conn_id].report_num; i++)
    {
        app_log("[HIDS]: report[%d]: report_id 0x%x, report_type 0x%x",
                            i, hids_client_table[conn_id].report[i].report_ref.report_id,
                            hids_client_table[conn_id].report[i].report_ref.report_type);
        app_log("            hdl_report_value 0x%x, hdl_report_cccd 0x%x, hdl_report_reference 0x%x",
                            hids_client_table[conn_id].report[i].hdl_report_value,
                            hids_client_table[conn_id].report[i].hdl_report_cccd,
                            hids_client_table[conn_id].report[i].hdl_report_reference);
    }
//    app_log("[HIDS]: read_flags 0x%x, protocol_mode 0x%x, hids_info , report_map[%d] ",
...
{
    uint16_t handle = 0;
    app_log("[HIDS]: hids_read: type %d", read_type);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_read: failed invalid conn_id %d", conn_id);
        return false;
    }
...
void hids_start_read(uint8_t conn_id, uint16_t cause)
{
    ...
    app_log("[HIDS]: hids_start_read: read_proc_flag 0x%x, cause 0x%x",
                        hids_client_table[conn_id].read_proc_flag, cause);
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_start_read: failed invalid conn_id %d", conn_id);
        return;
    }
...
bool hids_get_hdl_cache(uint8_t conn_id, T_HIDS_CLIENT_STORAGE_INFO *p_storage_info)
{
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_get_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (hids_client_table[conn_id].disc_state != DISC_HIDS_DONE)
    {
        app_log("[HIDS]: hids_get_hdl_cache: failed invalid state %d",
                             hids_client_table[conn_id].disc_state);
        return false;
    }
...
bool hids_set_hdl_cache(uint8_t conn_id, T_HIDS_CLIENT_STORAGE_INFO *p_storage_info)
{
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_set_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (hids_client_table[conn_id].disc_state != DISC_HIDS_IDLE)
    {
        app_log("[HIDS]: hids_set_hdl_cache: failed invalid state %d",
                             hids_client_table[conn_id].disc_state);
        return false;
    }
...
bool hids_read_report_map(uint8_t conn_id)
{
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_read_report_map: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (hids_client_table[conn_id].disc_state != DISC_HIDS_DONE)
    {
        app_log("[HIDS]: hids_read_report_map: failed invalid state %d",
                             hids_client_table[conn_id].disc_state);
        return false;
    }
...
bool hids_handle_report_map(uint8_t conn_id, uint16_t value_size, uint8_t *p_value)
{
    ...
    hids_client_table[conn_id].p_report_map = malloc(value_size);
    if (hids_client_table[conn_id].p_report_map != NULL)
    {
        memset(hids_client_table[conn_id].p_report_map, 0, value_size);
        ...
        app_log("[HIDS]: hids_handle_report_map: handle report map success");
        return true;
    }
    else
    {
        app_log("[HIDS]: hids_handle_report_map: handle report map failed, allocate memory failed");
        return false;
    }
}
...
bool hids_set_report_map(uint8_t conn_id, uint16_t value_size, uint8_t *p_value)
{
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_set_report_map: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (hids_client_table[conn_id].disc_state != DISC_HIDS_DONE)
    {
        app_log("[HIDS]: hids_set_report_map: failed invalid state %d",
                             hids_client_table[conn_id].disc_state);
        return false;
    }
    ...
    if (value_size != 0)
    {
        ...
        {
            app_log("[HIDS]: hids_set_report_map: configuration of report map success");
            return true;
        }
        else
        {
            app_log("[HIDS]: hids_set_report_map: configuration of report map failed");
            return false;
        }
    }
    else
    {
        app_log("[HIDS]: hids_set_report_map: configuration of report map failed, invalid size");
        return false;
    }
}
...
static bool hids_client_start_char_discovery(uint8_t conn_id)
{
    ...
    app_log("[HIDS]: hids_client_start_char_discovery");
...
static bool hids_client_start_char_descriptor_discovery(uint8_t conn_id)
{
    ...
    app_log("[HIDS]: hids_client_start_char_descriptor_discovery");
    start_handle = hids_client_table[conn_id].hdl_cache[HDL_HIDS_SRV_START];
    end_handle = hids_client_table[conn_id].hdl_cache[HDL_HIDS_SRV_END];
...
static void hids_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("[HIDS]: hids_client_discover_state_cb: discovery_state = %d", discovery_state);
    if (hids_client_table[conn_id].disc_state == DISC_HIDS_START)
    {
        ...
        default:
            app_log("Invalid Discovery State!");
            break;
        }
    }
...
{
    app_log("[HIDS]: hids_client_discover_result_cb: result_type = %d", result_type);
    if (hids_client_table[conn_id].disc_state == DISC_HIDS_START)
    {
        uint16_t handle;
        uint16_t *hdl_cache;
        ...
        case DISC_RESULT_CHAR_DESC_UUID16:
            ...
            if (result_data.p_char_desc_uuid16_disc_data->uuid16 == GATT_UUID_CHAR_CLIENT_CONFIG)
            {
                ...
                if (p_report != NULL)
                {
                    ...
                    app_log("[HIDS]: Found report cccd handle 0x%x", handle);
                }
            }
            else if (result_data.p_char_desc_uuid16_disc_data->uuid16 == GATT_UUID_CHAR_REPORT_REFERENCE)
            {
                ...
                if (p_report != NULL)
                {
                    ...
                    app_log("[HIDS]: Found report ref handle 0x%x", handle);
        ...
        default:
            app_log("[HIDS]: Invalid Discovery Result Type!");
            break;
        }
    }
                                        ...
                                        uint8_t credits)
{
    ...
    app_log("[HIDS]: hids_client_write_result_cb: handle 0x%x, cause 0x%x", handle, cause);
    ...
    if (hids_client_table[conn_id].enable_cccd_proc)
    {
        if (cause == GAP_SUCCESS)
        {
            ...
            {
                cb_data.cb_content.write_result.cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                app_log("[HIDS]: hids_client_write_result_cb: invalid cccd handle 0x%x, hande 0x%x",
        ...
        if (cause == ATT_SUCCESS)
        {
            if (handle == hdl_cache[HDL_HIDS_CHAR_HID_INFO])
            {
                if (value_size == 4)
                {
                    ...
                    app_log("[HIDS]: Read hid info success");
                }
                else
                {
                    cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                    app_log("[HIDS]: Read hid info failed, invalid size");
                }
            }
            else if (handle == hdl_cache[HDL_HIDS_CHAR_REPORT_MAP])
            {
                ...
                {
                    cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                    app_log("[HIDS]: Read report map failed, invalid size");
                }
            }
            else if (handle == hdl_cache[HDL_HIDS_CHAR_PROTOCOL_MODE])
            {
                if (value_size == 1)
                {
                    ...
                    app_log("[HIDS]: Read protocol mode success");
                }
                else
                {
                    cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                    app_log("[HIDS]: Read protocol mode failed, invalid size");
                }
            }
            else
            {
                ...
                {
                    cause = GATT_ERR | GATT_ERR_INVALID_PARAM;
                    app_log("[HIDS]: Read report ref, invalid size");
                }
...
{
    app_log("[HIDS]: hids_client_disc_cb.");
    if (conn_id >= hids_client_link_num)
    {
        app_log("[HIDS]: hids_client_disc_cb: failed invalid conn_id %d", conn_id);
        return;
    }
    if (hids_client_table[conn_id].p_report_map != NULL)
    {
