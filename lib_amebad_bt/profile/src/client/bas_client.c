...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "bas", M, ##__VA_ARGS__)
...
{
    app_log("bas_start_discovery");
    if (conn_id >= bas_link_num)
    {
        app_log("bas_start_discovery: failed invalid conn_id %d", conn_id);
        return false;
    }
...
bool bas_read_battery_level(uint8_t conn_id)
{
    if (conn_id >= bas_link_num)
    {
        app_log("bas_read_battery_level: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("bas_read_battery_level: false handle = 0");
    return false;
}
/**
...
bool bas_set_notify(uint8_t conn_id, bool notify)
{
    if (conn_id >= bas_link_num)
    {
        app_log("bas_set_notify: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("bas_set_notify: false handle = 0");
    return false;
}
...
bool bas_read_notify(uint8_t conn_id)
{
    if (conn_id >= bas_link_num)
    {
        app_log("bas_read_notify: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (bas_table[conn_id].hdl_cache[HDL_BAS_BATTERY_LEVEL_CCCD])
    {
...
bool bas_get_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= bas_link_num)
    {
        app_log("bas_get_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (bas_table[conn_id].disc_state != DISC_BAS_DONE)
    {
        app_log("bas_get_hdl_cache: failed invalid state %d", bas_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_BAS_CACHE_LEN)
    {
        app_log("bas_get_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
bool bas_set_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= bas_link_num)
    {
        app_log("bas_set_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (bas_table[conn_id].disc_state != DISC_BAS_IDLE)
    {
        app_log("bas_set_hdl_cache: failed invalid state %d", bas_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_BAS_CACHE_LEN)
    {
        app_log("bas_set_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
static bool bas_start_char_discovery(uint8_t conn_id)
{
    ...
    app_log("bas_start_char_discovery");
...
static bool bas_start_char_descriptor_discovery(uint8_t conn_id)
{
    ...
    app_log("bas_start_char_descriptor_discovery");
...
static void bas_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("bas_client_discover_state_cb: discovery_state = %d", discovery_state);
...
{
    app_log("bas_client_discover_result_cb: result_type = %d", result_type);
    if (bas_table[conn_id].disc_state == DISC_BAS_START)
    {
        switch (result_type)
        {
                                       ...
                                       uint8_t credits)
{
    ...
    app_log("bas_client_write_result_cb: handle 0x%x, cause 0x%x", handle, cause);
                                      ...
                                      uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    app_log("bas_client_read_result_cb: handle 0x%x, cause 0x%x", handle, cause);
    ...
    if (handle == hdl_cache[HDL_BAS_BATTERY_LEVEL_CCCD])
    {
        ...
        if (cause == GAP_SUCCESS)
        {
            ...
            if (value_size != 2)
            {
                app_log("bas_client_read_result_cb: invalid cccd len %d", value_size);
                return;
            }
    ...
    else if (handle == hdl_cache[HDL_BAS_BATTERY_LEVEL])
    {
        ...
        if (cause == GAP_SUCCESS)
        {
            if (value_size != 1)
            {
                app_log("bas_client_read_result_cb: invalid battery value len %d", value_size);
                return;
            }
...
{
    app_log("bas_client_disc_cb.");
    if (conn_id >= bas_link_num)
    {
        app_log("bas_client_disc_cb: failed invalid conn_id %d", conn_id);
        return;
    }
    memset(&bas_table[conn_id], 0, sizeof(T_BAS_LINK));
    return;
...
T_CLIENT_ID bas_add_client(P_FUN_GENERAL_APP_CB app_cb, uint8_t link_num)
{
    ...
    {
        app_log("bas_add_client: invalid link_num %d", link_num);
        return 0xff;
    }
    if (false == client_register_spec_client_cb(&bas_client, &bas_client_cbs))
    {
        bas_client = CLIENT_PROFILE_GENERAL_ID;
        app_log("bas_add_client:register fail");
        return bas_client;
    }
    app_log("bas_add_client: client id %d", bas_client);
