...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "sbc", M, ##__VA_ARGS__)
...
{
    app_log("simp_ble_client_start_discovery");
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_start_discovery: failed invalid conn_id %d", conn_id);
        return false;
    }
...
bool simp_ble_client_read_by_handle(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
    ...
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_read_by_handle: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("simp_ble_client_read_by_handle: Request fail! Please check!");
    return false;
}
...
bool simp_ble_client_read_by_uuid(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
    ...
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_read_by_uuid: failed invalid conn_id %d", conn_id);
        return false;
    }
...
bool simp_ble_client_set_v3_notify(uint8_t conn_id, bool notify)
{
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_set_v3_notify: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("simp_ble_client_set_v3_notify: Request fail! Please check!");
    return false;
}
...
bool simp_ble_client_set_v4_ind(uint8_t conn_id, bool ind)
{
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_set_v4_ind: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("simp_ble_client_set_v4_ind: Request fail! Please check!");
    return false;
}
                                   ...
                                   T_GATT_WRITE_TYPE type)
{
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_write_v2_char: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    app_log("simp_ble_client_write_v2_char: Request fail! Please check!");
    return false;
}
...
bool simp_ble_client_get_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_get_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (simp_table[conn_id].disc_state != DISC_SIMP_DONE)
    {
        app_log("simp_ble_client_get_hdl_cache: failed invalid state %d",
                         simp_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_SIMBLE_CACHE_LEN)
    {
        app_log("simp_ble_client_get_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
bool simp_ble_client_set_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_set_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (simp_table[conn_id].disc_state != DISC_SIMP_IDLE)
    {
        app_log("simp_ble_client_set_hdl_cache: failed invalid state %d",
                         simp_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_SIMBLE_CACHE_LEN)
    {
        app_log("simp_ble_client_set_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
static bool simp_ble_client_start_char_discovery(uint8_t conn_id)
{
    ...
    app_log("simp_ble_client_start_simp_char_discovery");
    start_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_START];
    end_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_END];
...
static bool simp_ble_client_start_char_descriptor_discovery(uint8_t conn_id)
{
    ...
    app_log("simp_ble_client_start_char_descriptor_discovery");
    start_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_V3_NOTIFY];
    end_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_END];
...
static void simp_ble_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("simp_ble_client_discover_state_cb: discovery_state %d", discovery_state);
    if (simp_table[conn_id].disc_state == DISC_SIMP_START)
    {
        ...
        case DISC_STATE_FAILED:
            ...
            break;
        default:
            app_log("simp_handle_discover_state: Invalid Discovery State!");
            break;
        }
    }
...
{
    app_log("simp_ble_client_discover_result_cb: result_type %d", result_type);
    if (simp_table[conn_id].disc_state == DISC_SIMP_START)
    {
        uint16_t handle;
        uint16_t *hdl_cache;
        ...
        default:
            app_log("simp_handle_discover_result: Invalid Discovery Result Type!");
            break;
        }
    }
                                           ...
                                           uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    app_log("simp_ble_client_read_result_cb: handle 0x%x, cause 0x%x", handle, cause);
    ...
    else if (handle == hdl_cache[HDL_SIMBLE_V3_NOTIFY_CCCD])
    {
        ...
        if (cause == GAP_SUCCESS)
        {
            ...
            if (value_size != 2)
            {
                app_log("simp_ble_client_read_result_cb: invalid cccd len %d", value_size);
                return;
            }
                                            ...
                                            uint8_t credits)
{
    ...
    app_log("simp_ble_client_write_result_cb: handle 0x%x, cause 0x%x", handle, cause);
...
{
    app_log("simp_ble_client_disconnect_cb.");
    if (conn_id >= simp_link_num)
    {
        app_log("simp_ble_client_disconnect_cb: failed invalid conn_id %d", conn_id);
        return;
    }
    memset(&simp_table[conn_id], 0, sizeof(T_SIMP_LINK));
    return;
...
T_CLIENT_ID simp_ble_add_client(P_FUN_GENERAL_APP_CB app_cb, uint8_t link_num)
{
    ...
    {
        app_log("simp_ble_add_client: invalid link_num %d", link_num);
        return 0xff;
    }
    if (false == client_register_spec_client_cb(&simp_client, &simp_ble_client_cbs))
    {
        simp_client = CLIENT_PROFILE_GENERAL_ID;
        app_log("simp_ble_add_client failed");
        return simp_client;
    }
    app_log("simp_ble_add_client: simp_client %d", simp_client);
