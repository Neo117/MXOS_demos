...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "ancs", M, ##__VA_ARGS__)
...
{
    app_log("ancs_start_discovery");
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_start_discovery: failed invalid conn_id %d", conn_id);
        return false;
    }
...
static bool ancs_start_char_discovery(uint8_t conn_id)
{
    ...
    app_log("ancs_start_char_discovery");
...
bool ancs_set_notification_source_notify(uint8_t conn_id, bool notify)
{
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_set_notification_source_notify: failed invalid conn_id %d", conn_id);
        return false;
    }
...
bool ancs_set_data_source_notify(uint8_t conn_id, bool notify)
{
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_set_data_source_notify: failed invalid conn_id %d", conn_id);
        return false;
    }
                                ...
                                uint8_t *p_attribute_ids, uint8_t attribute_ids_len)
{
    ...
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_get_notification_attr: failed invalid conn_id %d", conn_id);
        return false;
    }
                       ...
                       uint8_t attribute_ids_len)
{
    ...
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_get_app_attr: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (ancs_table[conn_id].hdl_cache[HDL_ANCS_CONTROL_POINT])
    {
...
bool ancs_perform_notification_action(uint8_t conn_id, uint32_t notification_uid, uint8_t action_id)
{
    ...
    if (conn_id >= ancs_link_num)
    {
        app_log("ancs_perform_notification_action: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (ancs_table[conn_id].hdl_cache[HDL_ANCS_CONTROL_POINT])
    {
...
static void ancs_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("ancs_client_discover_state_cb: discoveryState = %d", discovery_state);
...
{
    app_log("gap_client_discover_result_cb: resultType = %d", result_type);
    if (ancs_table[conn_id].disc_state == DISC_ANCS_START)
    {
        switch (result_type)
        {
                                        ...
                                        uint8_t credits)
{
    ...
    app_log("ancs_client_write_result_cb: handle=0x%x cause = 0x%x", handle, cause);
...
T_CLIENT_ID ancs_add_client(P_FUN_GENERAL_APP_CB app_cb, uint8_t link_num)
{
    ...
    {
        app_log("ancs_add_client: invalid link_num %d", link_num);
        return 0xff;
    }
    if (false == client_register_spec_client_cb(&ancs_client_id, &ancs_cbs))
    {
        ancs_client_id = CLIENT_PROFILE_GENERAL_ID;
        app_log("ancs_add_client Fail !!!");
        return ancs_client_id;
    }
    app_log("ancs_add_client: client ID = %d", ancs_client_id);
