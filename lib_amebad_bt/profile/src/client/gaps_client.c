...
#define app_log(M, ...)       MXOS_LOG(CONFIG_APP_DEBUG, "gaps", M, ##__VA_ARGS__)
...
{
    app_log("gaps_start_discovery");
    /* First clear handle cache. */
    if (conn_id >= gaps_link_num)
    {
        app_log("gaps_start_discovery: failed invalid conn_id %d", conn_id);
        return false;
    }
...
bool gaps_read(uint8_t conn_id, T_GAPS_READ_TYPE read_type)
{
    ...
    if (conn_id >= gaps_link_num)
    {
        app_log("gaps_read: failed invalid conn_id %d", conn_id);
        return false;
    }
    hdl_cache = gaps_table[conn_id].hdl_cache;

    app_log("gaps_read: charType = %d", read_type);
    ...
    app_log("gaps_read: Request fail! Please check!");
    return false;
}
...
bool gaps_get_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= gaps_link_num)
    {
        app_log("gaps_get_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (gaps_table[conn_id].disc_state != DISC_GAPS_DONE)
    {
        app_log("gaps_get_hdl_cache: failed invalid state %d", gaps_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_GAPS_CACHE_LEN)
    {
        app_log("gaps_get_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
bool gaps_set_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= gaps_link_num)
    {
        app_log("gaps_set_hdl_cache: failed invalid conn_id %d", conn_id);
        return false;
    }
    if (gaps_table[conn_id].disc_state != DISC_GAPS_IDLE)
    {
        app_log("gaps_set_hdl_cache: failed invalid state %d", gaps_table[conn_id].disc_state);
        return false;
    }
    if (len != sizeof(uint16_t) * HDL_GAPS_CACHE_LEN)
    {
        app_log("gaps_set_hdl_cache: failed invalid len %d", len);
        return false;
    }
...
static bool gaps_client_start_gap_char_discovery(uint8_t conn_id)
{
    ...
    app_log("gap_client_start_gap_char_discovery");
...
static void gaps_client_discover_state_cb(uint8_t conn_id,  T_DISCOVERY_STATE discovery_state)
{
    ...
    app_log("gaps_client_discover_state_cb: discovery_state %d", discovery_state);
...
{
    app_log("gap_client_discover_result_cb: result_type = %d", result_type);
    uint16_t handle;
    switch (result_type)
    {
    ...
    case DISC_RESULT_CHAR_UUID16:
        ...
        case GATT_UUID_CHAR_CENTRAL_ADDRESS_RESOLUTION:
            ...
            app_log("GATT_UUID_CHAR_CENTRAL_ADDRESS_RESOLUTION found");
            break;
        case GATT_UUID_CHAR_RESOLVABLE_PRIVATE_ADDRESS_ONLY:
            ...
            app_log("GATT_UUID_CHAR_RESOLVABLE_PRIVATE_ADDRESS_ONLY found");
            break;
        default:
                                       ...
                                       uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    app_log("gap_client_read_result_cb: handle 0x%x, cause 0x%x", handle, cause);
    ...
    if (handle == hdl_cache[HDL_GAPS_APPEARANCE])
    {
        ...
        if (cause == GAP_SUCCESS)
        {
            ...
            if (value_size != 2)
            {
                app_log("gaps_client_read_result_cb: invalid cccd len %d", value_size);
                return;
            }
...
{
    app_log("gap_client_disc_cb.");
    if (conn_id >= gaps_link_num)
    {
        app_log("gaps_client_disc_cb: failed invalid conn_id %d", conn_id);
        return;
    }
    memset(&gaps_table[conn_id], 0, sizeof(T_GAPS_LINK));
    return;
...
T_CLIENT_ID gaps_add_client(P_FUN_GENERAL_APP_CB app_cb, uint8_t link_num)
{
    ...
    {
        app_log("gaps_add_client: invalid link_num %d", link_num);
        return 0xff;
    }
    if (false == client_register_spec_client_cb(&gaps_client, &gaps_client_cbs))
    {
        gaps_client = CLIENT_PROFILE_GENERAL_ID;
        app_log("gaps_add_client Fail !!!");
        return gaps_client;
    }
    app_log("gaps_add_client: client ID = %d", gaps_client);
