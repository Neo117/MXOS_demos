...
#include "mxos_lwip_ethif_logging.h"
...
#if MXOS_CONFIG_IP_VER_PREF == 4
#define IP_VERSION_PREF             PREF_IPV4
#endif
#if MXOS_CONFIG_IP_VER_PREF == 6
...
static void mxos_eth_set_mac_address(void);
static void mxos_lwip_netif_link_irq(struct netif *lwip_netif);
static void mxos_lwip_netif_status_irq(struct netif *lwip_netif);

void mxos_eth_add_dns_addr();
...
merr_t mxos_eth_init( void )
{
    ...
    if (!mxos_eth_get_mac_address()) {
        ...
        netif_set_link_callback(&lwip_netif, mxos_lwip_netif_link_irq);
        netif_set_status_callback(&lwip_netif, mxos_lwip_netif_status_irq);
...
static const ip_addr_t *mxos_eth_get_ipv4_addr(void)
{
#if LWIP_IPV4
...
const ip_addr_t *mxos_eth_get_ip_addr(bool any_addr)
{
    ...
    pref_ip_addr = mxos_eth_get_ipv4_addr();
    npref_ip_addr = mxos_eth_get_ipv6_addr();
#else
    pref_ip_addr = mxos_eth_get_ipv6_addr();
    npref_ip_addr = mxos_eth_get_ipv4_addr();
#endif
...
void mxos_eth_add_dns_addr()
{
    ...
    const ip_addr_t *ip_addr = mxos_eth_get_ip_addr(true);
...
static void mxos_lwip_netif_status_irq(struct netif *lwip_netif)
{
    ...
    if (netif_is_up(lwip_netif)) {
        ...
        addr = mxos_eth_get_ip_addr(true);
        if (any_addr == true && addr) {
            eth_log("Ethernet interface has address: %s", ipaddr_ntoa(addr));
            any_addr = false;
            mxos_eth_add_dns_addr();
            mos_worker_send_async_event( MOS_WORKER_THREAD, notify_app_ethif_status_changed, (void *)NOTIFY_ETH_UP );

            return;
        }

        // Indicates that has preferred address
        addr = mxos_eth_get_ip_addr(false);
        if (addr) {
        ...
        any_addr = true;
        mos_worker_send_async_event( MOS_WORKER_THREAD, notify_app_ethif_status_changed, (void *)NOTIFY_ETH_DOWN );
...
struct netif *mxos_eth_get_if_handle(void)
{
    return &lwip_netif;
}
