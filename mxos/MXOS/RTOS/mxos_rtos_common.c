...
merr_t mxos_rtos_init( void )
{
    ...
    if ( result != kNoErr )
    {
        rtos_log("Failed to create MOS_WORKER_THREAD");
    }
    */

    result = mos_worker_thread_new( MOS_WORKER_THREAD, MOS_NETWORK_WORKER_PRIORITY, NETWORKING_WORKER_THREAD_STACK_SIZE, NETWORKING_WORKER_THREAD_QUEUE_SIZE );
    if ( result != kNoErr )
    {
        rtos_log("Failed to create MOS_WORKER_THREAD");
    }
...
merr_t mxos_rtos_deinit( void )
{
    ...
    if ( result == kNoErr )
    {
        result = mos_worker_thread_delete( MOS_WORKER_THREAD );
    }
