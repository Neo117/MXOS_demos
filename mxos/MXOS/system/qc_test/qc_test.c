...
typedef void (*qc_print_cb_t)( char* str);
void ble_qc_test(qc_print_cb_t printf_cb);

static void _qc_test_uart_init( void );
...
WEAK void user_qc_output(qc_print_cb_t qc_print)
{
}
...
{
    mxos_logic_partition_t *partition_flash = mhal_flash_get_info( MODULE_PARTITION_APPLICATION );
    ...
    while ( flash_len > 0 )
    {
        ...
        mhal_flash_read( MODULE_PARTITION_APPLICATION, &flash_addr, (uint8_t *) mfgbuf, buf_len );
...
WEAK void mxos_qc_hook(void)
{
}

static char *serial_number;

void mxos_set_serial_number(char *sn)
{
  serial_number = sn;
}
...
static void _qc_test_thread( void * arg )
{
    ...
    QC_TEST_PRINT_STRING( "Serial Number:", serial_number == NULL ? SERIAL_NUMBER : serial_number );
    ...
    user_qc_output(mf_printf);

    qc_scan( );

#ifdef QC_TEST_BLE_SCAN
    ble_qc_test(mf_printf);
#endif
