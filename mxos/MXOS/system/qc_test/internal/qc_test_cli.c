...
void mf_printf(char *str)
{
    if ((str == NULL) || (strlen(str)==0))
        return;
  mhal_uart_write( MXOS_APP_UART, str, strlen(str));
  mhal_uart_write( MXOS_LOG_UART, str, strlen(str));
}

void mf_putc(char ch)
{
  mhal_uart_write( MXOS_APP_UART, &ch, 1);
}
...
#define LF         0x0A
  ...
  *p_cmd_str = cmd_str;
  ...
  while(1) {
    size = 1;
    if( mhal_uart_read( MXOS_APP_UART, p, &size, 100) != kNoErr)
      continue;
