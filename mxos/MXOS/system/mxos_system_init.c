...
{
  /* Enter auto-conf mode only once in reboot mode, use MOS_WORKER_THREAD to save ram */
#ifdef  EasyLink_Needs_Reboot
    return mos_worker_send_async_event( MOS_WORKER_THREAD, system_config_mode_worker, NULL );
#else
...
{
  mhal_uart_write(MXOS_LOG_UART, &ch, 1);
}
...
static char cli_read_char(void)
{
  char ch;
  uint32_t n = 1;
  mhal_uart_read(MXOS_LOG_UART, &ch, &n, MOS_WAIT_FOREVER);
  return ch;
}
...
int cli_init(void)
{
    ...
    mxos_system_cli_cmds_add( );

    thread_id = mos_thread_new( MOS_APPLICATION_PRIORITY, "cli", (mos_thread_func_t) mcli_loop_run, MXOS_CLI_THREAD_STACK_SIZE, NULL );
...
merr_t mxos_system_init( void )
{
  ...
  system_easylink_btn_init( MXOS_ELINK_PIN, MXOS_CONFIG_EASYLINK_BTN_LONG_PRESS_TIMEOUT );
#endif
  ...
  err = mos_worker_thread_new( &wlan_autoconf_worker_thread, MOS_APPLICATION_PRIORITY, 0x1000, 1 );
