...
static merr_t _sys_will_power_off_handler(void *arg)
{
  ...
  mos_worker_register_timed_event( &_timed_sys_power_state_change, MOS_WORKER_THREAD, _sys_power_state_change_handler, 500, 0 );
...
merr_t mxos_system_power_perform( mxos_Context_t* const in_context, mxos_system_state_t new_state )
{
  ...
  mos_worker_send_async_event( MOS_WORKER_THREAD, _sys_will_power_off_handler, NULL );
