  ...
  if(err == kNoErr && strnicmpx( value, valueSize, kMIMEType_MXCHIP_OTA ) == 0){
    if(inPos==0 && inLen==0)
     ...
     if(inPos == 0){
       ...
       err = mhal_flash_erase( MODULE_PARTITION_OTA_TEMP, 0x0, ota_partition->partition_length);
       require_noerr(err, flashErrExit);
       err = mhal_flash_write( MODULE_PARTITION_OTA_TEMP, &context->offset, (uint8_t *)inData, inLen);
       require_noerr(err, flashErrExit);
