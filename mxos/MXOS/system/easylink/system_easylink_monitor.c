...
uint16_t mxos_easylinK_monitor_delay_switch(void);
/******************************************************
...
void mxos_easylink_monitor_set_lock_channel(uint8_t ch)
{
    if (ch < 1 || ch > 13)
        return;
    lock_channel = ch;
...
static void switch_channel_thread(void* arg)
{
    ...
    while(switch_channel_flag)
    {
        ...
        total_delay = 0;
        do {
            delay_time = mxos_easylinK_monitor_delay_switch();
            if (delay_time > 0) {
                total_delay += delay_time;
                if (total_delay > MAX_CHANNEL_DELAY_TIME) {
                    break;
                }
                mos_msleep(delay_time);
            }
...
WEAK uint16_t mxos_easylinK_monitor_delay_switch(void)
{
    return 0;
}
