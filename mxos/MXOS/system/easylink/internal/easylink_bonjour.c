...
static void _bonjour_generate_txt_record( char *txt_record, uint16_t txt_record_len, uint32_t easyLink_id,
                                          mwifi_if_t interface, system_context_t * const inContext )
{
    char *val = NULL;
    ...
    val = __strdup_trans_dot( inContext->mxosStatus.rf_version );
...
merr_t easylink_bonjour_start( mwifi_if_t interface, uint32_t easyLink_id, system_context_t * const inContext )
{
    ...
    easylink_service.port = MXOS_CONFIG_SERVER_PORT;
