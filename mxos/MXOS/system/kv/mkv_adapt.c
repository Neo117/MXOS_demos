...
{
    return mhal_flash_read(MODULE_PARTITION_KV, &offset, buf, nbytes);
}

int32_t kv_flash_write(uint32_t offset, void *buf, uint32_t nbytes)
{
    return mhal_flash_write(MODULE_PARTITION_KV, &offset, buf, nbytes);
}

int32_t kv_flash_erase(uint32_t offset, uint32_t size)
{
    return mhal_flash_erase(MODULE_PARTITION_KV, offset, size);
}
