 ...
 merr_t mxos_platform_init( void )
{
 ...
 merr_t MicoAdcInitialize( mhal_adc_t adc, uint32_t sampling_cycle )
 {
 ...
 merr_t  MicoAdcFinalize( mhal_adc_t adc )
 {
   if ( adc >= MXOS_ADC_NONE )
     return kUnsupportedErr;
 ...
 merr_t MicoAdcTakeSample( mhal_adc_t adc, uint16_t* output )
 {
   if ( adc >= MXOS_ADC_NONE )
     return kUnsupportedErr;
 ...
 merr_t MicoAdcTakeSampleStreram( mhal_adc_t adc, void* buffer, uint16_t buffer_length )
 {
   if ( adc >= MXOS_ADC_NONE )
     return kUnsupportedErr;
...
merr_t mhal_gpio_int_on( int gpio, mhal_gpio_irq_mode_t trigger, mhal_gpio_irq_handler_t handler,
                               void* arg )
{
    if ( gpio >= MXOS_GPIO_NONE )
        return kUnsupportedErr;
...
merr_t MicoI2cInitialize( mhal_i2c_device_t* device )
{
...
merr_t MicoI2cFinalize( mhal_i2c_device_t* device )
{
    platform_i2c_config_t config;
...
bool MicoI2cProbeDevice( mhal_i2c_device_t* device, int retries )
{
    bool ret;
    platform_i2c_config_t config;
...
merr_t MicoI2cBuildTxMessage( mhal_i2c_message_t* message, const void* tx_buffer, uint16_t tx_buffer_length,
                                uint16_t retries )
{
    return (merr_t) platform_i2c_init_tx_message( message, tx_buffer, tx_buffer_length, retries );
}

merr_t MicoI2cBuildRxMessage( mhal_i2c_message_t* message, void* rx_buffer, uint16_t rx_buffer_length,
                                uint16_t retries )
{
    return (merr_t) platform_i2c_init_rx_message( message, rx_buffer, rx_buffer_length, retries );
}

merr_t MicoI2cBuildCombinedMessage( mhal_i2c_message_t* message, const void* tx_buffer, void* rx_buffer,
...
merr_t MicoI2cTransfer( mhal_i2c_device_t* device, mhal_i2c_message_t* messages, uint16_t number_of_messages )
{
    merr_t err = kNoErr;
    platform_i2c_config_t config;
...
merr_t mhal_spi_send_and_recv( const mhal_spi_dev_t* spi, const mhal_spi_message_segment_t* segments,
...
// merr_t MicoSpiSlaveInitialize( int spi, const mhal_spi_slave_config_t* config )
// {
...
// merr_t MicoSpiSlaveFinalize( int spi )
// {
//   if ( spi >= MXOS_SPI_NONE )
//     return kUnsupportedErr;
...
// merr_t  MicoSpiSlaveSendErrorStatus( int spi, mhal_spi_slave_transfer_status_t error_status )
// {
//   if ( spi >= MXOS_SPI_NONE )
//     return kUnsupportedErr;
...
// merr_t MicoSpiSlaveReceiveCommand( int spi, mhal_spi_slave_command_t* command, uint32_t timeout_ms )
// {
//   if ( spi >= MXOS_SPI_NONE )
//     return kUnsupportedErr;  
...
// merr_t MicoSpiSlaveTransferData( int spi, mhal_spi_slave_transfer_direction_t direction, mhal_spi_slave_data_buffer_t* buffer, uint32_t timeout_ms )
// {
//   if ( spi >= MXOS_SPI_NONE )
//     return kUnsupportedErr;  
...
// merr_t MicoSpiSlaveGenerateInterrupt( int spi, uint32_t pulse_duration_ms )
// {
//   if ( spi >= MXOS_SPI_NONE )
//     return kUnsupportedErr;
...
static merr_t mhal_flash_open( int partition )
{
    merr_t err = kNoErr;
...
merr_t mhal_flash_write( int partition, volatile uint32_t* off_set, uint8_t* inBuffer,
...
merr_t mhal_flash_read( int partition, volatile uint32_t* off_set, uint8_t* outBuffer,
                        uint32_t inBufferLength )
{
    merr_t err = kNoErr;
    uint32_t start_addr, end_addr;
