...
static void irq_handler_asynch(void* context)
{
    ...
    if ( (driver->event & (SPI_EVENT_ALL)) && driver->transfer_complete ) {
        ...
        mos_semphr_release( driver->transfer_complete );
    }
}

merr_t platform_spi_init( platform_spi_driver_t* driver, const platform_spi_t* peripheral,
                            const platform_spi_config_t* config )
{
    ...
    if ( config->mode & SPI_USE_DMA ){
        ...
        {
            driver->transfer_complete = mos_semphr_new(1);
        }
    }
#endif
...
merr_t platform_spi_transfer( platform_spi_driver_t* driver, const platform_spi_config_t* config,
