...
merr_t iflash_erase( uint32_t StartAddress, uint32_t EndAddress )
{
  merr_t err = kNoErr;
...
static merr_t iflash_write_word(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength)
{
    merr_t err = kNoErr;
    uint32_t i = 0;
...
merr_t iflash_write(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength)
{
    merr_t err = kNoErr;
    uint32_t i = 0;
    uint32_t temp_data;
