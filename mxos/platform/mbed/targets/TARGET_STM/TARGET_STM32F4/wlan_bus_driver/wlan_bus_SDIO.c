...
#if !(defined (MXOS_DISABLE_MCU_POWERSAVE)) && !(defined (SDIO_1_BIT)) //SDIO 4 Bit mode and enable MCU powersave, need an OOB interrupt
...
{
    return MXOS_WIFI_OOB_IRQ_GPIO_PIN;
}
...
merr_t host_platform_bus_init( void )
{
    ...
    sdio_transfer_finished_semaphore = mos_semphr_new(1);
    if ( sdio_transfer_finished_semaphore == NULL )
    {
...
#ifdef MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_0
...
#endif //MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_0

#ifdef MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_1
    platform_gpio_init          ( &wifi_control_pin_drivers[WIFI_PIN_BOOTSTRAP_1], &wifi_control_pins[WIFI_PIN_BOOTSTRAP_1], OUTPUT_PUSH_PULL );
#ifdef MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_1_HIGH_FOR_SDIO
...
#endif //MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_1_HIGH_FOR_SDIO
#endif  //MXOS_WIFI_USE_GPIO_FOR_BOOTSTRAP_1
...
merr_t host_platform_bus_deinit( void )
{
    ...
    sdio_transfer_finished_semaphore = NULL;
...
restart:
    ...
    if ( command == SDIO_CMD_53 )
    {
        ...
        result = mos_semphr_acquire( sdio_transfer_finished_semaphore, 50 );
        if ( result != kNoErr )
        {
            goto exit;
        }
