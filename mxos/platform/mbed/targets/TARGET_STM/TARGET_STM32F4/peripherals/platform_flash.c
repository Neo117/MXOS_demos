...
#if defined USE_SPI_FLASH || defined USE_QUAD_SPI_FLASH
...
merr_t platform_flash_init( const platform_flash_t *peripheral )
{
  ...
  if( peripheral->flash_type == FLASH_TYPE_EMBEDDED ){
    ...
    err = qsflash_init( &platform_qspi_peripherals[MXOS_QSPI_1], peripheral, &sflash_handle, SFLASH_WRITE_ALLOWED );
    require_noerr(err, exit);
  }
#endif
#endif
