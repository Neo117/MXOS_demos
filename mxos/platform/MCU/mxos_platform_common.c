...
merr_t mhal_adc_open( mhal_adc_t adc, uint32_t sampling_cycle )
{
...
merr_t  mhal_adc_close( mhal_adc_t adc )
{
  if ( adc >= MXOS_ADC_NONE )
    return kUnsupportedErr;
...
uint16_t mhal_adc_get_bit_range( mhal_adc_t adc )
{
    if ( adc >= MXOS_ADC_NONE )
      return kUnsupportedErr;
...
merr_t mhal_adc_read( mhal_adc_t adc, uint16_t* output )
{
  if ( adc >= MXOS_ADC_NONE )
    return kUnsupportedErr;
...
merr_t mhal_adc_take_sampleStream( mhal_adc_t adc, void* buffer, uint16_t buffer_length )
{
  if ( adc >= MXOS_ADC_NONE )
    return kUnsupportedErr;
...
merr_t mhal_gpio_open( int gpio, mhal_gpio_mode_t configuration )
{
...
merr_t mhal_gpio_high( int gpio )
{
  if ( gpio >= MXOS_GPIO_NONE )
    return kUnsupportedErr;
...
merr_t mhal_gpio_toggle( int gpio )
{
  if ( gpio >= MXOS_GPIO_NONE )
    return kUnsupportedErr;
...
merr_t mhal_gpio_close( int gpio )
{
  if ( gpio >= MXOS_GPIO_NONE )
    return kUnsupportedErr;
...
merr_t mhal_gpio_int_on( int gpio, mhal_gpio_irq_mode_t trigger, mhal_gpio_irq_handler_t handler, void* arg )
{
  if ( gpio >= MXOS_GPIO_NONE )
    return kUnsupportedErr;
...
merr_t mhal_gpio_int_off( int gpio )
{
  if ( gpio >= MXOS_GPIO_NONE )
    return kUnsupportedErr;
...
merr_t mhal_i2c_open( mhal_i2c_device_t* device )
{
  mhal_i2c_config_t config;
  merr_t result;
...
merr_t mhal_i2c_close( mhal_i2c_device_t* device )
{
  mhal_i2c_config_t config;
...
bool mhal_i2c_probe_dev( mhal_i2c_device_t* device, int retries )
{
  bool ret;
  mhal_i2c_config_t config;
...
merr_t mhal_i2c_build_tx_msg( mhal_i2c_message_t* message, const void* tx_buffer, uint16_t  tx_buffer_length, uint16_t retries )
{
  return (merr_t) platform_i2c_init_tx_message( message, tx_buffer, tx_buffer_length, retries );
}

merr_t mhal_i2c_build_rx_msg( mhal_i2c_message_t* message, void* rx_buffer, uint16_t rx_buffer_length, uint16_t retries )
{
  return (merr_t) platform_i2c_init_rx_message( message, rx_buffer, rx_buffer_length, retries );
}

merr_t mhal_i2c_build_comb_msg( mhal_i2c_message_t* message, const void* tx_buffer, void* rx_buffer, uint16_t tx_buffer_length, uint16_t rx_buffer_length, uint16_t retries )
{
  return (merr_t) platform_i2c_init_combined_message( message, tx_buffer, rx_buffer, tx_buffer_length, rx_buffer_length, retries );
}

merr_t mhal_i2c_send_and_recv( mhal_i2c_device_t* device, mhal_i2c_message_t* messages, uint16_t number_of_messages )
{
  merr_t err = kNoErr;
  mhal_i2c_config_t config;
...
merr_t mhal_spi_open( const mhal_spi_dev_t* spi )
{
...
merr_t mhal_spi_close( const mhal_spi_dev_t* spi )
{
  merr_t err = kNoErr;
...
merr_t mhal_spi_send_and_recv( const mhal_spi_dev_t* spi, const mhal_spi_message_segment_t* segments, uint16_t number_of_segments )
{
  platform_spi_config_t config;
  merr_t err = kNoErr;
...
#if 0
merr_t MxosSpiSlaveInitialize( int spi, const mhal_spi_slave_config_t* config )
{
...
merr_t MxosSpiSlaveFinalize( int spi )
{
  if ( spi >= MXOS_SPI_NONE )
    return kUnsupportedErr;
...
merr_t  MxosSpiSlaveSendErrorStatus( int spi, mhal_spi_slave_transfer_status_t error_status )
{
  if ( spi >= MXOS_SPI_NONE )
    return kUnsupportedErr;
...
merr_t MxosSpiSlaveReceiveCommand( int spi, mhal_spi_slave_command_t* command, uint32_t timeout_ms )
{
  if ( spi >= MXOS_SPI_NONE )
    return kUnsupportedErr;  
...
merr_t MxosSpiSlaveTransferData( int spi, mhal_spi_slave_transfer_direction_t direction, mhal_spi_slave_data_buffer_t* buffer, uint32_t timeout_ms )
{
  if ( spi >= MXOS_SPI_NONE )
    return kUnsupportedErr;  
...
merr_t MxosSpiSlaveGenerateInterrupt( int spi, uint32_t pulse_duration_ms )
{
  if ( spi >= MXOS_SPI_NONE )
    return kUnsupportedErr;
...
#endif
merr_t mhal_uart_open( int uart, const mhal_uart_config_t* config, ring_buffer_t* optional_rx_buffer )
{
...
merr_t mhal_stdio_uart_open( const mhal_uart_config_t* config, ring_buffer_t* optional_rx_buffer )
{

    return (merr_t) platform_uart_init( &platform_uart_drivers[MXOS_LOG_UART],
                                          &platform_uart_peripherals[MXOS_LOG_UART],
...
mxos_logic_partition_t* mhal_flash_get_info( int inPartition )
{
    mxos_logic_partition_t *logic_partition = NULL;
    require( inPartition >= 0 && inPartition < MODULE_PARTITION_MAX, exit );
...
static merr_t MxosFlashInitialize( int partition )
{
  merr_t err = kNoErr;
  mxos_logic_partition_t *partition_info;
  
  require_action_quiet( partition > MODULE_PARTITION_ERROR, exit, err = kParamErr );
  require_action_quiet( partition < MODULE_PARTITION_MAX, exit, err = kParamErr );
  ...
  platform_flash_drivers[ partition_info->partition_owner ].peripheral = (int *)&platform_flash_peripherals[ partition_info->partition_owner ];
...
merr_t mhal_flash_erase(int partition, uint32_t off_set, uint32_t size)
{
  merr_t err = kNoErr;
  ...
  require_action_quiet( partition > MODULE_PARTITION_ERROR && partition < MODULE_PARTITION_MAX, exit, err = kParamErr );
  require_action_quiet( partition < MODULE_PARTITION_MAX, exit, err = kParamErr );
...
merr_t mhal_flash_write( int partition, volatile uint32_t* off_set, uint8_t* inBuffer ,uint32_t inBufferLength)
{
  merr_t err = kNoErr;
  uint32_t start_addr, end_addr;
  mxos_logic_partition_t *partition_info;
...
merr_t mhal_flash_read( int partition, volatile uint32_t* off_set, uint8_t* outBuffer ,uint32_t inBufferLength)
{
  merr_t err = kNoErr;
  uint32_t start_addr, end_addr;
  mxos_logic_partition_t *partition_info;
...
merr_t mxos_flash_enable_security( int partition, uint32_t off_set, uint32_t size )
{
  merr_t err = kNoErr;
  uint32_t start_addr, end_addr;
  mxos_logic_partition_t *partition_info;
...
char *mxos_get_bootloader_ver(void)
{
  ...
  #else
  static char ver[33];
  const mxos_logic_partition_t* bootloader_partition = &mxos_partitions[ MODULE_PARTITION_BOOTLOADER ];
  uint32_t version_offset = bootloader_partition->partition_length - 0x20;

  memset(ver, 0, sizeof(ver));
  mhal_flash_read( MODULE_PARTITION_BOOTLOADER, &version_offset, (uint8_t *)ver , 32);
  return ver;
  #endif
}
...
{
   uint8_t ver[33];
   mxos_logic_partition_t *boot_partition = mhal_flash_get_info( MODULE_PARTITION_BOOTLOADER );
   uint32_t flashaddr =  boot_partition->partition_length - 0x20;
   int i;

   memset(ver, 0, sizeof(ver));
   mhal_flash_read( MODULE_PARTITION_BOOTLOADER, &flashaddr, (uint8_t *)ver , 32);
   for(i=0;i<32;i++) {
       if (ver[i] != 0xFF)
           return;
   }
   snprintf((char *)ver, 33, "%s %s %d", MODEL, Bootloader_REVISION , MXOS_LOG_UART_BAUDRATE);
   flashaddr =  boot_partition->partition_length - 0x20;
   mxos_flash_disable_security( MODULE_PARTITION_BOOTLOADER, 0x0, boot_partition->partition_length );
   mhal_flash_write( MODULE_PARTITION_BOOTLOADER, &flashaddr, ver , 32);
}

merr_t mxos_flash_disable_security( int partition, uint32_t off_set, uint32_t size )
{
  merr_t err = kNoErr;
  uint32_t start_addr, end_addr;
  mxos_logic_partition_t *partition_info;
