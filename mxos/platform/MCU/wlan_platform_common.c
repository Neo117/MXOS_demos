 ...
 * @version V1.0.0
 * @date    05-May-2014
 * @brief   This file provide functions called by MXOS to wlan RF module
 ******************************************************************************
 ...
 ******************************************************/
merr_t host_platform_deinit_wlan_powersave_clock( void );
...
{
#if   defined ( MXOS_USE_WIFI_POWER_PIN ) && defined ( MXOS_USE_WIFI_POWER_PIN_ACTIVE_HIGH )
...
#elif defined ( MXOS_USE_WIFI_POWER_PIN )
...
merr_t host_platform_init( void )
{
...
#if defined ( MXOS_USE_WIFI_POWER_PIN )
...
MXOS_WEAK merr_t host_platform_init_wlan_powersave_clock( void )
{
...
merr_t host_platform_deinit( void )
{
#if defined ( MXOS_USE_WIFI_RESET_PIN )
    platform_gpio_init( &wifi_control_pins[WIFI_PIN_RESET], OUTPUT_PUSH_PULL );
    host_platform_reset_wifi( true );  /* Start wifi chip in reset */
...
#ifdef MXOS_USE_BUILTIN_RF_DRIVER
...
#define READ_LEN 2048
    mxos_logic_partition_t *driver_partition = mhal_flash_get_info( MODULE_PARTITION_RF_FIRMWARE );
    ...
    do {
        ...
        mhal_flash_read( MODULE_PARTITION_RF_FIRMWARE, &offset, (uint8_t *)buf, READ_LEN);
        offset -= READ_LEN; // mhal_flash_read will increase FlashAddress READ_LEN, move back.
...
uint32_t platform_get_wifi_image(unsigned char* buffer, uint32_t size, uint32_t offset)
{
    ...
    mhal_flash_read( MODULE_PARTITION_RF_FIRMWARE, &read_address, buffer, buffer_size);
    return buffer_size;
}
#endif
