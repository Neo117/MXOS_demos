...
static merr_t _GetAddress(uint32_t sector, uint32_t *startAddress, uint32_t *endAddress);
static merr_t internalFlashInitialize( void );
static merr_t internalFlashErase(uint32_t StartAddress, uint32_t EndAddress);
static merr_t internalFlashWrite(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength);
static merr_t internalFlashByteWrite( volatile uint32_t* FlashAddress, uint8_t* Data ,uint32_t DataLength );
...
#endif
#ifdef USE_QUAD_SPI_FLASH
static merr_t qspiFlashErase( uint32_t StartAddress, uint32_t EndAddress );
...
merr_t platform_flash_init( const platform_flash_t *peripheral )
{
  ...
  if( peripheral->flash_type == FLASH_TYPE_EMBEDDED ){
    ...
    err = init_qsflash( &platform_qspi_peripherals[MXOS_QSPI_1], peripheral->flash_length, &sflash_handle, SFLASH_WRITE_ALLOWED );
    require_noerr(err, exit);
  }
  #endif
#endif
...
merr_t internalFlashInitialize( void )
{ 
  platform_log_trace();
  FLASH_Unlock(); 
...
merr_t internalFlashErase(uint32_t StartAddress, uint32_t EndAddress)
{
  platform_log_trace();
  merr_t err = kNoErr;
...
#ifdef MCU_EBANLE_FLASH_PROTECT
merr_t internalFlashProtect(uint32_t StartAddress, uint32_t EndAddress, bool enable)
{
  merr_t err = kNoErr;
  uint16_t WRP = 0x0;
...
merr_t internalFlashWrite(volatile uint32_t* FlashAddress, uint32_t* Data ,uint32_t DataLength)
{
  platform_log_trace();
  merr_t err = kNoErr;
  uint32_t i = 0;
...
merr_t internalFlashByteWrite(__IO uint32_t* FlashAddress, uint8_t* Data ,uint32_t DataLength)
{
  uint32_t i = 0;
  uint32_t dataInRam;
  merr_t err = kNoErr;
...
#ifdef USE_QUAD_SPI_DMA
  const platform_qspi_t* peripheral = &platform_qspi_peripherals[MXOS_QSPI_1];
#else
  uint8_t* buffer = NULL;
#endif
