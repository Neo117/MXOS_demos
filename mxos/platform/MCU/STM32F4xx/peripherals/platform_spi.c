...
static merr_t calculate_prescaler( uint32_t speed, uint16_t* prescaler );
static uint16_t spi_transfer       ( const platform_spi_t* spi, uint16_t data );
static merr_t spi_dma_transfer   ( const platform_spi_t* spi, const platform_spi_config_t* config );
