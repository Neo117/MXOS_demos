 ...
 * @version V1.0.0
 * @date    05-May-2014
 * @brief   This file provide functions called by MXOS to drive stm32f2xx
...
merr_t stdio_hardfault( char* data, uint32_t size )
{
  ...
  for(idx = 0; idx < size; idx++){
    while ( ( platform_uart_peripherals[ MXOS_LOG_UART ].port->SR & USART_SR_TXE ) == 0 );
    platform_uart_peripherals[ MXOS_LOG_UART ].port->DR = (data[idx] & (uint16_t)0x01FF);
...
const char *mxos_generate_cid( uint8_t* length )
{
