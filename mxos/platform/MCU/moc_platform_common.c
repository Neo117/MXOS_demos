...
merr_t mhal_adc_take_sampleStreram( mhal_adc_t adc, void* buffer, uint16_t buffer_length )
{
  if ( adc >= MXOS_ADC_NONE )
    return kUnsupportedErr;
...
merr_t mhal_i2c_open( mhal_i2c_device_t* device )
{
  ...
  if( platform_i2c_drivers[device->port].i2c_mutex == NULL)
    mos_mutex_new( &platform_i2c_drivers[device->port].i2c_mutex );
...
merr_t mhal_rtc_get_time(mhal_rtc_time_t* time)
{
  return (merr_t) platform_rtc_get_time( time );
}

merr_t mhal_rtc_set_time(mhal_rtc_time_t* time)
{
  return (merr_t) platform_rtc_set_time( time );
}
...
merr_t mhal_spi_open( const mhal_spi_dev_t* spi )
{
  ...
  if( platform_spi_drivers[spi->port].spi_mutex == NULL)
    mos_mutex_new( &platform_spi_drivers[spi->port].spi_mutex );
...
static merr_t MxosFlashInitialize( int partition )
{
  ...
  if( platform_flash_drivers[ partition_info->partition_owner ].flash_mutex == NULL){
    err = mos_mutex_new( &platform_flash_drivers[ partition_info->partition_owner ].flash_mutex );
...
merr_t mhal_flash_write( int partition, volatile uint32_t* off_set, uint8_t* inBuffer ,uint32_t inBufferLength)
{
  ...
  require_action_quiet( end_addr < partition_info->partition_start_addr + mxos_partitions[ partition ].partition_length , exit, err = kParamErr );
