...
{
	return lib_api_p->mwifi_softap_startEnt(attr);
}
...
merr_t mhal_flash_erase(int inPartition, uint32_t off_set, uint32_t size)
{
	return lib_api_p->mhal_flash_erase(inPartition, off_set, size);
}
merr_t mhal_flash_write( int inPartition, volatile uint32_t* off_set, uint8_t* inBuffer ,uint32_t inBufferLength)
{
	 lib_api_p->mhal_flash_write(inPartition, off_set, inBuffer, inBufferLength);
	 return 0;
}
merr_t mhal_flash_read( int inPartition, volatile uint32_t* off_set, uint8_t* outBuffer, uint32_t inBufferLength)
{
...
{
    return lib_api_p->mhal_uart_open(MXOS_LOG_UART, config, optional_rx_buffer );
}
...
merr_t MxosUartPinRedirect( int uart )
{
...
{
	lib_api_p->mhal_rtc_open();
	return kNoErr;
}

merr_t mhal_rtc_get_time(time_t* time)
{
	return lib_api_p->mhal_rtc_get_time(time);
}

merr_t mhal_rtc_set_time(time_t time)
{
	return lib_api_p->mhal_rtc_set_time(time);
}
...
{
	return lib_api_p->adc_apis->mhal_adc_open(adc, sampling_cycle);
}

merr_t  mhal_adc_close( mhal_adc_t adc )
{
    return lib_api_p->adc_apis->mhal_adc_close(adc);
}

merr_t mhal_adc_read( mhal_adc_t adc, uint16_t* output )
{
    return lib_api_p->adc_apis->mhal_adc_read(adc, output);
}

merr_t mhal_adc_take_sampleStreram( mhal_adc_t adc, void* buffer, uint16_t buffer_length )
{
    return lib_api_p->adc_apis->mhal_adc_take_sampleStreram(adc, buffer, buffer_length);
}
...
{
    uint32_t d;
    mhal_rng_read((unsigned char*)&d, 4);
    return d;
}
