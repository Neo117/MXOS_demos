...
typedef struct
{
    merr_t (*i2c_init)( mhal_i2c_device_t* device );
    merr_t (*i2c_deinit)( mhal_i2c_device_t* device );
    bool (*i2c_probe_device)( mhal_i2c_device_t* device, int retries );
    merr_t (*i2c_build_tx_msg)( mhal_i2c_message_t* message, const void* tx_buffer, uint16_t tx_buffer_length,
                                  uint16_t retries );
    merr_t (*i2c_build_rx_msg)( mhal_i2c_message_t* message, void* rx_buffer, uint16_t rx_buffer_length,
                                  uint16_t retries );
    merr_t (*i2c_build_combined_msg)( mhal_i2c_message_t* message, const void* tx_buffer, void* rx_buffer,
                                        uint16_t tx_buffer_length, uint16_t rx_buffer_length, uint16_t retries );
    merr_t (*i2c_transfer)( mhal_i2c_device_t* device, mhal_i2c_message_t* messages, uint16_t number_of_messages );
} i2c_api_t;

typedef struct
{
    merr_t (*spi_init)( const mhal_spi_dev_t* spi );
    merr_t (*spi_transfer)( const mhal_spi_dev_t* spi, const mhal_spi_message_segment_t* segments,
                              uint16_t number_of_segments );
    merr_t (*spi_finalize)( const mhal_spi_dev_t* spi );
} spi_api_t;

typedef struct {
    merr_t (*iis_init)( const mhal_i2s_dev_t* iis );
    merr_t (*iis_finalize)( const mhal_i2s_dev_t* iis );
	merr_t (*iis_transfer)( const mhal_i2s_dev_t* iis, const mhal_i2s_message_segment_t* segments,
                              uint16_t number_of_segments );
    merr_t (*iis_write)( const mhal_i2s_dev_t* iis, uint8_t *p_buf, uint32_t size );
    merr_t (*iis_read)( const mhal_i2s_dev_t* iis, uint8_t *p_buf, uint32_t size );
}iis_api_t;
...
typedef struct mxos_api_struct
{
    ...
    void (*mos_msleep)( uint32_t seconds );
    ...
    /* SSL */
    void (*mtls_set_cert)( const char *_cert_pem, const char *private_key_pem );
    void* (*mtls_connect)( int fd, int calen, char*ca, int *ssl_errno );
    void* (*mtls_accept)( int fd );
    int (*mtls_send)( void* ssl, char *data, int len );
    int (*mtls_recv)( void* ssl, char *data, int len );
    int (*mtls_close)( void* ssl );
    ...
    merr_t (*mhal_flash_write)( int inPartition, volatile uint32_t* off_set, uint8_t* inBuffer,
                                uint32_t inBufferLength );
    merr_t (*mhal_flash_read)( int inPartition, volatile uint32_t* off_set, uint8_t* outBuffer,
                               uint32_t inBufferLength );
    ...
    merr_t (*mhal_uart_open)( int uart, const mhal_uart_config_t* config,
    ...
    void (*MxosUartPinRedirect)( int uart );
    ...
    int (*mtls_socket)( void* ssl );
	int (*mxos_wlan_get_channel)(void);
	
	int (*mtls_pending)(void* ssl);
	int (*mtls_get_error)(void* ssl, int ret);
	void (*mtls_set_set_nonblock)(void* ssl, int nonblock);
	...
	void* (*mtls_connect_sni)(int fd, int calen, char*ca, char *sni_servername, int *errno);
