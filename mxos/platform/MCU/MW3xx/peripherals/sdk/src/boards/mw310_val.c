...
const platform_gpio_t platform_gpio_pins[] =
{
    [BOOT_SEL]        = MXCHIP_GPIO(39),
  	...
  	[MXOS_ELINK_PIN] = MXCHIP_GPIO(1),
  	[MXOS_SYS_LED]    = MXCHIP_GPIO(23),
  	[MXOS_RF_LED]     = MXCHIP_GPIO(24),
};
#else
/* MXCHIP board */
const platform_gpio_t platform_gpio_pins[] =
{
    [BOOT_SEL]        = MXCHIP_GPIO(4),
  	...
  	[MXOS_ELINK_PIN] = MXCHIP_GPIO(6),
  	[MXOS_SYS_LED]    = MXCHIP_GPIO(16),
  	//[MXOS_RF_LED]     = MXCHIP_GPIO(24),
  	[MXOS_GPIO_12]    = MXCHIP_GPIO(0),
  	[MXOS_GPIO_13]    = MXCHIP_GPIO(1),
  	[MXOS_GPIO_14]    = MXCHIP_GPIO(2),
  	[MXOS_GPIO_15]    = MXCHIP_GPIO(3),
};
#endif
...
{
  if ((mhal_gpio_value((int)BOOT_SEL) == false) &&
	  (mhal_gpio_value((int)MXOS_ELINK_PIN) == true) &&
	  (mhal_gpio_value((int)MFG_SEL) == true))
	  return true;
  else
  	return false;
}
...
{
  if ((mhal_gpio_value((int)BOOT_SEL) == false) &&
	  (mhal_gpio_value((int)MFG_SEL) == false))
	  return true;
  else
  	return false;
}
...
{
  if ((mhal_gpio_value((int)BOOT_SEL) == false) &&
	  (mhal_gpio_value((int)MXOS_ELINK_PIN) == false) &&
	  (mhal_gpio_value((int)MFG_SEL) == true))
	  return true;
  else
  	return false;
