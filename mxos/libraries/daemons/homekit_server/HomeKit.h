...
typedef merr_t (*hk_key_storage_cb)(unsigned char *pk, unsigned char *sk, bool write);
...
merr_t hk_server_start( hk_init_t init );

merr_t hk_server_update_attri_db( hapProduct_t *hap_product, int config_number );

merr_t hk_server_notify_by_bonjour( void );
