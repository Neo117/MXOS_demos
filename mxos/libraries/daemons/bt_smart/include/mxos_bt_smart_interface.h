...
/** @file
 *  Defines common constants and types for MXOS support for Bluetooth Smart
 */
...
} mxos_bt_smart_filter_duplicated_t;
...
} mxos_bt_smart_advertising_event_t;
...
} mxos_bt_smart_advertising_type_t;
...
typedef enum 
{
    /* Bonding event -- user should reply it with #mxos_bt_smart_bond_reply() */
    MXOS_BT_SMART_BOND_PASS_KEY_REQ     = 0x01,
    /* Bonding event -- user should reply it with #mxos_bt_smart_bond_reply() */
    MXOS_BT_SMART_BOND_USR_CONFIRM_REQ  = 0x02,
    /* Bonding event -- user should reply it with #mxos_bt_smart_bond_reply() */
    MXOS_BT_SMART_BOND_OOB_DATA_REQ     = 0x03,
    /* Bonding event -- user should not reply it */
    MXOS_BT_SMART_BOND_PASS_KEY_NOTIFY  = 0x04,
} mxos_bt_smart_bond_request_type_t;
...
} mxos_bt_smart_key_distribution_t;
...
typedef struct
{
    mxos_bt_device_address_t            address;      /**< Bluetooth device address */
    mxos_bt_smart_address_type_t        address_type; /**< Address Type             */
    char                                name[31];     /**< User-friendly name       */
} mxos_bt_smart_device_t;
...
typedef struct
{
    mxos_bt_smart_scan_type_t          type;                 /**< Scan type                                                                                     */
    mxos_bt_smart_filter_policy_t      filter_policy;        /**< Scan filter policy                                                                            */
    mxos_bt_smart_filter_duplicated_t  filter_duplicates;    /**< Scan duplicates filter                                                                        */
...
typedef struct
{
    mxos_bt_smart_device_t              remote_device;   /**< Remote device                                      */
    int8_t                              signal_strength; /**< RSSI in dBm                                        */
    mxos_bt_smart_advertising_event_t   event;           /**< Advertising event received                         */
...
} mxos_bt_smart_advertising_report_t;

/**
 *  Bluetooth Smart scan result
 */
typedef struct mxos_bt_smart_scan_result
{
    mxos_bt_smart_device_t              remote_device;                   /**< Remote device                       */
    int8_t                              signal_strength;                 /**< RSSI in dBm                         */
    mxos_bt_smart_advertising_report_t  last_scan_response_received;     /**< Last scan response event received   */
    mxos_bt_smart_advertising_report_t  last_advertising_event_received; /**< Last advertising event received     */
    struct mxos_bt_smart_scan_result*   next;                            /**< Pointer to the next scan result     */

    /* Additional flag to help application filter scan results */
    mxos_bool_t                         filter_display;                  /**< Set to MXOS_TRUE if filter display */

} mxos_bt_smart_scan_result_t;
...
typedef struct
{
    mxos_bt_smart_advertising_type_t    type;                               /**< Advertising type                                               */
    mxos_bool_t                         use_high_duty;                      /**< Start advertising use high duty cycle interval                 */
    ...
    mxos_bt_smart_address_type_t        directed_advertisement_addr_type;   /**< Target device address type for directed advertising            */
    mxos_bt_device_address_t            directed_advertisement_addr;        /**< Target device address for directed advertising                 */
} mxos_bt_smart_advertising_settings_t;
...
typedef struct
{
    ...
    mxos_bt_smart_filter_policy_t       filter_policy;                 /**< Connection initiator filter policy: No filter or using white list                          */
...
} mxos_bt_smart_connection_settings_t;
...
} mxos_bt_smart_eir_data_structure_t;
...
typedef struct
{
    ...
    mxos_bt_io_capabilities_t         io_capabilities;             /**< Device I/O capability                         */
    mxos_bt_smart_auth_req_t          authentication_requirements; /**< Authentication requirements                   */
    mxos_bt_smart_oob_auth_t          oob_authentication;          /**< OOB authentication data                       */
    uint8_t                           max_encryption_key_size;     /**< Encryption key size (7 to 16 bytes)           */
    mxos_bt_smart_key_distribution_t  master_key_distribution;     /**< Bit mask of master/initiator key distribution */
    mxos_bt_smart_key_distribution_t  slave_key_distribution;      /**< Bit mask of slave/responder key distribution  */
} mxos_bt_smart_security_settings_t;
...
typedef struct
{
    mxos_bt_device_address_t      peer_address; /**< Bonded peer device address                                                                            */
    mxos_bt_smart_address_type_t  address_type; /**< Peer device's address type                                                                            */
...
{
    /* Type of the request */
    mxos_bt_smart_bond_request_type_t type;

    union 
    {
        /* MXOS_BT_SMART_BOND_PASS_KEY_REQ */
        struct 
        {
            mxos_bt_device_address_t addr;
        } passkey;

        /* MXOS_BT_SMART_BOND_USR_CONFIRM_REQ */
        struct 
        {
            mxos_bt_device_address_t addr;
        ...
        /* MXOS_BT_SMART_BOND_OOB_DATA_REQ */
        struct 
        {
            mxos_bt_device_address_t addr;
        } oob_data;

        /* MXOS_BT_SMART_BOND_PASS_KEY_NOTIFY */
        struct 
        {
            mxos_bt_device_address_t addr;
            uint32_t                 passkey;
...
typedef struct 
{
    ...
    /* reply status */
    mxos_bt_result_t res;

    /* reply data -- only valid when 'res' is MXOS_BT_SUCCESS */
    union 
    {
        /* Used for MXOS_BT_SMART_BOND_USER_CONFIRM */
        struct 
        {
            mxos_bt_device_address_t    addr;
        } confirm;

        /* Used for MXOS_BT_SMART_BOND_PASS_KEY_REQ */
        struct 
        {
            mxos_bt_device_address_t    addr;
            uint32_t                    passkey;
        ...
        /* Used for MXOS_BT_SMART_BOND_OOB_DATA_REQ */
        struct 
        {
            mxos_bt_device_address_t    addr;
            uint8_t                    *data;
 ...
 */
typedef event_handler_t mxos_bt_smart_scan_complete_callback_t;

/**
 * Bluetooth Smart advertising report callback
 */
typedef merr_t (*mxos_bt_smart_advertising_report_callback_t)  ( const mxos_bt_smart_advertising_report_t* result );

/**
 * Bluetooth Smart Device Bond Callback.
 */
typedef merr_t (*mxos_bt_smart_bonding_callback_t)(const mxos_bt_smart_bond_request_t *request);

/**
 * Bluetooth Smart advertising complete callback
 */
typedef event_handler_t mxos_bt_smart_advertising_complete_callback_t;
 ...
 *  @return merr_t
 */
extern merr_t mxos_bt_smart_bond_reply(const mxos_bt_smart_bond_reply_t *response);
