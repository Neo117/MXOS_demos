...
} mxos_bt_smartbridge_socket_status_t;
 ...
 */
typedef struct mxos_bt_smartbridge_socket mxos_bt_smartbridge_socket_t;

/**
 * Auto connection callback parameters 
 */
typedef struct mxos_bt_smartbridge_auto_conn_cback_parm mxos_bt_smartbridge_auto_conn_cback_parms_t;
 ...
 */
struct mxos_bt_smartbridge_socket
{
    linked_list_node_t                             node;                           /**< Socket list node                                              */
    mxos_bt_smart_device_t                         remote_device;                  /**< Remote Bluetooth device MXOS is connected with (bridging)    */
    ...
    mxos_bt_smartbridge_auto_connection_callback_t auto_connection_callback;       /**< Callback for handing connection event by Auto Connection      */
    mxos_bt_smartbridge_disconnection_callback_t   disconnection_callback;         /**< Callback for handling disconnection event by remote device    */
    mxos_bt_smart_bonding_callback_t               bonding_callback;               /**< Callback for handling pairing/bonding successful event        */
    mxos_bt_smartbridge_notification_callback_t    notification_callback;          /**< Callback for handling GATT notification from remote device    */
    mxos_bt_smart_connection_settings_t            connection_settings;            /**< Connection settings                                           */
...
struct mxos_bt_smartbridge_auto_conn_cback_parm
{
    ...
    mxos_bt_smartbridge_auto_connection_callback_t auto_connection_callback;       /**< Callback for handing connection event by Auto connection */
    mxos_bt_smartbridge_disconnection_callback_t   auto_disconn_callback;          /**< Callback for handing disconnection event */
    mxos_bt_smartbridge_notification_callback_t    notification_callback;          /**< Callback for handing GATT notification from a remote device */
};
...
/** Initialise the MXOS SmartBridge
 *
 * @note
 ...
 * of packet exhange required between MXOS SmartBridge and the server.
 *
 ...
 * 
 * Specified services can be used in ATT cache generation, it makes MXOS SmartBridge 
 * only cache the services that application can support and operate.
 *
 * @param[in]  cache_count    : the number of caches that will be supported by MXOS SmartBridge
...
/** Check if the MXOS SmartBridge is currently scanning for Bluetooth Smart devices
 *
 * @note
 * This function checks if the Bluetooth Controller is currently scanning for Bluetooth
 * Smart devices
 *
 * @return @ref mxos_bool_t
 */
 ...
 *     are retrieved using @ref mxos_bt_smartbridge_get_scan_result_list once scan
 *     is complete
 ...
 * \li complete_callback runs on the context of MXOS_BT_EVT_WORKER_THREAD
 * \li If the whitelist filter is enabled in the scan settings, only devices
 *     in the whitelist appear in the scan results. Call @ref mxos_bt_smartbridge_add_device_to_whitelist()
...
merr_t mxos_bt_smartbridge_connect( mxos_bt_smartbridge_socket_t*                socket,
                                       ...
                                       mxos_bt_smartbridge_notification_callback_t  notification_callback );
 ...
 *
 * @warning
 * This function must be called before calling @ref mxos_bt_smartbridge_connect
 *
 ...
 *
 * @warning
 * \li This function must be called before calling @ref mxos_bt_smartbridge_connect
 * \li Callback functions run on the context of MXOS_BT_EVT_WORKER_THREAD
 *
