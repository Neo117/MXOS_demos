...
typedef struct
{
    mxos_bt_smartbridge_socket_t           *socket;                         /**< A socket associated with the auto connection */
    mxos_bt_smart_connection_settings_t     conn_settings;                  /**< The connection settings associated with a socket */
...
typedef struct 
{
    ...
    mxos_bt_smart_advertising_report_t  report;        /* Remote BT device */
...
static merr_t smartbridge_app_notification_handler             ( void* arg );
static merr_t smartbridge_app_disconnection_handler            ( void* arg );
...
static merr_t    smartbridge_gap_auto_conn_asyn_event_handler  ( void *arg );
static merr_t    smartbridge_gap_auto_conn_scan_report_handler ( const mxos_bt_smart_advertising_report_t *report );
static merr_t    smartbridge_gap_auto_conn_scan_cmpl_handler   ( void *arg );
static merr_t    smartbridge_gap_auto_conn_cryption            ( void* arg );

static merr_t    smartbridge_auto_conn_user_parms              ( void* arg );
...
static merr_t    smartbridge_auto_conn_list_add                ( const mxos_bt_smart_advertising_report_t *report );
static merr_t    smartbridge_auto_conn_list_remove             ( mxos_bt_device_address_t bdaddr );
static merr_t    smartbridge_auto_conn_list_clear              ( void );
static merr_t    smartbridge_auto_conn_list_remove_by_state    ( smartbridge_auto_conn_state_t state );
static merr_t    smartbridge_auto_conn_list_set_state          ( const mxos_bt_device_address_t bdaddr, smartbridge_auto_conn_state_t state );
static merr_t    smartbridge_auto_conn_list_get_by_state       ( mxos_bt_smart_advertising_report_t **report, smartbridge_auto_conn_state_t state );

/******************************************************
 *               Variable Definitions
 ******************************************************/

mxos_bt_smartbridge_socket_t*           connecting_socket = NULL;
...
{
    int8_t                                   idx = 0;
    mxos_bt_smartbridge_socket_t            *p_socket;
...
static void smartbridge_gatt_disconnection_handler( uint16_t connection_handle )
{
    ...
    mxos_bt_smartbridge_socket_t* removed_socket = NULL;

    /* Remove socket from the connected list */
    if ( bt_smartbridge_socket_manager_remove_socket( connection_handle, &removed_socket ) == MXOS_BT_SUCCESS )
    {
        ...
        bt_smartbridge_att_cache_set_active_state( (bt_smartbridge_att_cache_t*)removed_socket->att_cache, MXOS_FALSE );
        ...
        else
        {
            ...
            {
                mos_worker_send_async_event( MXOS_BT_EVT_WORKER_THREAD, smartbridge_app_disconnection_handler, (void*)removed_socket );
            }
...
static void smartbridge_gatt_read_operation_complete_handler( mxos_bt_gatt_data_t* response_data )
{
    mxos_bt_smart_attribute_t* attr;
    uint8_t *data = NULL;
    /* Create a new attribute */
    mxos_bt_smart_attribute_create( &attr, MXOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, response_data->len );
    ...
    if ( attr != NULL )
    {
        ...
        //memcpy( &attr->type, &subprocedure.uuid, sizeof(mxos_bt_uuid_t));
        ...
        smartbridge_subprocedure.result = MXOS_BT_SUCCESS;
    ...
    {
        smartbridge_subprocedure.result = MXOS_BT_OUT_OF_HEAP_SPACE;
    }

}

static merr_t smartbridge_gatt_notification_indication_handler( mxos_bt_gatt_operation_complete_t* operation_complete )
{
    ...
    if( operation_complete->op == GATTC_OPTYPE_INDICATION )
    {
        ...
        if ( mxos_bt_gatt_send_indication_confirm( connection_handle, attribute_handle ) != 0 ) 
        {
    ...
    if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == MXOS_BT_SUCCESS )
    {
        if ( bt_smartbridge_att_cache_is_enabled() == MXOS_TRUE && socket->att_cache != NULL )
        {
            ...
            if ( mxos_bt_smart_attribute_search_list_by_handle( att_cache_list, attribute_handle, &att ) == MXOS_BT_SUCCESS )
            {
                mxos_bt_uuid_t uuid       = att->type;
                mxos_bool_t    is_new_att = MXOS_FALSE;
                ...
                if ( length > att->value_length )
                {
                    ...
                    mxos_bt_smart_attribute_remove_from_list( att_cache_list, attribute_handle );
                    att = NULL;

                    /* Create a new one and marked as new */
                    mxos_bt_smart_attribute_create( &att, MXOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, length );
                    is_new_att = MXOS_TRUE;
                }
                ...
                if ( is_new_att == MXOS_TRUE )
                {
                    /* Add newly created att to the list */
                    mxos_bt_smart_attribute_add_to_list( att_cache_list, att );
                }
            }
        ...
        {
            if( kNoErr != mos_worker_send_async_event( MXOS_BT_EVT_WORKER_THREAD, smartbridge_app_notification_handler, (void*)socket ) )
            {
...
static void smartbridge_gatt_discover_characteristic_descriptor_result( mxos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    mxos_bt_smart_attribute_t* attr;

    mxos_bt_smart_attribute_create( &attr, MXOS_ATTRIBUTE_TYPE_NO_VALUE, 0 );
...
static void smartbridge_gatt_discover_services_result( mxos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    mxos_bt_smart_attribute_t* attr;

    mxos_bt_smart_attribute_create( &attr, MXOS_ATTRIBUTE_TYPE_PRIMARY_SERVICE, 0 );
    ...
    if ( attr != NULL )
    {
        ...
        memcpy( &attr->value.service.uuid, &p_event_data->discovery_result.discovery_data.group_value.service_type, sizeof(mxos_bt_uuid_t) );
...
static void smartbridge_gatt_discover_included_services_result( mxos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    mxos_bt_smart_attribute_t* attr;
    //uint16_t start_handle = 0x0;

    mxos_bt_smart_attribute_create( &attr, MXOS_ATTRIBUTE_TYPE_INCLUDE, 0 );
    ...
    if ( attr != NULL )
    {
        ...
        memcpy( &attr->value.include.uuid, &p_event_data->discovery_result.discovery_data.included_service.service_type, sizeof(mxos_bt_uuid_t) );
...
static void smartbridge_gatt_discover_characteristic_result(  mxos_bt_gatt_event_data_t *p_event_data )
{
    /* Create attribute(s) based on information included in the response PDU */
    mxos_bt_smart_attribute_t* attr;

    mxos_bt_smart_attribute_create( &attr, MXOS_ATTRIBUTE_TYPE_CHARACTERISTIC, 0 );
    ...
    if ( attr != NULL )
    {
        ...
        memcpy( &attr->value.characteristic.uuid, &current_characteristic.char_uuid, sizeof(mxos_bt_uuid_t) );
...
static void smartbridge_gatt_discovery_complete_handler( mxos_bt_gatt_event_data_t *p_event_data )
{
    ...
    switch( discovery_complete_type )
    {
        ...
        case GATT_DISCOVER_CHARACTERISTIC_DESCRIPTORS:
            ...
            {
                smartbridge_subprocedure.result = MXOS_BT_ITEM_NOT_IN_LIST;
            }
...
mxos_bt_gatt_status_t smartbridge_gatt_callback( mxos_bt_gatt_evt_t event, mxos_bt_gatt_event_data_t *p_event_data )
{
...
merr_t mxos_bt_smartbridge_init( uint8_t count )
{
    ...
    bt_smartbridge_log( "Initialising MXOS SmartBridge ..." );
...
mxos_bool_t mxos_bt_smartbridge_is_scanning( void )
{
    return ( initialised == MXOS_TRUE ) ? smartbridge_bt_interface_is_scanning( ) : MXOS_FALSE;
}

mxos_bool_t mxos_bt_smartbridge_is_ready_to_connect( void )
{
    return ( initialised == MXOS_FALSE || connecting_socket != NULL ) ? MXOS_FALSE : MXOS_TRUE;
}

merr_t mxos_bt_smartbridge_start_scan( const mxos_bt_smart_scan_settings_t* settings, mxos_bt_smart_scan_complete_callback_t complete_callback, mxos_bt_smart_advertising_report_callback_t advertising_report_callback )
{
...
merr_t mxos_bt_smartbridge_set_auto_connection_action( mxos_bool_t start_stop, const mxos_bt_smart_scan_settings_t *scan_settings, mxos_bt_smartbridge_auto_connection_parms_cback_t p_auto_conn_cback )
{
    ...
    {         
        ...
        memcpy(&g_auto_conn_scan_cfg, scan_settings, sizeof(mxos_bt_smart_scan_settings_t));
    ...
    else 
    {
        ...
        memset(&g_auto_conn_scan_cfg, 0, sizeof(mxos_bt_smart_scan_settings_t));
        err = mxos_bt_smartbridge_stop_scan();
    }
    return err;
}
...
merr_t mxos_bt_smartbridge_get_socket_status( mxos_bt_smartbridge_socket_t* socket, mxos_bt_smartbridge_socket_status_t* status )
{
    ...
    else if ( socket->state == SOCKET_STATE_LINK_CONNECTED )
    {
        ...
        if ( smartbridge_helper_socket_check_actions_disabled( socket, SOCKET_ACTION_ENCRYPT_USING_BOND_INFO | SOCKET_ACTION_INITIATE_PAIRING ) == MXOS_TRUE )
        {
            *status = SMARTBRIDGE_SOCKET_CONNECTED;
        }
        else
...
merr_t mxos_bt_smartbridge_connect( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_smart_device_t* remote_device, const mxos_bt_smart_connection_settings_t* settings, mxos_bt_smartbridge_disconnection_callback_t disconnection_callback, mxos_bt_smartbridge_notification_callback_t notification_callback )
{
    ...
    if ( connecting_socket != NULL )
    {
        ...
        return MXOS_BT_CONNECT_IN_PROGRESS;
    }

    if ( bt_smartbridge_socket_manager_is_full() == MXOS_TRUE )
    {
        return MXOS_BT_MAX_CONNECTIONS_REACHED;
    }

    if ( bt_smartbridge_socket_manager_find_socket_by_address( &remote_device->address, &found_socket ) == MXOS_BT_SUCCESS )
    {
        /* device is already connected */
        return MXOS_BT_SOCKET_IN_USE;
    }

    bt_smartbridge_log( "connect()...socket things are okay" );

    /* Clean-up accidentally set semaphores */
    while( mos_semphr_acquire(socket->semaphore, MOS_NO_WAIT ) == MXOS_BT_SUCCESS )
    {
    }
    ...
    else
    {
        ...
        mxos_bt_stop_pairing( socket->remote_device.address );
    }
    ...
    if ( socket->state == SOCKET_STATE_LINK_CONNECTED )
    {
        ...
        if ( smartbridge_helper_socket_check_actions_enabled( socket, SOCKET_ACTION_INITIATE_PAIRING ) == MXOS_TRUE ||
             smartbridge_helper_socket_check_actions_enabled( socket, SOCKET_ACTION_ENCRYPT_USING_BOND_INFO ) == MXOS_TRUE )
        {
            ...
            if ( socket->state != SOCKET_STATE_LINK_ENCRYPTED )
            {
                result = MXOS_BT_ENCRYPTION_FAILED;
                mxos_bt_dev_delete_bonded_device( (uint8_t *)remote_device->address );
                goto error;
            }
        }
    }
    ...
    {
        result = MXOS_BT_SOCKET_NOT_CONNECTED;
        goto error;
    }

    /* Successful */
    if ( bt_smartbridge_att_cache_is_enabled() == MXOS_TRUE )
    {
        ...
        bt_smartbridge_att_cache_set_active_state( cache, MXOS_TRUE );
...
merr_t mxos_bt_smartbridge_disconnect( mxos_bt_smartbridge_socket_t* socket, mxos_bool_t remove_it_from_whitelist )
{
    ...
    {
        mxos_bt_smartbridge_socket_t* removed_socket;
        ...
        /* Mark att cache as inactive and reset reference to cache */
        bt_smartbridge_att_cache_set_active_state( (bt_smartbridge_att_cache_t*)socket->att_cache, MXOS_FALSE );
...
merr_t mxos_bt_smartbridge_set_bond_info( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_smart_security_settings_t* settings, const mxos_bt_smart_bond_info_t* bond_info )
{
    if ( initialised == MXOS_FALSE )
    {
        return MXOS_BT_SMART_APPL_UNINITIALISED;
...
merr_t mxos_bt_smartbridge_enable_pairing( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_smart_security_settings_t* settings, mxos_bt_smart_bonding_callback_t bonding_callback )
{
    if ( initialised == MXOS_FALSE )
    {
        return MXOS_BT_SMART_APPL_UNINITIALISED;
...
merr_t mxos_bt_smartbridge_enable_attribute_cache_notification( mxos_bt_smartbridge_socket_t* socket, mxos_bool_t is_notification_or_indication )
{
    ...
    if ( bt_smartbridge_att_cache_is_enabled( ) == MXOS_FALSE )
    {
    ...
    if ( bt_smartbridge_att_cache_is_discovering( cache ) == MXOS_TRUE )
    {
        return MXOS_BT_DISCOVER_IN_PROGRESS;
    }
    ...
    result = mxos_bt_smart_attribute_get_list_head( list, &iterator );
    if ( result != MXOS_BT_SUCCESS )
    {
        return result;
    }
...
merr_t mxos_bt_smartbridge_get_attribute_cache_list( mxos_bt_smartbridge_socket_t* socket, mxos_bt_smart_attribute_list_t** att_cache_list )
{
    ...
    if ( bt_smartbridge_att_cache_is_discovering( (bt_smartbridge_att_cache_t*)socket->att_cache ) == MXOS_TRUE )
    {
        return MXOS_BT_DISCOVER_IN_PROGRESS;
    }
...
merr_t mxos_bt_smartbridge_get_attribute_cache_by_handle( mxos_bt_smartbridge_socket_t* socket, uint16_t handle, mxos_bt_smart_attribute_t* attribute, uint16_t size )
{
    ...
    result = mxos_bt_smart_attribute_search_list_by_handle( att_cache_list, handle, &att );

    if ( result == MXOS_BT_SUCCESS )
    {
        if ( att->value_struct_size + ATTR_COMMON_FIELDS_SIZE > size )
        {
            result = MXOS_BT_ATTRIBUTE_VALUE_TOO_LONG;
        }
        else
        {
...
merr_t mxos_bt_smartbridge_get_attribute_cache_by_uuid( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_uuid_t* uuid, uint16_t starting_handle, uint16_t ending_handle, mxos_bt_smart_attribute_t* attribute, uint32_t size )
{
    ...
    result = mxos_bt_smart_attribute_search_list_by_uuid( att_cache_list, uuid, starting_handle, ending_handle, &att );
    if ( result == MXOS_BT_SUCCESS )
    {
        if ( att->value_struct_size + ATTR_COMMON_FIELDS_SIZE > size )
        {
...
merr_t mxos_bt_smartbridge_get_service_from_attribute_cache_by_uuid( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_uuid_t* uuid, uint16_t starting_handle, uint16_t ending_handle, mxos_bt_smart_attribute_t* attribute, uint32_t size )
{
    ...
    /* Find service */
    while( ( result = mxos_bt_smartbridge_get_attribute_cache_by_uuid( socket, &service_uuid, _starting_handle, _ending_handle, attribute, size ) )== MXOS_BT_SUCCESS )
    {
...
merr_t mxos_bt_smartbridge_get_characteritics_from_attribute_cache_by_uuid( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_uuid_t* uuid, uint16_t starting_handle, uint16_t ending_handle, mxos_bt_smart_attribute_t* attribute, uint32_t size )
{
    ...
    /* Find service */
    while( ( result = mxos_bt_smartbridge_get_attribute_cache_by_uuid( socket, &characteristic_uuid, _starting_handle, _ending_handle, attribute, size ) )== MXOS_BT_SUCCESS )
    {
...
merr_t mxos_bt_smartbridge_refresh_attribute_cache_characteristic_value( mxos_bt_smartbridge_socket_t* socket, uint16_t handle )
{
    ...
    result = mxos_bt_smart_attribute_search_list_by_handle( att_cache_list, handle, &current_att );
    if ( result == MXOS_BT_SUCCESS )
    {
        ...
        if ( result == MXOS_BT_SUCCESS )
        {
            ...
            result = mxos_bt_smart_attribute_remove_from_list( att_cache_list, current_att->handle );
            if ( result == MXOS_BT_SUCCESS )
            {
                result = mxos_bt_smart_attribute_add_to_list( att_cache_list, refreshed_att );
            }
        }
    }
...
merr_t mxos_bt_smartbridge_write_attribute_cache_characteristic_value( mxos_bt_smartbridge_socket_t* socket, const mxos_bt_smart_attribute_t* char_value )
{
    bt_smartbridge_att_cache_t*      cache          = NULL;
    mxos_bt_smart_attribute_list_t* att_cache_list = NULL;
    mxos_bt_smart_attribute_t*      att            = NULL;
    ...
    {
        result = smartbridge_bt_interface_write_characteristic_value( socket->connection_handle, (mxos_bt_smart_attribute_t*)char_value );
    }
    else
    {
        result = smartbridge_bt_interface_write_long_characteristic_value( socket->connection_handle, (mxos_bt_smart_attribute_t*)char_value );
    }
    ...
    result = mxos_bt_smart_attribute_search_list_by_handle( att_cache_list, char_value->handle, &att );
    ...
    if ( result == MXOS_BT_SUCCESS )
    {
        ...
        {
            result = mxos_bt_smart_attribute_remove_from_list( att_cache_list, att->handle );
            ...
            result = mxos_bt_smart_attribute_create( &att, MXOS_ATTRIBUTE_TYPE_CHARACTERISTIC_VALUE, char_value->value_length );
            ...
            result = mxos_bt_smart_attribute_add_to_list( att_cache_list, att );
        }
        else
        {
            memcpy( att->value.value, char_value->value.value, char_value->value_length );
    ...
    else if ( result == MXOS_BT_ITEM_NOT_IN_LIST )
    {
...
// merr_t mxos_bt_smartbridge_bond_info_update( mxos_bt_device_link_keys_t paired_device_keys )
// {
//     mxos_bt_smart_bond_info_t          bond_info;
//     mxos_bt_ble_keys_t                  *le_security_keys;
//     mxos_bt_device_sec_keys_t           *security_keys;

//     merr_t result = MXOS_BT_ERROR;

//     memcpy(bond_info.peer_address, paired_device_keys.bd_addr, BD_ADDR_LEN );

//     security_keys       = (mxos_bt_device_sec_keys_t*)&paired_device_keys.key_data;
...
//     le_security_keys    = ( mxos_bt_ble_keys_t* )&security_keys->le_keys;
...
//     bond_info.address_type  = (mxos_bt_smart_address_type_t)security_keys->ble_addr_type;
...
// static merr_t smartbridge_gap_bonding_handler( uint16_t connection_handle, const mxos_bt_smart_bond_info_t* bond_info )
// {
//     mxos_bt_smartbridge_socket_t* socket;

//     if ( bt_smartbridge_socket_manager_find_socket_by_handle( connection_handle, &socket ) == MXOS_BT_SUCCESS )
//     {
//         /* Bonding successful. Update socket's bond info and post callback to MXOS_BT_EVT_WORKER_THREAD */
//         memcpy( &socket->bond_info, bond_info, sizeof( *bond_info ) );

//         if ( socket != NULL && socket->bonding_callback != NULL )
//         {
//             mos_worker_send_async_event( MXOS_BT_EVT_WORKER_THREAD, smartbridge_app_pairing_handler, (void*)socket );
//         }
...
// static merr_t smartbridge_app_pairing_handler( void* arg )
// {
//     mxos_bt_smartbridge_socket_t* socket = (mxos_bt_smartbridge_socket_t*)arg;
...
static merr_t smartbridge_gap_auto_conn_asyn_event_handler(void *arg)
{
    ...
    if (err == kNoErr) 
    {
        ...
        {            
            ...
            /* Clear semaphore */
            while (mos_semphr_acquire(g_auto_conn_sem, MOS_NO_WAIT) == kNoErr);
            ...
            gatt_status = mxos_bt_gatt_le_connect((uint8_t *)report->remote_device.address,
...
err_exit:
            ...
            /* Next Connection */
            mos_worker_send_async_event(MXOS_BT_WORKER_THREAD, 
...
static merr_t smartbridge_gap_auto_conn_scan_cmpl_handler(void *arg)
{
    ...
    if (g_auto_conn_scan_duration > 0)
    {
        ...
        if (err == MXOS_BT_PENDING && g_auto_conn_scan_duration < (uint16_t)(-1))
        {
...
static merr_t smartbridge_gap_auto_conn_cryption( void *arg )
{
    ...
    if (socket != NULL && socket->auto_connection_callback != NULL)
    {
        if (socket->security_settings.authentication_requirements != BT_SMART_AUTH_REQ_NONE)
        {
            ...
            mxos_bt_start_encryption( &socket->remote_device.address );
            mos_semphr_acquire(g_auto_conn_sem, MOS_NEVER_TIMEOUT);
...
static mxos_bool_t smartbridge_gap_auto_conn_user_cfg( const mxos_bt_device_address_t device_address, const uint8_t *device_name, const uint8_t *p_data, uint8_t length )
{
    ...
    mxos_bt_cfg_settings.ble_scan_cfg.conn_min_interval        = g_auto_conn_dev_table[idx].conn_settings.interval_min;
    mxos_bt_cfg_settings.ble_scan_cfg.conn_max_interval        = g_auto_conn_dev_table[idx].conn_settings.interval_max;
    mxos_bt_cfg_settings.ble_scan_cfg.conn_latency             = g_auto_conn_dev_table[idx].conn_settings.latency;
    mxos_bt_cfg_settings.ble_scan_cfg.conn_supervision_timeout = g_auto_conn_dev_table[idx].conn_settings.supervision_timeout;
    return MXOS_TRUE;
exit:
    return MXOS_FALSE;
}
...
static merr_t smartbridge_auto_conn_user_parms( void *arg )
{
        ...
        ) != kNoErr )
    {
        ...
        memset( (void *) &p_entity->conn_settings, 0, sizeof(mxos_bt_smart_connection_settings_t) );
...
{
    int8_t idx = 0;
    mxos_bt_smartbridge_socket_t *temp_socket;
    ...
    for (idx = 0; idx < sizeof(g_auto_conn_dev_table)/sizeof(g_auto_conn_dev_table[0]); idx++)
    {
        ...
        if ((temp_socket != (void *)0)
            && (memcmp((void *)temp_socket->remote_device.address,
                       (void *)device_address,
                       sizeof(mxos_bt_device_address_t)) == 0))
        {
            return idx;
        }
    }
...
void smartbridge_auto_connection_encryption_check(const mxos_bt_dev_pairing_cplt_t *p_pairing_cplt)
{
...
static merr_t smartbridge_auto_conn_list_deinit(void)
{
    mos_mutex_delete(g_auto_conn_list.mutex);
    return kNoErr;
}
...
static mxos_bool_t smartbridge_auto_conn_compare_state(linked_list_node_t *node_to_compare, void *user_data)
{
    smartbridge_auto_conn_report_t *report = (smartbridge_auto_conn_report_t *)node_to_compare;
