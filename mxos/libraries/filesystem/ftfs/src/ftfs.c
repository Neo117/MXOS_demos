...
static FT_FILE *_ft_fopen( struct fs *fs, const char *path, const char *mode )
{
    ...
    do
    {
        mhal_flash_read( fs->partition, &addr, (uint8_t *) &entry, sizeof(entry) );
...
size_t ft_fread( void *ptr, size_t size, size_t nmemb, file *f )
{
    ...
    for ( i = 0; i < nmemb; i++ )
    {
        ...
        mhal_flash_read( stream->sb->fs.partition, &addr, (uint8_t *) (b + len), size );
        len += size;
        stream->fp += size;
        if ( stream->fp >= stream->length )
            break;
...
struct fs *ftfs_init( struct ftfs_super *sb, int partition )
{
    FT_HEADER sec;
...
bool ft_is_content_valid( int partition, int be_ver )
{
    FT_HEADER sec1;
    bool ret = false;
