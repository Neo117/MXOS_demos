...
/* I2C device */
mhal_i2c_device_t ssd1106_i2c_device = {
...
merr_t ssd1106_i2c_bus_write(uint8_t reg_addr, uint8_t *reg_data, uint8_t cnt)
{
  ...
  mhal_i2c_message_t ssd1106_i2c_msg = {NULL, NULL, 0, 0, 10, false};
  ...
  err = mhal_i2c_build_tx_msg(&ssd1106_i2c_msg, array, cnt + 1, 3);
  require_noerr( err, exit );
  err = mhal_i2c_send_and_recv(&ssd1106_i2c_device, &ssd1106_i2c_msg, 1);
  require_noerr( err, exit );
...
const mhal_spi_dev_t mxoskit_spi_oled =
{
    .port        = OLED_SPI_PORT,
...
{  
  ...
  mhal_spi_send_and_recv( &mxoskit_spi_oled, &oled_spi_msg, 1 );
  ...
  if (mxoskit_spi_oled.port >= MXOS_SPI_NONE )
    return;

  config.chip_select = &platform_gpio_pins[mxoskit_spi_oled.chip_select];
  config.speed       = mxoskit_spi_oled.speed;
  config.mode        = mxoskit_spi_oled.mode;
  ...
  platform_spi_transfer( &platform_spi_drivers[mxoskit_spi_oled.port], &config, &oled_spi_msg, 1 );
...
{ 	 
#ifdef SSD1106_USE_I2C
  if( kNoErr != mhal_i2c_open( &ssd1106_i2c_device ) )
  {
      oled_log( "OLED_ERROR: mhal_i2c_open err." );
      return;
  }
  if( false == mhal_i2c_probe_dev(&ssd1106_i2c_device, 5) ){
      oled_log("OLED_ERROR: no i2c device found!");
      return;
  }
  _oled_avalid = 1;
#else
  mhal_spi_open( &mxoskit_spi_oled );
  OLED_DC_INIT();   
  OLED_DC_Clr();
#endif
