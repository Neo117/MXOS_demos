...
/* I2C device */
mhal_i2c_device_t lsm9ds1_mag_i2c_device = {
...
{
  // I2C init
  mhal_i2c_close(&lsm9ds1_mag_i2c_device);   // in case error
  mhal_i2c_open(&lsm9ds1_mag_i2c_device);
  
  if( false == mhal_i2c_probe_dev(&lsm9ds1_mag_i2c_device, 5) ){
...
{
  mhal_i2c_message_t lsm9ds1_mag_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  ...
  iError = mhal_i2c_build_tx_msg(&lsm9ds1_mag_i2c_msg, array, NumByteToWrite + 1, 3);
  iError = mhal_i2c_send_and_recv(&lsm9ds1_mag_i2c_device, &lsm9ds1_mag_i2c_msg, 1);
  if(kNoErr != iError){
    iError = kWriteErr;
  }
...
static merr_t LSM9DS1_MAG_IO_Read(uint8_t* pBuffer, uint8_t RegisterAddr, uint16_t NumByteToRead)
{
  ...
  iError = mhal_i2c_build_comb_msg(&lsm9ds1_mag_i2c_msg, array, pBuffer, 1, NumByteToRead, 3);
  if(kNoErr != iError){
    return kReadErr; 
  }
  iError = mhal_i2c_send_and_recv(&lsm9ds1_mag_i2c_device, &lsm9ds1_mag_i2c_msg, 1);
...
merr_t lsm9ds1_mag_sensor_deinit(void)
{
  ...
  return mhal_i2c_close(&lsm9ds1_mag_i2c_device);
}
