...
/* I2C device */
mhal_i2c_device_t lsm9ds1_acc_gyr_i2c_device = {
  LSM9DS1_I2C_PORT, 0x6A, I2C_ADDRESS_WIDTH_7BIT, I2C_STANDARD_SPEED_MODE
};

static merr_t LSM9DS1_ACC_GYR_IO_Init(void)
{
  // I2C init
  mhal_i2c_close(&lsm9ds1_acc_gyr_i2c_device);   // in case error
  mhal_i2c_open(&lsm9ds1_acc_gyr_i2c_device);
  
  if( false == mhal_i2c_probe_dev(&lsm9ds1_acc_gyr_i2c_device, 5) ){
...
*/
static merr_t LSM9DS1_ACC_GYR_IO_Write(uint8_t* pBuffer, uint8_t RegisterAddr, uint16_t NumByteToWrite)
{
  mhal_i2c_message_t lsm9ds1_acc_gyr_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  ...
  iError = mhal_i2c_build_tx_msg(&lsm9ds1_acc_gyr_i2c_msg, array, NumByteToWrite + 1, 3);
  iError = mhal_i2c_send_and_recv(&lsm9ds1_acc_gyr_i2c_device, &lsm9ds1_acc_gyr_i2c_msg, 1);
  if(kNoErr != iError){
    iError = kWriteErr;
  }
...
*/
static merr_t LSM9DS1_ACC_GYR_IO_Read(uint8_t* pBuffer, uint8_t RegisterAddr, uint16_t NumByteToRead)
{
  mhal_i2c_message_t lsm9ds1_acc_gyr_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  merr_t iError = kNoErr;
  uint8_t array[8] = {0};
  ...
  iError = mhal_i2c_build_comb_msg(&lsm9ds1_acc_gyr_i2c_msg, array, pBuffer, 1, NumByteToRead, 3);
  if(kNoErr != iError){
    return kReadErr; 
  }
  iError = mhal_i2c_send_and_recv(&lsm9ds1_acc_gyr_i2c_device, &lsm9ds1_acc_gyr_i2c_msg, 1);
...
static merr_t LSM9DS1_ACC_POWER_ON(void)
{
  merr_t err = kNoErr;
  uint8_t temp = 0;
...
static merr_t LSM9DS1_GYR_POWER_ON(void)
{
  merr_t err = kNoErr;
  uint8_t temp = 0;
...
static merr_t LSM9DS1_GYR_POWER_OFF(void)
{
  merr_t err = kNoErr;
  uint8_t temp = 0;
...
static merr_t LSM9DS1_ACC_POWER_OFF(void)
{
  merr_t err = kNoErr;
  uint8_t temp = 0;
...
static merr_t LSM9DS1_ACC_GYR_Init(void)
{
  merr_t err = kNoErr;
...
static merr_t LSM9DS1_ACC_GET_XYZ(int16_t *ACC_X, int16_t *ACC_Y, int16_t *ACC_Z)
{
  merr_t err = kNoErr;
  uint8_t temp[6] = {0};
...
static merr_t LSM9DS1_GYR_GET_XYZ(int16_t *GYR_X, int16_t *GYR_Y, int16_t *GYR_Z)
{
  merr_t err = kNoErr;
  uint8_t temp[6] = {0};
...
merr_t lsm9ds1_acc_gyr_sensor_deinit(void)
{
  ...
  return mhal_i2c_close(&lsm9ds1_acc_gyr_i2c_device);
}
