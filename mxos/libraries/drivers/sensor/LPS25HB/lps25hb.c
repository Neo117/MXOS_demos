...
/* I2C device */
mhal_i2c_device_t lps25hb_i2c_device = {
...
{
  // I2C init
  mhal_i2c_close(&lps25hb_i2c_device);   // in case error
  mhal_i2c_open(&lps25hb_i2c_device);

  if( false == mhal_i2c_probe_dev(&lps25hb_i2c_device, 5) ){
...
{
  mhal_i2c_message_t lps25hb_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  int iError = 0;
  uint8_t array[8];
  uint8_t stringpos;
  ...
  iError = mhal_i2c_build_tx_msg(&lps25hb_i2c_msg, array, NumByteToWrite + 1, 3);
  iError = mhal_i2c_send_and_recv(&lps25hb_i2c_device, &lps25hb_i2c_msg, 1);
  if(0 != iError){
...
PRESSURE_StatusTypeDef LPS25HB_IO_Read(uint8_t* pBuffer, uint8_t DeviceAddr, uint8_t RegisterAddr, uint16_t NumByteToRead)
{
  ...
  iError = mhal_i2c_build_comb_msg(&lps25hb_i2c_msg, array, pBuffer, 1, NumByteToRead, 3);
  if(0 != iError){
    return PRESSURE_ERROR; 
  }
  iError = mhal_i2c_send_and_recv(&lps25hb_i2c_device, &lps25hb_i2c_msg, 1);
...
merr_t lps25hb_sensor_deinit(void)
{
  ...
  if(mhal_i2c_close(&lps25hb_i2c_device) != PRESSURE_OK){
    return -1;
  }
  return 0;
}
