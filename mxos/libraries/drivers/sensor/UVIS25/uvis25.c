...
/* I2C device */
mhal_i2c_device_t uvis25_i2c_device = {
...
{
  // I2C init
  mhal_i2c_close(&uvis25_i2c_device);   // in case error
  mhal_i2c_open(&uvis25_i2c_device);

  if( false == mhal_i2c_probe_dev(&uvis25_i2c_device, 5) ){
...
{
  mhal_i2c_message_t uvis25_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  ...
  iError = mhal_i2c_build_tx_msg(&uvis25_i2c_msg, array, NumByteToWrite + 1, 3);
  iError = mhal_i2c_send_and_recv(&uvis25_i2c_device, &uvis25_i2c_msg, 1);
  if(kNoErr != iError){
    iError = kWriteErr;
  }
...
merr_t UVIS25_IO_Read(uint8_t* pBuffer, uint8_t RegisterAddr, uint16_t NumByteToRead)
{
  ...
  iError = mhal_i2c_build_comb_msg(&uvis25_i2c_msg, array, pBuffer, 1, NumByteToRead, 3);
  if(kNoErr != iError){
    return kReadErr; 
  }
  iError = mhal_i2c_send_and_recv(&uvis25_i2c_device, &uvis25_i2c_msg, 1);
...
{
  return mhal_i2c_close(&uvis25_i2c_device);
}
