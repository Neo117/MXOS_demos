...
/* I2C device */
mhal_i2c_device_t bmm050_i2c_device = {
...
s8 BMM050_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
        mhal_i2c_message_t bmm050_i2c_msg = {NULL, NULL, 0, 0, 0, false};
	...
	*/
        ...
        iError = mhal_i2c_build_tx_msg(&bmm050_i2c_msg, array, cnt + 1, 3);
        iError = mhal_i2c_send_and_recv(&bmm050_i2c_device, &bmm050_i2c_msg, 1);
        if(0 != iError){
          iError = -1;
        }
...
//	u8 stringpos = BMM050_INIT_VALUE;
	 ...
	 */
        ...
        iError = mhal_i2c_build_comb_msg(&bmm050_i2c_msg, array, reg_data, 1, cnt, 3);
         if(0 != iError){
          return (s8)iError; 
        }
        iError = mhal_i2c_send_and_recv(&bmm050_i2c_device, &bmm050_i2c_msg, 1);
...
merr_t bmm050_sensor_init(void)
{
 ...
 // u8 v_stand_by_time_u8 = BME280_INIT_VALUE;  //  The variable used to assign the standby time
  ...
  // I2C init
  mhal_i2c_close(&bmm050_i2c_device);   // in case error
  err = mhal_i2c_open(&bmm050_i2c_device);
  require_noerr_action( err, exit, bmm050_user_log("BMM050_ERROR: mhal_i2c_open err = %d.", err) );
  if( false == mhal_i2c_probe_dev(&bmm050_i2c_device, 5) ){
...
merr_t bmm050_data_readout(s16 *v_mag_datax_s16, s16 *v_mag_datay_s16, s16 *v_mag_dataz_s16)
{
  ...
  mhal_i2c_open(&bmm050_i2c_device);
  //------------------------------------------------------------------
...
merr_t bmm050_sensor_deinit(void)
{
  ...
  err = mhal_i2c_close(&bmm050_i2c_device);
  require_noerr_action( err, exit, bmm050_user_log("BMM050_ERROR: mhal_i2c_close err = %d.", err));
