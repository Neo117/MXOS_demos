...
/* I2C device */
mhal_i2c_device_t hts221_i2c_device = {
...
{
  // I2C init
  mhal_i2c_close(&hts221_i2c_device);   // in case error
  mhal_i2c_open(&hts221_i2c_device);
  if( false == mhal_i2c_probe_dev(&hts221_i2c_device, 5) ){
...
{
  mhal_i2c_message_t hts221_i2c_msg = {NULL, NULL, 0, 0, 0, false};
  int iError = 0;
  uint8_t array[8];
  uint8_t stringpos;
  ...
  iError = mhal_i2c_build_tx_msg(&hts221_i2c_msg, array, NumByteToWrite + 1, 3);
  iError = mhal_i2c_send_and_recv(&hts221_i2c_device, &hts221_i2c_msg, 1);
  if(0 != iError){
...
HUM_TEMP_StatusTypeDef HTS221_IO_Read(uint8_t* pBuffer, uint8_t DeviceAddr, uint8_t RegisterAddr, uint16_t NumByteToRead)
{
  ...
  iError = mhal_i2c_build_comb_msg(&hts221_i2c_msg, array, pBuffer, 1, NumByteToRead, 3);
  if(0 != iError){
    return HUM_TEMP_ERROR; 
  }
  iError = mhal_i2c_send_and_recv(&hts221_i2c_device, &hts221_i2c_msg, 1);
...
merr_t hts221_sensor_deinit(void)
{
  ...
  if(mhal_i2c_close(&hts221_i2c_device) != HUM_TEMP_OK){
    return -1;
  }
  return 0;
}
