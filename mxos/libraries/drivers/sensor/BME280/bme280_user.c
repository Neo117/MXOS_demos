...
/* I2C device */
mhal_i2c_device_t user_i2c_device = {
...
s8 BME280_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
        mhal_i2c_message_t user_i2c_msg = {NULL, NULL, 0, 0, 0, false};
	...
	*/
        iError = mhal_i2c_build_tx_msg(&user_i2c_msg, array, cnt + 1, 3);
        iError = mhal_i2c_send_and_recv(&user_i2c_device, &user_i2c_msg, 1);
        if(0 != iError){
          iError = -1;
        }
	return (s8)iError;
...
s8 BME280_I2C_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
     ...
     * and FAILURE defined as -1
	 */
        //iError = mhal_i2c_build_rx_msg(&user_i2c_msg, array, cnt + 1, 3);
        iError = mhal_i2c_build_comb_msg(&user_i2c_msg, array, reg_data, 1, cnt, 3);
         if(0 != iError){
          return (s8)iError; 
        }
        iError = mhal_i2c_send_and_recv(&user_i2c_device, &user_i2c_msg, 1);
...
merr_t bme280_sensor_init(void)
{
 ...
 // u8 v_stand_by_time_u8 = BME280_INIT_VALUE;  //  The variable used to assign the standby time
  ...
  // I2C init
  mhal_i2c_close(&user_i2c_device);   // in case error
  err = mhal_i2c_open(&user_i2c_device);
  require_noerr_action( err, exit, bme280_user_log("BME280_ERROR: mhal_i2c_open err = %d.", err) );
  if( false == mhal_i2c_probe_dev(&user_i2c_device, 5) ){
...
merr_t bme280_sensor_deinit(void)
{
  ...
  err = mhal_i2c_close(&user_i2c_device);
  require_noerr_action( err, exit, bme280_user_log("BME280_ERROR: mhal_i2c_close err = %d.", err));
