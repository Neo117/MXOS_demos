...
static volatile temp_hum_sensor_type_t temp_hum_sensor_type = MXOSKIT_TEMP_HUM_SENSOR_NONE;
...
merr_t temp_hum_sensor_init(void)
{
  ...
  if(kNoErr != err){  // bme280 not work, use DHT11
    ...
    if(0 != ret){  // init error
      ...
      temp_hum_sensor_type = MXOSKIT_TEMP_HUM_SENSOR_DHT11;
      err = kNoErr;
    }
  }
  else{  // use bme280
    temp_hum_sensor_type = MXOSKIT_TEMP_HUM_SENSOR_BME280;
  }
...
merr_t temp_hum_sensor_read(int32_t *temperature,  uint32_t *humidity)
{
  ...
  case MXOSKIT_TEMP_HUM_SENSOR_BME280:
    {
  ...
  case MXOSKIT_TEMP_HUM_SENSOR_DHT11:
    {
