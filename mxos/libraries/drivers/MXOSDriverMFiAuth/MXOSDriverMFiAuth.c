...
static mhal_i2c_device_t MFi_CP =
{
    .port = MXKIT_I2C,
...
merr_t mhal_mfi_auth_open( int i2c )
{
    ...
    err = mhal_i2c_open( (mhal_i2c_device_t *)&MFi_CP );
    require_noerr_action(err, exit, err = kRequestErr);

    isCPFound = mhal_i2c_probe_dev( (mhal_i2c_device_t *)&MFi_CP, 3 );
    if(!isCPFound){
...
{
  merr_t err = kNoErr;
  mhal_i2c_message_t message;
  uint8_t *txData = NULL;
  txData = malloc(NumByteToWrite+1);
  memcpy(txData, &WriteAddr, 1);
  memcpy(txData+1, pBuffer, NumByteToWrite);
  
  err = mhal_i2c_build_tx_msg(&message, txData, NumByteToWrite+1, 100);
  require_noerr( err , exit ) ;
  err = mhal_i2c_send_and_recv( (mhal_i2c_device_t *)&MFi_CP, &message, 1 );
  require_noerr( err , exit ) ;
...
merr_t CP_ReadBuffer(uint8_t* pBuffer, uint8_t ReadAddr, uint16_t NumByteToRead)
{
  ...
  err = mhal_i2c_build_comb_msg(&message, &ReadAddr, pBuffer, 1, NumByteToRead, 100);
  require_noerr( err , exit ) ;
  err = mhal_i2c_send_and_recv( (mhal_i2c_device_t *)&MFi_CP, &message, 1 );
  require_noerr( err , exit ) ;
