...
UDRV_API void UPIO_Set( tUPIO_TYPE type, tUPIO pio, tUPIO_STATE state )
{
	...
	if(UPIO_OFF == state)
	{
		platform_gpio_output_low( mxos_bt_control_pins[MXOS_BT_PIN_DEVICE_WAKE] );
	}else
	{
		platform_gpio_output_high( mxos_bt_control_pins[MXOS_BT_PIN_DEVICE_WAKE] );
	}
#endif
}
...
UDRV_API tUPIO_STATE UPIO_Read( tUPIO_TYPE type, tUPIO pio )
{
	...
	return platform_gpio_input_get(mxos_bt_control_pins[MXOS_BT_PIN_HOST_WAKE]) ? UPIO_ON : UPIO_OFF;
#else
    return UPIO_OFF;
#endif
}
