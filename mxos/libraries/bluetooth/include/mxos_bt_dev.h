...
/** Result/Status for mxos_bt_dev */
typedef mxos_bt_result_t  mxos_bt_dev_status_t;      /**< Result/Status for mxos_bt_dev */
...
} mxos_bt_dev_vendor_specific_command_complete_params_t;
...
enum mxos_bt_discoverability_mode_e {
...
enum mxos_bt_connectability_mode_e {
 ...
 */
#ifndef BTM_INQUIRY_MODE            /* To avoid redefintions when including mxos_bt_dev.h */
#define BTM_INQUIRY_MODE
enum mxos_bt_inquiry_mode_e {
...
/* Inquiry Filter Condition types (see mxos_bt_dev_inq_parms_t) */
#ifndef BTM_INQUIRY_FILTER
#define BTM_INQUIRY_FILTER
enum mxos_bt_dev_filter_cond_e {
 ...
 * Used in the mxos_bt_dev_inq_info_t structure, and returned in the
...
typedef struct
{
    mxos_bt_dev_class_t            dev_class;          /**< class of device */
    mxos_bt_dev_class_t            dev_class_mask;     /**< class of device filter mask */
} mxos_bt_dev_cod_cond_t;

/** Inquiry filter */
typedef union
{
    mxos_bt_device_address_t       bdaddr_cond;        /**< bluetooth address filter */
    mxos_bt_dev_cod_cond_t         cod_cond;           /**< class of device filter */
} mxos_bt_dev_inq_filt_cond_t;

/** Inquiry Parameters */
typedef struct
{
    uint8_t                         mode;               /**< Inquiry mode (see #mxos_bt_inquiry_mode_e) */
    uint8_t                         duration;           /**< Inquiry duration (1.28 sec increments) */
    uint8_t                         filter_cond_type;   /**< Inquiry filter type  (see #mxos_bt_dev_filter_cond_e) */
    mxos_bt_dev_inq_filt_cond_t    filter_cond;        /**< Inquiry filter */
} mxos_bt_dev_inq_parms_t;

/** Inquiry Results */
typedef struct
{
    uint16_t                        clock_offset;                           /**< Clock offset */
    mxos_bt_device_address_t       remote_bd_addr;                         /**< Device address */
    mxos_bt_dev_class_t            dev_class;                              /**< Class of device */
    ...
    mxos_bool_t                    eir_complete_list;                      /**< TRUE if EIR array is complete */
} mxos_bt_dev_inquiry_scan_result_t;

/** RSSI Result (in response to #mxos_bt_dev_read_rssi) */
typedef struct
{
    mxos_bt_result_t                  status;             /**< Status of the operation */
    uint8_t                         hci_status;         /**< Status from controller */
    int8_t                          rssi;               /**< RSSI */
    mxos_bt_device_address_t       rem_bda;            /**< Remote BD address */
} mxos_bt_dev_rssi_result_t;

/** TX Power Result (in response to #mxos_bt_dev_read_tx_power) */
typedef struct
{
    mxos_bt_result_t                  status;             /**< Status of the operation */
    uint8_t                         hci_status;         /**< Status from controller */
...
#define BTM_SECURITY_KEY_DATA_LEN       132     /**< Security key data length (used by mxos_bt_device_link_keys_t structure) */
#endif

#ifndef BTM_SECURITY_LOCAL_KEY_DATA_LEN
#define BTM_SECURITY_LOCAL_KEY_DATA_LEN 65      /**< Local security key data length (used by mxos_bt_local_identity_keys_t structure) */
#endif

/** Pairing IO Capabilities */
enum mxos_bt_dev_io_cap_e
{
...
typedef uint8_t mxos_bt_dev_io_cap_t;          /**< IO capabilities (see #mxos_bt_dev_io_cap_e) */

/** BR/EDR Authentication requirement */
enum mxos_bt_dev_auth_req_e {
...
typedef uint8_t mxos_bt_dev_auth_req_t;                /**< BR/EDR authentication requirement (see #mxos_bt_dev_auth_req_e) */

/** Device Security Mode */
enum mxos_bt_security_mode_e {
...
typedef uint8_t mxos_bt_security_mode_t;

/** LE Authentication requirement */
enum mxos_bt_dev_le_auth_req_e {
...
typedef uint8_t mxos_bt_dev_le_auth_req_t;             /**< BLE authentication requirement (see #mxos_bt_dev_le_auth_req_e) */

/** OOB Data status */
#ifndef BTM_OOB_STATE
#define BTM_OOB_STATE
enum mxos_bt_dev_oob_data_e
{
...
#endif
typedef uint8_t mxos_bt_dev_oob_data_t;                /**< OOB data (see #mxos_bt_dev_oob_data_e) */

/** Data type for IO capabalities response (BTM_PAIRING_IO_CAPABILITIES_BR_EDR_RESPONSE_EVT) */
typedef struct
{
    mxos_bt_device_address_t   bd_addr;                /**< Peer address */
    mxos_bt_dev_io_cap_t       io_cap;                 /**< Peer IO capabilities */
    mxos_bt_dev_oob_data_t     oob_data;               /**< OOB data present at peer device for the local device */
    mxos_bt_dev_auth_req_t     auth_req;               /**< Authentication required for peer device */
} mxos_bt_dev_bredr_io_caps_rsp_t;
...
typedef struct
{
    ...
    mxos_bool_t                just_works;                         /**< TRUE, if using "just works" association model */
    mxos_bt_dev_auth_req_t     local_authentication_requirements;  /**< Authentication requirement for local device */
    mxos_bt_dev_auth_req_t     remote_authentication_requirements; /**< Authentication requirement for peer device */
} mxos_bt_dev_user_cfm_req_t;

/** Pairing user passkey request  (BTM_USER_PASSKEY_REQUEST_EVT event data type) */
typedef struct
{
    mxos_bt_device_address_t   bd_addr;            /**< peer address       */
} mxos_bt_dev_user_key_req_t;
...
} mxos_bt_dev_user_key_notif_t;


/** Pairing keypress types */
enum mxos_bt_dev_passkey_entry_type_e
{
...
typedef uint8_t mxos_bt_dev_passkey_entry_type_t;  /**< Bluetooth pairing keypress value (see #mxos_bt_dev_passkey_entry_type_e)  */

/** Pairing keypress notification (BTM_USER_KEYPRESS_NOTIFICATION_EVT event data type) */
typedef struct
{
    mxos_bt_device_address_t           bd_addr;        /**< peer address       */
    mxos_bt_dev_passkey_entry_type_t   keypress_type;  /**< type of keypress   */
} mxos_bt_dev_user_keypress_t;
...
} mxos_bt_dev_br_edr_pairing_info_t;

/** BLE pairing complete infomation */
typedef struct
{
    mxos_bt_result_t                 status;                 /**< status of the simple pairing process   */
    uint8_t                          reason;                 /**< failure reason (see #mxos_bt_smp_status_t) */
    uint8_t                          sec_level;              /**< 0 - None, 1- Unauthenticated Key, 4-Authenticated Key  */
    mxos_bool_t                      privacy_supported;      /**< True if privacy supported, False if not    */
    mxos_bool_t                      is_pair_cancel;         /**< True if cancelled, else False   */
    mxos_bt_device_address_t         resolved_bd_addr;       /**< Resolved address (if remote device using private address) */
    mxos_bt_ble_address_type_t       resolved_bd_addr_type;  /**< Resolved addr type of bonded device */
} mxos_bt_dev_ble_pairing_info_t;

/** Transport dependent pairing complete infomation */
typedef union
{
    mxos_bt_dev_br_edr_pairing_info_t  br_edr;         /**< BR/EDR pairing complete infomation */
    mxos_bt_dev_ble_pairing_info_t     ble;            /**< BLE pairing complete infomation */
} mxos_bt_dev_pairing_info_t;

/** Pairing complete notification (BTM_PAIRING_COMPLETE_EVT event data type) */
typedef struct
{
    uint8_t                     *bd_addr;               /**< peer address           */
    mxos_bt_transport_t         transport;              /**< BT_TRANSPORT_BR_EDR or BT_TRANSPORT_LE */
    mxos_bt_dev_pairing_info_t  pairing_complete_info;  /**< Transport dependent pairing complete infomation */
    mxos_bt_result_t            bonding_status;         /**< current status of bonding process to notify app of completion status of storing keys */
} mxos_bt_dev_pairing_cplt_t;

/** Check if application wishes to upgrade security (BTM_SECURITY_UPGRADE_EVT event data type) */
typedef struct
{
    mxos_bt_device_address_t   bd_addr;                /**< [in]  Peer address */
    mxos_bool_t                upgrade;                /**< [out] Set to TRUE to request security upgrade  */
} mxos_bt_dev_security_upgrade_t;

/** Security request (BTM_SECURITY_REQUEST_EVT event data type) */
typedef struct
{
    mxos_bt_device_address_t      bd_addr;             /**< peer address           */
} mxos_bt_dev_security_request_t;

/** LE Key type */
#ifndef BTM_LE_KEY_TYPES
#define BTM_LE_KEY_TYPES
enum mxos_bt_dev_le_key_type_e
{
...
typedef uint8_t mxos_bt_dev_le_key_type_t;             /**< LE key type (see #mxos_bt_dev_le_key_type_e) */


enum mxos_bt_dev_link_key_type_e
{
...
typedef uint8_t mxos_bt_security_level;
...
typedef struct
{
    ...
    BT_OCTET8               rand;
    UINT16                  ediv;
    mxos_bt_security_level  sec_level;
    UINT8                   key_size;
}tBTM_LE_PENC_KEYS;
...
{
    BT_OCTET16                  irk;
    mxos_bt_ble_address_type_t  addr_type;
    mxos_bt_device_address_t    static_addr;
}tBTM_LE_PID_KEYS;
...
} mxos_bt_security_key_value_t;


/** Scan duty cycle (used for BTM_BLE_SCAN_STATE_CHANGED_EVT and mxos_bt_dev_create_connection) */
#ifndef BTM_BLE_SCAN_TYPE
#define BTM_BLE_SCAN_TYPE
enum mxos_bt_ble_scan_type_e
{
...
#endif
typedef uint8_t mxos_bt_ble_scan_type_t;   /**< scan type (see #mxos_bt_ble_scan_type_e) */


/** bonding device information from mxos_bt_dev_get_bonded_devices */
typedef struct
{
    mxos_bt_device_address_t     bd_addr;                 /**< peer address           */
    mxos_bt_ble_address_type_t   addr_type;               /**< peer address type : BLE_ADDR_PUBLIC/BLE_ADDR_RANDOM */
    mxos_bt_device_type_t        device_type;             /**< peer device type : BT_DEVICE_TYPE_BREDR/BT_DEVICE_TYPE_BLE/BT_DEVICE_TYPE_BREDR_BLE  */
}mxos_bt_dev_bonded_device_info_t;

/* LE Secure connection event data */
/** Type of OOB data required  */
#ifndef BTM_OOB_REQ_TYPE
#define BTM_OOB_REQ_TYPE
enum mxos_bt_dev_oob_data_req_type_e
{
...
#endif
typedef UINT8 mxos_bt_dev_oob_data_req_type_t;         /**< OOB data type requested (see #mxos_bt_dev_oob_data_req_type_t) */

/** SMP Pairing status codes */
enum mxos_bt_smp_status_e
{
...
typedef uint8_t mxos_bt_smp_status_t;      /**< SMP Pairing status (see #mxos_bt_smp_status_e) */

/** data type for BTM_SMP_REMOTE_OOB_DATA_REQUEST_EVT */
typedef struct
{
    mxos_bt_device_address_t           bd_addr;        /* peer address */
} mxos_bt_smp_remote_oob_req_t;

/** data type for BTM_SMP_SC_REMOTE_OOB_DATA_REQUEST_EVT */
typedef struct
{
    mxos_bt_device_address_t           bd_addr;        /* peer address */
    mxos_bt_dev_oob_data_req_type_t    oob_type;       /* requested oob data types (BTM_OOB_PEER, BTM_OOB_LOCAL, or BTM_OOB_BOTH) */
} mxos_bt_smp_sc_remote_oob_req_t;
...
} mxos_bt_local_identity_keys_t;

/** SCO connected event related data */
typedef struct {
    uint16_t    sco_index;                  /**< SCO index */
} mxos_bt_sco_connected_t;

/** SCO disconnected event related data */
typedef struct {
    uint16_t    sco_index;                  /**< SCO index */
} mxos_bt_sco_disconnected_t;

/**  SCO connect request event related data */
typedef struct {
    uint16_t                    sco_index;      /**< SCO index */
    mxos_bt_device_address_t   bd_addr;        /**< Peer bd address */
    mxos_bt_dev_class_t        dev_class;      /**< Peer device class */
    mxos_bt_sco_type_t         link_type;      /**< SCO link type */
} mxos_bt_sco_connection_request_t;
...
#endif
typedef uint8_t mxos_bt_dev_power_mgmt_status_t;   /**< Power management status (see #mxos_bt_dev_power_mgmt_status_e) */
...
enum mxos_bt_management_evt_e {
    /* Bluetooth status events */
    BTM_ENABLED_EVT,                                /**< Bluetooth controller and host stack enabled. Event data: mxos_bt_dev_enabled_t */
    BTM_DISABLED_EVT,                               /**< Bluetooth controller and host stack disabled. Event data: NULL */
    BTM_POWER_MANAGEMENT_STATUS_EVT,                /**< Power management status change. Event data: mxos_bt_power_mgmt_notification_t */

    /* Security events */
    BTM_PIN_REQUEST_EVT,                            /**< PIN request (used only with legacy devices). Event data: #mxos_bt_dev_name_and_class_t */
    BTM_USER_CONFIRMATION_REQUEST_EVT,              /**< received USER_CONFIRMATION_REQUEST event (respond using #mxos_bt_dev_confirm_req_reply). Event data: #mxos_bt_dev_user_cfm_req_t */
    BTM_PASSKEY_NOTIFICATION_EVT,                   /**< received USER_PASSKEY_NOTIFY event. Event data: #mxos_bt_dev_user_key_notif_t */
    BTM_PASSKEY_REQUEST_EVT,                        /**< received USER_PASSKEY_REQUEST event (respond using #mxos_bt_dev_pass_key_req_reply). Event data: #mxos_bt_dev_user_key_req_t */
    BTM_KEYPRESS_NOTIFICATION_EVT,                  /**< received KEYPRESS_NOTIFY event. Event data: #mxos_bt_dev_user_keypress_t */
    BTM_PAIRING_IO_CAPABILITIES_BR_EDR_REQUEST_EVT, /**< Requesting IO capabilities for BR/EDR pairing. Event data: #mxos_bt_dev_bredr_io_caps_req_t */
    BTM_PAIRING_IO_CAPABILITIES_BR_EDR_RESPONSE_EVT,/**< Received IO capabilities response for BR/EDR pairing. Event data: #mxos_bt_dev_bredr_io_caps_rsp_t */
    BTM_PAIRING_IO_CAPABILITIES_BLE_REQUEST_EVT,    /**< Requesting IO capabilities for BLE pairing. Event data: #mxos_bt_dev_ble_io_caps_req_t */
    BTM_PAIRING_COMPLETE_EVT,                       /**< received SIMPLE_PAIRING_COMPLETE event. Event data: #mxos_bt_dev_pairing_cplt_t */
    BTM_ENCRYPTION_STATUS_EVT,                      /**< Encryption status change. Event data: #mxos_bt_dev_encryption_status_t */
    BTM_SECURITY_REQUEST_EVT,                       /**< Security request (respond using #mxos_bt_ble_security_grant). Event data: #mxos_bt_dev_security_request_t */
    BTM_SECURITY_UPGRADE_EVT,                       /**< Check if the application wants to upgrade the link key. Event data: #mxos_bt_dev_security_upgrade_t */
    BTM_SECURITY_ABORTED_EVT,                       /**< Security procedure aborted locally, or unexpected link drop. Event data: #mxos_bt_dev_name_and_class_t */

    BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT,           /**< Result of reading local OOB data (mxos_bt_dev_read_local_oob_data). Event data: #mxos_bt_dev_local_oob_t */
    BTM_REMOTE_OOB_DATA_REQUEST_EVT,                /**< OOB data from remote device (respond using #mxos_bt_dev_remote_oob_data_reply). Event data: #mxos_bt_dev_remote_oob_t */

    BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT,         /**< Updated remote device link keys (store device_link_keys to  NV memory). Event data: #mxos_bt_device_link_keys_t */
    BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT,        /**< Request for stored remote device link keys (restore device_link_keys from NV memory). If successful, return MXOS_BT_SUCCESS. Event data: #mxos_bt_device_link_keys_t */

    BTM_LOCAL_IDENTITY_KEYS_UPDATE_EVT,             /**< Update local identity key (stored local_identity_keys NV memory). Event data: #mxos_bt_local_identity_keys_t */
    BTM_LOCAL_IDENTITY_KEYS_REQUEST_EVT,            /**< Request local identity key (get local_identity_keys from NV memory). If successful, return MXOS_BT_SUCCESS. Event data: #mxos_bt_local_identity_keys_t */

    BTM_BLE_SCAN_STATE_CHANGED_EVT,                 /**< BLE scan state change. Event data: #mxos_bt_ble_scan_type_t */
    BTM_BLE_ADVERT_STATE_CHANGED_EVT,               /**< BLE advertisement state change. Event data: #mxos_bt_ble_advert_mode_t */

    /* BLE Secure Connection events */
    BTM_SMP_REMOTE_OOB_DATA_REQUEST_EVT,            /**< SMP remote oob data request. Reply using mxos_bt_smp_oob_data_reply. Event data: mxos_bt_smp_remote_oob_req_t  */
    BTM_SMP_SC_REMOTE_OOB_DATA_REQUEST_EVT,         /**< LE secure connection remote oob data request. Reply using mxos_bt_smp_sc_oob_reply. Event data: #mxos_bt_smp_sc_remote_oob_req_t */
    BTM_SMP_SC_LOCAL_OOB_DATA_NOTIFICATION_EVT,     /**< LE secure connection local OOB data (mxos_bt_smp_create_local_sc_oob_data). Event data: #mxos_bt_smp_sc_local_oob_t*/

    BTM_SCO_CONNECTED_EVT,                          /**< SCO connected event. Event data: #mxos_bt_sco_connected_t */
    BTM_SCO_DISCONNECTED_EVT,                       /**< SCO disconnected event. Event data: #mxos_bt_sco_disconnected_t */
    BTM_SCO_CONNECTION_REQUEST_EVT,                 /**< SCO connection request event. Event data: #mxos_bt_sco_connection_request_t */
    BTM_SCO_CONNECTION_CHANGE_EVT                   /**< SCO connection change event. Event data: #mxos_bt_sco_connection_change_t */
};
#endif
typedef uint8_t mxos_bt_management_evt_t;          /**< Bluetooth management events (see #mxos_bt_management_evt_e) */

/** Device enabled (used by BTM_ENABLED_EVT) */
typedef struct {
    mxos_bt_result_t          status;                     /**< Status */
} mxos_bt_dev_enabled_t;

/** Remote device information (used by BTM_PIN_REQUEST_EVT, BTM_SECURITY_ABORTED_EVT) */
typedef struct {
    mxos_bt_device_address_t  *bd_addr;                /**< BD Address of remote */
    mxos_bt_dev_class_t    *dev_class;                 /**< peer class of device   */
    uint8_t                 *bd_name;                   /**< BD Name of remote */
} mxos_bt_dev_name_and_class_t;

/** Change in power management status  (used by BTM_POWER_MANAGEMENT_STATUS_EVT notication) */
typedef struct {
    mxos_bt_device_address_t           bd_addr;        /**< BD Address of remote */
    mxos_bt_dev_power_mgmt_status_t    status;         /**< PM status */
    uint16_t                            value;          /**< Additional mode data */
    uint8_t                             hci_status;     /**< HCI status */
} mxos_bt_power_mgmt_notification_t;

/** Encryption status change (used by BTM_ENCRYPTION_STATUS_EVT) */
typedef struct {
    uint8_t                 *bd_addr;                   /**< BD Address of remote */
    mxos_bt_transport_t    transport;                  /**< BT_TRANSPORT_BR_EDR or BT_TRANSPORT_LE */
    void                    *p_ref_data;                /**< Optional data passed in by mxos_bt_dev_set_encryption */
    mxos_bt_result_t          result;                     /**< Result of the operation */
} mxos_bt_dev_encryption_status_t;

/** Local OOB data BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT */
typedef struct {
    mxos_bt_result_t          status;                     /**< Status */
    mxos_bool_t            is_extended_oob_data;       /**< TRUE if extended OOB data */
...
} mxos_bt_dev_local_oob_t;

/** BTM_REMOTE_OOB_DATA_REQUEST_EVT */
typedef struct {
    mxos_bt_device_address_t   bd_addr;                /**< BD Address of remote */
    mxos_bool_t                extended_oob_data;      /**< TRUE if requesting extended OOB (P-256) */
} mxos_bt_dev_remote_oob_t;

/** BR/EDR Pairing IO Capabilities (to be filled by application callback on BTM_PAIRING_IO_CAPABILITIES_REQUEST_EVT) */
typedef struct
{
    mxos_bt_device_address_t   bd_addr;                /**< [in] BD Address of remote   */
    mxos_bt_dev_io_cap_t       local_io_cap;           /**< local IO capabilities (to be filled by application callback) */
    mxos_bt_dev_oob_data_t     oob_data;               /**< OOB data present at peer device for the local device   */
    mxos_bt_dev_auth_req_t     auth_req;               /**< Authentication required for peer device                */
    mxos_bool_t                is_orig;                /**< TRUE, if local device initiated the pairing process    */
} mxos_bt_dev_bredr_io_caps_req_t;
...
typedef struct
{
    ...
    mxos_bt_dev_le_auth_req_t  auth_req;               /**< Authentication request (for local device) contain bonding and MITM info    */
    uint8_t                     max_key_size;           /**< Max encryption key size                                                    */
    mxos_bt_dev_le_key_type_t  init_keys;              /**< Keys to be distributed, bit mask                                           */
    mxos_bt_dev_le_key_type_t  resp_keys;              /**< keys to be distributed, bit mask                                           */
} mxos_bt_dev_ble_io_caps_req_t;
...
typedef struct
{
    ...
    mxos_bt_link_key_t                br_edr_key;             /**<  BR/EDR Link Key */

    /* LE Keys */
    mxos_bt_dev_le_key_type_t         le_keys_available_mask; /**<  Mask of available BLE keys */
    mxos_bt_ble_address_type_t        ble_addr_type;          /**<  LE device type: public or random address */
    mxos_bt_ble_address_type_t        static_addr_type;       /**<  static address type */
    mxos_bt_device_address_t          static_addr;            /**<  static address */
    mxos_bt_ble_keys_t                le_keys;                /**<  LE keys */
} mxos_bt_device_sec_keys_t;

/** Paired device link key notification (used by BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT notication) */
typedef struct
{
    mxos_bt_device_address_t   bd_addr;         /**< [in] BD Address of remote */
    mxos_bt_device_sec_keys_t  key_data;        /**< [in/out] Key data */
} mxos_bt_device_link_keys_t;

/** advertisement type (used when calling mxos_bt_start_advertisements) */
#ifndef BTM_BLE_ADVERT_MODE
#define BTM_BLE_ADVERT_MODE
enum mxos_bt_ble_advert_mode_e
{
...
#endif
typedef uint8_t mxos_bt_ble_advert_mode_t;   /**< Advertisement type (see #mxos_bt_ble_advert_mode_e) */

/** scan mode used in initiating */
#ifndef BTM_BLE_CONN_MODE
#define BTM_BLE_CONN_MODE
enum mxos_bt_ble_conn_mode_e
{
...
#endif
typedef uint8_t mxos_bt_ble_conn_mode_t;       /**< Conn mode (see #mxos_bt_ble_conn_mode_e) */

/** Structure definitions for Bluetooth Management (mxos_bt_management_cback_t) event notifications */
typedef union
{
    /* Bluetooth status event data types*/
    mxos_bt_dev_enabled_t              enabled;                            /**< Data for BTM_ENABLED_EVT */
    mxos_bt_power_mgmt_notification_t  power_mgmt_notification;            /**< Data for BTM_POWER_MANAGEMENT_STATUS_EVT */

    /* Security event data types */
    mxos_bt_dev_name_and_class_t       pin_request;                        /**< Data for BTM_PIN_REQUEST_EVT */
    mxos_bt_dev_user_cfm_req_t         user_confirmation_request;          /**< Data for BTM_USER_CONFIRMATION_REQUEST_EVT */
    mxos_bt_dev_user_key_notif_t       user_passkey_notification;          /**< Data for BTM_USER_PASSKEY_NOTIFICATION_EVT */
    mxos_bt_dev_user_key_req_t         user_passkey_request;               /**< Data for BTM_USER_PASSKEY_REQUEST_EVT */
    mxos_bt_dev_user_keypress_t        user_keypress_notification;         /**< Data for BTM_USER_KEYPRESS_NOTIFICATION_EVT - See #mxos_bt_dev_user_keypress_t */
    mxos_bt_dev_bredr_io_caps_req_t    pairing_io_capabilities_br_edr_request; /**< Data for BTM_PAIRING_IO_CAPABILITIES_BR_EDR_REQUEST_EVT */
    mxos_bt_dev_bredr_io_caps_rsp_t    pairing_io_capabilities_br_edr_response;/**< Data for BTM_PAIRING_IO_CAPABILITIES_BR_EDR_RESPONSE_EVT */
    mxos_bt_dev_ble_io_caps_req_t      pairing_io_capabilities_ble_request;    /**< Data for BTM_PAIRING_IO_CAPABILITIES_BLE_REQUEST_EVT */
    mxos_bt_dev_pairing_cplt_t         pairing_complete;                   /**< Data for BTM_PAIRING_COMPLETE_EVT */
    mxos_bt_dev_encryption_status_t    encryption_status;                  /**< Data for BTM_ENCRYPTION_STATUS_EVT */
    mxos_bt_dev_security_request_t     security_request;                   /**< Data for BTM_SECURITY_REQUEST_EVT */
    mxos_bt_dev_security_upgrade_t     security_upgrade;                   /**< Data for BTM_SECURITY_UPGRADE_EVT See #mxos_bt_dev_security_upgrade_t */
    mxos_bt_dev_name_and_class_t       security_aborted;                   /**< Data for BTM_SECURITY_ABORTED_EVT */

    mxos_bt_dev_local_oob_t            read_local_oob_data_complete;       /**< Data for BTM_READ_LOCAL_OOB_DATA_COMPLETE_EVT */
    mxos_bt_dev_remote_oob_t           remote_oob_data_request;            /**< Data for BTM_REMOTE_OOB_DATA_REQUEST_EVT */

    mxos_bt_device_link_keys_t         paired_device_link_keys_update;     /**< Data for BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT */
    mxos_bt_device_link_keys_t         paired_device_link_keys_request;    /**< Data for BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT */
    mxos_bt_local_identity_keys_t      local_identity_keys_update;         /**< Data for BTM_LOCAL_IDENTITY_KEYS_UPDATE_EVT */
    mxos_bt_local_identity_keys_t      local_identity_keys_request;        /**< Data for BTM_LOCAL_IDENTITY_KEYS_REQUEST_EVT */

    mxos_bt_ble_scan_type_t            ble_scan_state_changed;             /**< Data for BTM_BLE_SCAN_STATE_CHANGED_EVT */
    mxos_bt_ble_advert_mode_t          ble_advert_state_changed;           /**< Data for BTM_BLE_ADVERT_STATE_CHANGED_EVT */

    mxos_bt_smp_remote_oob_req_t       smp_remote_oob_data_request;        /**< Data for BTM_SMP_REMOTE_OOB_DATA_REQUEST_EVT */
    mxos_bt_smp_sc_remote_oob_req_t    smp_sc_remote_oob_data_request;     /**< Data for BTM_SMP_SC_REMOTE_OOB_DATA_REQUEST_EVT */
    mxos_bt_smp_sc_local_oob_t         *p_smp_sc_local_oob_data;           /**< Data for BTM_SMP_SC_LOCAL_OOB_DATA_NOTIFICATION_EVT */

    mxos_bt_sco_connected_t            sco_connected;                      /**< Data for BTM_SCO_CONNECTED_EVT */
    mxos_bt_sco_disconnected_t         sco_disconnected;                   /**< Data for BTM_SCO_DISCONNECTED_EVT */
    mxos_bt_sco_connection_request_t   sco_connection_request;             /**< Data for BTM_SCO_CONNECTION_REQUEST_EVT */
    mxos_bt_sco_connection_change_t    sco_connection_change;              /**< Data for BTM_SCO_CONNECTION_CHANGE_EVT */

} mxos_bt_management_evt_data_t;

/**
 * Bluetooth Management callback
 *
 * Callback for Bluetooth Management event notifications.
 * Registered using mxos_bt_stack_init()
 *
 ...
 */
typedef mxos_bt_result_t (mxos_bt_management_cback_t) (mxos_bt_management_evt_t event, mxos_bt_management_evt_data_t *p_event_data);

/**
 * Connection status change callback
 *
 * Callback for Bluetooth Management event notifications.
 * Registered using mxos_bt_register_connection_status_change()
 *
 ...
 * @return void
 */
typedef void (mxos_bt_connection_status_change_cback_t) (mxos_bt_device_address_t bd_addr, uint8_t *p_features, mxos_bool_t is_connected, uint16_t handle, mxos_bt_transport_t transport);  /**<   connection status change callback */
 ...
 * @return Nothing
 */
typedef void (mxos_bt_inquiry_result_cback_t) (mxos_bt_dev_inquiry_scan_result_t *p_inquiry_result, uint8_t *p_eir_data); /**<   inquiry result callback */
 ...
 * @return Nothing
 */
typedef void (mxos_bt_dev_cmpl_cback_t) (void *p_data);
 ...
 * @return Nothing
 */
typedef void (mxos_bt_dev_vendor_specific_command_complete_cback_t) (mxos_bt_dev_vendor_specific_command_complete_params_t *p_command_complete_params);
 ...
 *
 * @addtogroup  mxosbt_DeviceManagement    Device Management
 * @ingroup     mxosbt
 *
 * @{
 */
 ...
 *
 * @addtogroup  mxosbt_bredr    BR/EDR (Bluetooth Basic Rate / Enhanced Data Rate)
 * @ingroup     mxosbt_DeviceManagement
 *
 * @{
 */
/****************************************************************************/
...
/**
 * Function         mxos_bt_start_inquiry
 *
 ...
 *
 * @return          mxos_bt_result_t
 *
 *                  MXOS_BT_PENDING if successfully initiated
 *                  MXOS_BT_BUSY if already in progress
 *                  MXOS_BT_ILLEGAL_VALUE if parameter(s) are out of range
 *                  MXOS_BT_NO_RESOURCES if could not allocate resources to start the command
 *                  MXOS_BT_WRONG_MODE if the device is not up.
 */
mxos_bt_result_t  mxos_bt_start_inquiry (mxos_bt_dev_inq_parms_t *p_inqparms, mxos_bt_inquiry_result_cback_t *p_inquiry_result_cback);

/**
 * Function         mxos_bt_cancel_inquiry
 *
 *                  Cancel inquiry
 *
 * @return
 *
 *                  MXOS_BT_SUCCESS         if successful
 *                  MXOS_BT_NO_RESOURCES    if could not allocate a message buffer
 *                  MXOS_BT_WRONG_MODE      if the device is not up.
 */
mxos_bt_result_t mxos_bt_cancel_inquiry(void);

/**
 * Function         mxos_bt_dev_read_local_addr
 *
 ...
 *
 */
void mxos_bt_dev_read_local_addr (mxos_bt_device_address_t bd_addr);


/**
 * Function         mxos_bt_dev_set_advanced_connection_params
 *
 ...
 *
 * @param[in]       p_inquiry_scan_result : Inquiry scan result (from #mxos_bt_inquiry_result_cback_t)
 *
 * @return          mxos_bt_result_t
 *
 *                  MXOS_BT_SUCCESS : on success;
 *                  MXOS_BT_FAILED  : if an error occurred
 */
mxos_bt_result_t mxos_bt_dev_set_advanced_connection_params (mxos_bt_dev_inquiry_scan_result_t *p_inquiry_scan_result);


/**
 * Function         mxos_bt_dev_set_local_device_address
 *
 *                  
 ...
 * @return
 *
 *                  MXOS_BT_SUCCESS    : Command sent. Does not expect command complete event. (command complete callback param is NULL)
 *                  MXOS_BT_NO_RESOURCES: Failure and no resources.
 *
 */
mxos_bt_result_t mxos_bt_dev_set_local_device_address(mxos_bt_device_address_t bdaddr, 
                                                      mxos_bt_dev_cmpl_cback_t *p_cback);

/**
 * Function         mxos_bt_dev_vendor_specific_command
 *
 ...
 *
 *                  MXOS_BT_SUCCESS    : Command sent. Does not expect command complete event. (command complete callback param is NULL)
 *                  MXOS_BT_PENDING    : Command sent. Waiting for command complete event.
 *                  MXOS_BT_BUSY       : Command not sent. Waiting for command complete event for prior command.
 *
 */
mxos_bt_result_t mxos_bt_dev_vendor_specific_command (uint16_t opcode, uint8_t param_len, uint8_t *p_param_buf,
                                mxos_bt_dev_vendor_specific_command_complete_cback_t *p_cback);

/**
 * Function         mxos_bt_dev_set_discoverability
 *
 *                  Set discoverability
 *
 * @note            The duration must be less than or equal to the interval.
 *
 * @param[in]       inq_mode        : Discoverability mode (see #mxos_bt_discoverability_mode_e)
 ...
 *
 *                  MXOS_BT_SUCCESS:        If successful
 *                  MXOS_BT_BUSY:           If a setting of the filter is already in progress
 *                  MXOS_BT_NO_RESOURCES:   If couldn't get a memory pool buffer
 *                  MXOS_BT_ILLEGAL_VALUE:  If a bad parameter was detected
 *                  MXOS_BT_WRONG_MODE:     If the device is not up
 */
mxos_bt_result_t  mxos_bt_dev_set_discoverability (uint8_t inq_mode, uint16_t duration,
                                                    uint16_t interval);

/**
 * Function         mxos_bt_dev_set_connectability
 *
 *                  Set connectablilty
 *
 * @note            The duration (window parameter) must be less than or equal to the interval.
 *
 * @param[in]       page_mode       : Connectability mode (see #mxos_bt_connectability_mode_e)
 ...
 *
 *                  MXOS_BT_SUCCESS:        If successful
 *                  MXOS_BT_ILLEGAL_VALUE:  If a bad parameter is detected
 *                  MXOS_BT_NO_RESOURCES:   If could not allocate a message buffer
 *                  MXOS_BT_WRONG_MODE:     If the device is not up
 */
mxos_bt_result_t mxos_bt_dev_set_connectability (uint8_t page_mode, uint16_t window,
                                                      uint16_t interval);
/**
 * Function         mxos_bt_dev_register_connection_status_change
 *
 *                  Register callback for connection status change
 *
 *
 * @param[in]       p_mxos_bt_connection_status_change_cback - Callback for connection status change
 *
 * @return          mxos_bt_result_t
 *
 *                  MXOS_BT_SUCCESS : on success;
 *                  MXOS_BT_FAILED : if an error occurred
 */
mxos_bt_result_t mxos_bt_dev_register_connection_status_change(mxos_bt_connection_status_change_cback_t *p_mxos_bt_connection_status_change_cback);

/**
 * Function         mxos_bt_dev_set_sniff_mode
 *
 ...
 *
 * @return          MXOS_BT_PENDING if successfully initiated, otherwise error
 */
mxos_bt_result_t mxos_bt_dev_set_sniff_mode (mxos_bt_device_address_t remote_bda, uint16_t min_period,
                                             uint16_t max_period, uint16_t attempt,
                                             uint16_t timeout);


/**
 * Function         mxos_bt_dev_cancel_sniff_mode
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_cancel_sniff_mode (mxos_bt_device_address_t remote_bda);


/**
 *
 * Function         mxos_bt_dev_set_sniff_subrating
 *
 ...
 *
 *                  MXOS_BT_SUCCESS        : on success;
 *                  MXOS_BT_ILLEGAL_ACTION : if an error occurred
 */
mxos_bt_result_t mxos_bt_dev_set_sniff_subrating (mxos_bt_device_address_t remote_bda, uint16_t max_latency,
                              uint16_t min_remote_timeout, uint16_t min_local_timeout);

/**
 * Function         mxos_bt_dev_read_rssi
 *
 ...
 * @param[in]       p_cback         : Result callback (mxos_bt_dev_rssi_result_t will be passed to the callback)
 *
 * @return
 *
 *                  MXOS_BT_PENDING if command issued to controller.
 *                  MXOS_BT_NO_RESOURCES if couldn't allocate memory to issue command
 *                  MXOS_BT_UNKNOWN_ADDR if no active link with bd addr specified
 *                  MXOS_BT_BUSY if command is already in progress
 *
 */
mxos_bt_result_t mxos_bt_dev_read_rssi (mxos_bt_device_address_t remote_bda, mxos_bt_transport_t transport, mxos_bt_dev_cmpl_cback_t *p_cback);

/**
 * Function         mxos_bt_dev_read_tx_power
 *
 *                  Read the transmit power for the requested link
 *
 * @param[in]       remote_bda      : BD address of connection to read tx power
 * @param[in]       transport       : Transport type
 * @param[in]       p_cback         : Result callback (mxos_bt_tx_power_result_t will be passed to the callback)
 *
 * @return
 *
 *                  MXOS_BT_PENDING if command issued to controller.
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_read_tx_power (mxos_bt_device_address_t remote_bda, mxos_bt_transport_t transport,
                                            mxos_bt_dev_cmpl_cback_t *p_cback);


/**
 *
 * Function         mxos_bt_dev_write_eir
 *
 ...
 *
 * @return          MXOS_BT_SUCCESS if successful
 *                  MXOS_BT_NO_RESOURCES if couldn't allocate memory to issue command
 *                  MXOS_BT_UNSUPPORTED if local device cannot support request
 *
 */
mxos_bt_result_t mxos_bt_dev_write_eir (uint8_t *p_buff, uint16_t len);
...
/**
 * Function         mxos_bt_dev_set_pin_code_only 
 * 
 ...
 * @return          mxos_bt_result_t
 */
mxos_bt_result_t mxos_bt_dev_set_pin_code_only(mxos_bool_t enable);

/**
 * Function         mxos_bt_dev_set_security_mode 
 *
 ...
 *
 */
void mxos_bt_dev_set_pin_type(mxos_bool_t fixed_pin, uint8_t *pin_code, uint8_t pin_code_len);


/**
 * Function         mxos_bt_dev_pin_code_reply
 *
 *                  PIN code reply (use in response to <b>BTM_PIN_REQUEST_EVT </b> in #mxos_bt_management_cback_t)
 *
 *  @param[in]      bd_addr     : Address of the device for which PIN was requested
 *  @param[in]      res         : result of the operation MXOS_BT_SUCCESS if success
 *  @param[in]      pin_len     : length in bytes of the PIN Code
 *  @param[in]      p_pin       : pointer to array with the PIN Code
 *
 * @return          void
 */
void mxos_bt_dev_pin_code_reply (mxos_bt_device_address_t bd_addr, mxos_bt_result_t res, uint8_t pin_len, uint8_t *p_pin);

/**
 * Function         mxos_bt_dev_sec_bond
 *
 ...
 *                  error code
 */
mxos_bt_result_t mxos_bt_dev_sec_bond (mxos_bt_device_address_t bd_addr, mxos_bt_ble_address_type_t bd_addr_type, mxos_bt_transport_t transport, uint8_t pin_len, uint8_t *p_pin);


/**
 * Function         mxos_bt_dev_sec_bond_cancel
 *
 ...
 *  @return
 *
 *                  MXOS_BT_PENDING if cancel initiated,
 *                  MXOS_BT_SUCCESS if cancel has completed already, else error code.
 */
mxos_bt_result_t mxos_bt_dev_sec_bond_cancel (mxos_bt_device_address_t bd_addr);


/**
 * Function         mxos_bt_dev_set_encryption
 *
 *                  Encrypt the specified connection.
 *                  Status is notified using <b>BTM_ENCRYPTION_STATUS_EVT </b> of #mxos_bt_management_cback_t.
 *
 ...
 * @return
 *
 *                  MXOS_BT_SUCCESS            : already encrypted
 ...
 */
mxos_bt_result_t mxos_bt_dev_set_encryption (mxos_bt_device_address_t bd_addr, mxos_bt_transport_t transport, void *p_ref_data);


/**
 * Function         mxos_bt_dev_confirm_req_reply
 *
 *                  Confirm the numeric value for pairing (in response to <b>BTM_USER_CONFIRMATION_REQUEST_EVT </b> of #mxos_bt_management_cback_t)
 *
 * @param[in]       res           : result of the operation MXOS_BT_SUCCESS if success
 ...
 * @return          void
 */
void mxos_bt_dev_confirm_req_reply(mxos_bt_result_t res, mxos_bt_device_address_t bd_addr);

/**
 * Function         mxos_bt_dev_pass_key_req_reply
 *
 *                  Provide the pairing passkey (in response to <b>BTM_PASSKEY_REQUEST_EVT </b> of #mxos_bt_management_cback_t)
 *
 * @param[in]       res           : result of the operation MXOS_BT_SUCCESS if success
 * @param[in]       bd_addr       : Address of the peer device
 ...
 * @return          void
 */
void mxos_bt_dev_pass_key_req_reply(mxos_bt_result_t res, mxos_bt_device_address_t bd_addr, uint32_t passkey);

/**
 * Function         mxos_bt_dev_send_key_press_notif
 *
 ...
 *
 */
void mxos_bt_dev_send_key_press_notif(mxos_bt_device_address_t bd_addr, mxos_bt_dev_passkey_entry_type_t type);

/**
 *
 * Function         mxos_bt_dev_read_local_oob_data
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_read_local_oob_data(void);

/**
 * Function         mxos_bt_dev_remote_oob_data_reply
 *
 ...
 *
 */
void mxos_bt_dev_remote_oob_data_reply (mxos_bt_result_t res, mxos_bt_device_address_t bd_addr,
                                              mxos_bool_t is_extended_oob_data,
                                              BT_OCTET16 c_192, BT_OCTET16 r_192,
                                              BT_OCTET16 c_256, BT_OCTET16 r_256);

/*
 *
 * Function         mxos_bt_dev_build_oob_data
 *
 ...
 *
 */
uint16_t mxos_bt_dev_build_oob_data(uint8_t *p_data, uint16_t max_len,
                                          mxos_bool_t is_extended_oob_data,
                                          BT_OCTET16 c_192, BT_OCTET16 r_192,
                                          BT_OCTET16 c_256, BT_OCTET16 r_256);
...
/*
 *
 * Function         mxos_bt_smp_oob_data_reply
 *
 ...
 *
 */
void mxos_bt_smp_oob_data_reply(mxos_bt_device_address_t bd_addr, mxos_bt_result_t res, uint8_t len, uint8_t *p_data);

/**
 * Function         mxos_bt_smp_create_local_sc_oob_data
 *
 ...
 *
 */
mxos_bool_t mxos_bt_smp_create_local_sc_oob_data (mxos_bt_device_address_t bd_addr, mxos_bt_ble_address_type_t bd_addr_type);

/**
 *
 * Function         mxos_bt_smp_sc_oob_reply
 *
 ...
 *
 */
void mxos_bt_smp_sc_oob_reply (uint8_t *p_oob_data);

/**@} btm_sec_api_functions */


/**@} mxosbt_DeviceManagement */
...
}mxos_bt_hci_trace_type_t;

/**
 * HCI trace callback
 *
 * Callback for HCI traces
 * Registered using mxos_bt_dev_register_hci_trace()
 *
 ...
 * @return void
 */
typedef void ( mxos_bt_hci_trace_cback_t )( mxos_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );

/**
 * Function         mxos_bt_dev_register_hci_trace
 *
 ...
 *
 */
void mxos_bt_dev_register_hci_trace( mxos_bt_hci_trace_cback_t* p_cback );


/**
 * Function         mxos_bt_dev_get_bonded_devices
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_get_bonded_devices(mxos_bt_dev_bonded_device_info_t *p_paired_device_list,uint16_t *p_num_devices);

/**
 * Function         mxos_bt_dev_find_bonded_device
 *
 ...
 *
 */
mxos_bool_t mxos_bt_dev_find_bonded_device( mxos_bt_device_address_t bd_addr);

/**
 * Function         mxos_bt_dev_get_key_by_keytype
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_get_key_by_keytype(mxos_bt_device_address_t bd_addr, mxos_bt_dev_le_key_type_t key_type, mxos_bt_security_key_value_t *p_sec_keys);



/**
 * Function         mxos_bt_dev_delete_bonded_device
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_delete_bonded_device(mxos_bt_device_address_t bd_addr);


/**
 * Function         mxos_bt_dev_add_device_to_address_resolution_db
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_add_device_to_address_resolution_db(mxos_bt_device_link_keys_t *p_link_keys);


/**
 * Function         mxos_bt_dev_remove_device_from_address_resolution_db
 *
 ...
 *
 */
mxos_bt_result_t mxos_bt_dev_remove_device_from_address_resolution_db(mxos_bt_device_link_keys_t *p_link_keys);
