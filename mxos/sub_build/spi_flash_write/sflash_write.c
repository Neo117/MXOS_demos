...
{
    [BOOTLOADER_FIRMWARE_PARTITION_TCL]         = MODULE_PARTITION_BOOTLOADER,
    [APPLICATION_FIRMWARE_PARTITION_TCL]        = MODULE_PARTITION_APPLICATION,
    [ATE_FIRMWARE_PARTITION_TCL]                = MODULE_PARTITION_ATE,
    [WIFI_FIRMWARE_PARTITION_TCL]               = MODULE_PARTITION_RF_FIRMWARE,
    [PARAMETER_1_IMAGE_PARTITION_TCL]           = MODULE_PARTITION_INFO,
    [PARAMETER_2_IMAGE_PARTITION_TCL]           = MODULE_PARTITION_PARAMETER_2,
#ifdef MXOS_USE_BT_PARTITION
    [BT_PATCH_FIRMWARE_PARTITION_TCL]           = MODULE_PARTITION_BT_FIRMWARE,
#else
    [BT_PATCH_FIRMWARE_PARTITION_TCL]           = -1,
#endif
    [FILESYSTEM_IMAGE_PARTITION_TCL]            = MODULE_PARTITION_FILESYS,
...
//    sflash_handle_t sflash_handle;
    ...
    while ( 1 == 1 )
    {
        ...
        if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_READ) != 0 )
        {
            ...
            if ( 0
                != mhal_flash_read( partition_remapping[data_transfer.partition], &position, data_transfer.data,
        ...
        if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_INITIAL_VERIFY) != 0 )
        {
            ...
            while ( pos < data_transfer.size )
            {
                ...
                position = pos + data_transfer.partition_offset;
                if ( 0
                    != mhal_flash_read( partition_remapping[data_transfer.partition], &position, Rx_Buffer,
                                      read_size ) )
                {
        ...
        if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_ERASE_CHIP) != 0 )
        {
            ...
            if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_VERIFY_CHIP_ERASURE) != 0 )
            {
                ...
                while ( pos < chip_size )
                {
                    ...
                    if ( 0
                        != mhal_flash_read( partition_remapping[data_transfer.partition], &pos, Rx_Buffer, read_size ) )
                    {
                        /* Verify Error - Chip not erased properly */
                        data_transfer.result = MFG_SPI_FLASH_RESULT_READ_FAILED;
                        flash_program_log( "Read error - abort!" );
        ...
        if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_WRITE) != 0 )
        {
            ...
            while ( pos < data_transfer.size )
            {
                ...
                position = pos + data_transfer.partition_offset;
                if ( 0
                    != mhal_flash_write( partition_remapping[data_transfer.partition], &position,
        ...
        if ( (data_transfer.command & MFG_SPI_FLASH_COMMAND_WRITE_ERASE_IF_NEEDED) != 0 )
        {
            ...
            partition_info = mhal_flash_get_info( partition_remapping[data_transfer.partition] );
            if(  platform_flash_peripherals[partition_info->partition_owner].flash_type == FLASH_TYPE_EMBEDDED && erase_once )
            {
                ...
                mhal_flash_erase( partition_remapping[data_transfer.partition], 0x0, partition_info->partition_length);
                erase_once = false;
            }
            ...
            while ( size_left_to_compare > 0 )
            {
...
#endif /* ALWAYS_ERASE_SECTOR */
                {
                    ...
                    if ( not_erased( &Rx_Buffer[compare_start], cmp_len ) )
                    {
                        ...
                        offset = pos;
                        if ( 0 != mhal_flash_erase( partition_remapping[data_transfer.partition], offset, SECTOR_SIZE ) )
                        {
                    ...
                    if ( 0
                        != mhal_flash_write( partition_remapping[data_transfer.partition], &position, Rx_Buffer,
                                           (unsigned int) SECTOR_SIZE ) )
                    {
...
__root void _mxos_iar_program_start(void)
{
