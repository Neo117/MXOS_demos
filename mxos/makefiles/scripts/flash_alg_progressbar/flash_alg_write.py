...
def sflasher(argv):
    ...
    parser.add_argument('-d', '--debugger', type=str,
                        required=True, help='debugger name')
    ...
    cmd = '%s \
    -f mxos/makefiles/OpenOCD/interface/%s.cfg \
    ...
    2>build/openocd.log' \
    % (args.openocd, args.debugger, args.mcu, args.mcu, args.mcu, args.file, args.addr)
