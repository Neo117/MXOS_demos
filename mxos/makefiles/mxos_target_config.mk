...
COMPONENT_DIRECTORIES := $(MXOS_OS_PATH) \
                         $(MXOS_OS_PATH)/sub_build \
                         $(MXOS_OS_PATH)/module \
                         $(MXOS_OS_PATH)/platform \
                         $(MXOS_OS_PATH)/MXOS \
                         $(MXOS_OS_PATH)/MXOS/net \
                         $(MXOS_OS_PATH)/MXOS/RTOS \
                         $(MXOS_OS_PATH)/MXOS/security/TLS \
                         $(MXOS_OS_PATH)/libraries \
                         $(SOURCE_ROOT) \
                         $(SOURCE_ROOT)/module \
                         $(SOURCE_ROOT)/demos

MXOS_SDK_VERSION ?= $(MXOS_SDK_VERSION_MAJOR).$(MXOS_SDK_VERSION_MINOR).$(MXOS_SDK_VERSION_REVISION)
...
$(eval $(NAME)_MAKEFILE := $(TEMP_MAKEFILE))
MXOS_SDK_MAKEFILES     += $($(NAME)_MAKEFILE)
...
MXOS_SDK_INCLUDES           +=$(addprefix -I$($(NAME)_LOCATION),$(GLOBAL_INCLUDES))
MXOS_SDK_INCLUDES           +=$(addprefix -I,$(GLOBAL_ABS_INCLUDES))

MXOS_SDK_LINK_SCRIPT        +=$(if $(GLOBAL_LINK_SCRIPT),$(GLOBAL_LINK_SCRIPT),)
MXOS_SDK_DEFAULT_LINK_SCRIPT+=$(if $(DEFAULT_LINK_SCRIPT),$(addprefix $($(NAME)_LOCATION),$(DEFAULT_LINK_SCRIPT)),)
MXOS_SDK_DEFINES            +=$(GLOBAL_DEFINES)
MXOS_SDK_CFLAGS             +=$(GLOBAL_CFLAGS)
MXOS_SDK_CXXFLAGS           +=$(GLOBAL_CXXFLAGS)
MXOS_SDK_ASMFLAGS           +=$(GLOBAL_ASMFLAGS)
MXOS_SDK_LDFLAGS            +=$(GLOBAL_LDFLAGS)
MXOS_SDK_CHIP_SPECIFIC_SCRIPT += $(CHIP_SPECIFIC_SCRIPT)
MXOS_SDK_CONVERTER_OUTPUT_FILE += $(CONVERTER_OUTPUT_FILE)
MXOS_SDK_FINAL_OUTPUT_FILE += $(BIN_OUTPUT_FILE)
...
MXOS_SDK_LDFLAGS  += $(COMPILER_SPECIFIC_RELEASE_LDFLAGS)
else
MXOS_SDK_LDFLAGS  += $(COMPILER_SPECIFIC_DEBUG_LDFLAGS)
endif
...
PLATFORM_FULL   	:=$(strip $(foreach comp,$(subst .,/,$(COMPONENTS)),$(if $(wildcard $(MXOS_OS_PATH)/module/$(comp)),$(MXOS_OS_PATH)/module/$(comp),$(if $(wildcard $(SOURCE_ROOT)/module/$(comp)),$(SOURCE_ROOT)/module/$(comp), ))))
...
$(error mbed os component not found, use mxos cube command: "mxos add mbed-os".)
endif
endif
...
APP_FULL        :=$(strip $(foreach comp,$(subst .,/,$(COMPONENTS)),$(if $(wildcard $(SOURCE_ROOT)$(comp)),$(SOURCE_ROOT)$(comp),$(if $(wildcard $(MXOS_OS_PATH)/$(comp)),$(MXOS_OS_PATH)/$(comp),$(if $(wildcard $(SOURCE_ROOT)/demos/$(comp)),$(SOURCE_ROOT)/demos/$(comp))))))
APP         :=$(notdir $(APP_FULL))


EXTRA_CFLAGS :=    -DMXOS_SDK_VERSION_MAJOR=$(MXOS_SDK_VERSION_MAJOR) \
                   -DMXOS_SDK_VERSION_MINOR=$(MXOS_SDK_VERSION_MINOR) \
                   -DMXOS_SDK_VERSION_REVISION=$(MXOS_SDK_VERSION_REVISION) \
                   -DBUS=$(SLASH_QUOTE_START)$$(BUS)$(SLASH_QUOTE_END) \
                   -I$(OUTPUT_DIR)/resources/  \
                   -DPLATFORM=$(SLASH_QUOTE_START)$$(PLATFORM)$(SLASH_QUOTE_END)

MXOS_OS_VERSION := $(shell git -C mxos describe)
GITDIFFSTAT := $(shell git -C mxos diff --stat)
ifeq ($(GITDIFFSTAT),)
EXTRA_CFLAGS += -DMXOS_OS_VERSION=\"$(MXOS_OS_VERSION)\"
else
EXTRA_CFLAGS += -DMXOS_OS_VERSION=\"$(MXOS_OS_VERSION)-dirty\"
endif
MXOS_OS_USER_NAME := $(shell git config user.name)
EXTRA_CFLAGS += -DMXOS_OS_USER_NAME=\"$(firstword $(MXOS_OS_USER_NAME))\"
...
ifneq ($(filter $(HOST_ARCH),Cortex-M3 Cortex-M4 Cortex-M4F Cortex-R4 Cortex-M0 Cortex-M0plus ARM968E-S Cortex-M33 Cortex-M33F),)
...
include $(MAKEFILES_PATH)/mxos_toolchain_Win32_MIPS.mk
...
CC_VERSION := $(shell $(TOOLCHAIN_PATH)$(TOOLCHAIN_PREFIX)gcc$(EXECUTABLE_SUFFIX) -dumpversion)
...
MXOS_SDK_INCLUDES += -I$(MXOS_OS_PATH)/include -I$(APP_FULL) -I.
MXOS_SDK_DEFINES += $(EXTERNAL_MXOS_GLOBAL_DEFINES)
...
$(if $(PLATFORM),,$(error No platform specified. Options are: $(notdir $(wildcard module/*))))
...
MXOS_SDK_PREBUILT_LIBRARIES +=$(foreach comp,$(PROCESSED_COMPONENTS), $(addprefix $($(comp)_LOCATION),$($(comp)_PREBUILT_LIBRARY)))
MXOS_SDK_LINK_FILES         +=$(foreach comp,$(PROCESSED_COMPONENTS), $(addprefix $$(OUTPUT_DIR)/Modules/$(call GET_BARE_LOCATION,$(comp)),$($(comp)_LINK_FILES)))
MXOS_SDK_UNIT_TEST_SOURCES  +=$(foreach comp,$(PROCESSED_COMPONENTS), $(addprefix $($(comp)_LOCATION),$($(comp)_UNIT_TEST_SOURCES)))

ifeq ($(ADD_UNIT_TESTS_TO_LINK_FILES),1)
MXOS_SDK_LINK_FILES         += $(patsubst %.cpp,%.o,$(patsubst %.cc,%.o,$(patsubst %.c,%.o, $(foreach comp,$(PROCESSED_COMPONENTS), $(addprefix $$(OUTPUT_DIR)/Modules/$(call GET_BARE_LOCATION,$(comp)),$($(comp)_UNIT_TEST_SOURCES))) )))
endif
...
$(CONFIG_FILE): $(MXOS_SDK_MAKEFILES) | $(CONFIG_FILE_DIR)
	$(QUIET)$(call WRITE_FILE_CREATE, $(CONFIG_FILE) ,MXOS_SDK_MAKEFILES           		+= $(MXOS_SDK_MAKEFILES))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,TOOLCHAIN_NAME            		:= $(TOOLCHAIN_NAME))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_LDFLAGS             		+= $(strip $(MXOS_SDK_LDFLAGS)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,RESOURCE_CFLAGS					+= $(strip $(MXOS_SDK_CFLAGS)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_LINK_SCRIPT         		+= $(strip $(if $(strip $(MXOS_SDK_LINK_SCRIPT)),$(MXOS_SDK_LINK_SCRIPT),$(MXOS_SDK_DEFAULT_LINK_SCRIPT))))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_LINK_SCRIPT_CMD    	 	+= $(call COMPILER_SPECIFIC_LINK_SCRIPT,$(strip $(if $(strip $(MXOS_SDK_LINK_SCRIPT)),$(MXOS_SDK_LINK_SCRIPT),$(MXOS_SDK_DEFAULT_LINK_SCRIPT)))))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_PREBUILT_LIBRARIES 	 	+= $(strip $(MXOS_SDK_PREBUILT_LIBRARIES)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_CERTIFICATES       	 	+= $(strip $(MXOS_SDK_CERTIFICATES)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_PRE_APP_BUILDS      		+= $(strip $(PRE_APP_BUILDS)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_LINK_FILES          		+= $(MXOS_SDK_LINK_FILES))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_INCLUDES           	 	+= $(call unique,$(MXOS_SDK_INCLUDES)))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_DEFINES             		+= $(call unique,$(strip $(addprefix -D,$(MXOS_SDK_DEFINES)))))
	...
	$(QUIET)$(foreach comp,$(PROCESSED_COMPONENTS), $(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,$(comp)_CFLAGS           := $(MXOS_SDK_CFLAGS) $($(comp)_CFLAGS_ALL)))
	$(QUIET)$(foreach comp,$(PROCESSED_COMPONENTS), $(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,$(comp)_CXXFLAGS         := $(MXOS_SDK_CXXFLAGS) $($(comp)_CXXFLAGS_ALL)))
	$(QUIET)$(foreach comp,$(PROCESSED_COMPONENTS), $(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,$(comp)_ASMFLAGS         := $(MXOS_SDK_ASMFLAGS) $($(comp)_ASMFLAGS)))
	...
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_UNIT_TEST_SOURCES   		:= $(MXOS_SDK_UNIT_TEST_SOURCES))
	...
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_ROM_SYMBOL_LIST_FILE 		:= $(MXOS_ROM_SYMBOL_LIST_FILE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_CHIP_SPECIFIC_SCRIPT		:= $(MXOS_SDK_CHIP_SPECIFIC_SCRIPT))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_CONVERTER_OUTPUT_FILE	:= $(MXOS_SDK_CONVERTER_OUTPUT_FILE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_SDK_FINAL_OUTPUT_FILE 		:= $(MXOS_SDK_FINAL_OUTPUT_FILE))
	$(QUIET)$(call WRITE_FILE_APPEND, $(CONFIG_FILE) ,MXOS_RAM_STUB_LIST_FILE 			:= $(MXOS_RAM_STUB_LIST_FILE))
