...
static merr_t http_generate_request( FILE_DOWNLOAD_CONTEXT_S *file_download_context );
static merr_t socket_send( int fd, const char *inBuf, uint32_t inBufLen );
static merr_t file_download_dns( FILE_DOWNLOAD_CONTEXT_S *file_download_context, const char * domain, uint8_t * addr, uint8_t addrLen );
static merr_t onReceivedData( struct _HTTPHeader_t * inHeader, uint32_t inPos, uint8_t * inData, size_t inLen, void * inUserContext );

static merr_t http_connect( FILE_DOWNLOAD_CONTEXT_S *file_download_context );
static merr_t http_disconnect( FILE_DOWNLOAD_CONTEXT_S *file_download_context );
...
extern merr_t http_file_download_start( FILE_DOWNLOAD_CONTEXT_S **file_download_context_user, const char *file_url, FILE_DOWNLOAD_STATE_CB download_state_cb, FILE_DOWNLOAD_DATA_CB download_data_cb, uint32_t user_args);
...
static merr_t http_process_redirect( FILE_DOWNLOAD_CONTEXT_S * file_download_context )
{
...
//set tcp send and recv timeout
static int user_set_tcp_option( int socket, int send_timeout, int send_utimeout, int recv_timeout, int recv_utimeout )
{
    int retVal = 0;
    struct timeval opt;

    opt.tv_sec = send_timeout;
    opt.tv_usec = send_utimeout;
    retVal = setsockopt( socket, SOL_SOCKET, SO_SNDTIMEO, (void *) &opt, sizeof(opt) );
    require_string( retVal >= 0, exit, "SO_SNDTIMEO setsockopt error!" );

    app_log("setsockopt SO_SNDTIMEO=%d ms ok.", send_timeout);

    opt.tv_sec = recv_timeout;
    opt.tv_usec = recv_utimeout;
    retVal = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (void *)&opt,sizeof(opt));
...
static merr_t http_connect( FILE_DOWNLOAD_CONTEXT_S *file_download_context )
{
    ...
    {
        ret = user_set_tcp_option( file_download_context->ip_info.http_fd, HTTP_SEND_TIME_OUT, HTTP_SEND_UTIME_OUT, HTTP_RECV_TIME_OUT, HTTP_RECV_UTIME_OUT );
    ...
    if ( file_download_context->ip_info.serurity_type == HTTP_SECURITY_HTTPS )
    {
        ...
        file_download_context->ip_info.http_ssl = mtls_connect( file_download_context->ip_info.http_fd, 0, NULL, &ssl_errno);
    ...
    app_log("#####FILE_DOWNLOAD CONNECT#####:num_of_chunks:%d, free:%d, fd:%d", mos_mallinfo()->chunks, mos_mallinfo()->free, file_download_context->ip_info.http_fd);
    err = kNoErr;
...
static merr_t http_disconnect( FILE_DOWNLOAD_CONTEXT_S *file_download_context )
{
    ...
    require_string( file_download_context != NULL, exit, "param error" );
    app_log("#####FILE_DOWNLOAD DISCONNECT#####:num_of_chunks:%d, free:%d, fd:%d", mos_mallinfo()->chunks, mos_mallinfo()->free, file_download_context->ip_info.http_fd);
    ...
    if ( file_download_context->ip_info.serurity_type == HTTP_SECURITY_HTTPS )
    {
        ...
        {
            mtls_close( file_download_context->ip_info.http_ssl );
...
//http send request
static merr_t http_send_req( FILE_DOWNLOAD_CONTEXT_S *file_download_context )
{
    merr_t err = kGeneralErr;
    int ret = 0;
    ...
    if ( file_download_context->ip_info.serurity_type == HTTP_SECURITY_HTTPS )
    {
        ret = mtls_send( file_download_context->ip_info.http_ssl, file_download_context->http_req, strlen( (const char *) file_download_context->http_req ) ); /* Send HTTP Request */
        if ( ret > 0 )
        {
...
void http_file_download_thread( void *arg )
{
    merr_t err = kGeneralErr;
            ...
            case kNoErr:
                {
                    ...
                    if ( (file_download_context->httpHeader->statusCode == 200) || (file_download_context->httpHeader->statusCode == 206) )
                    {
                        ...
                        if (file_download_context->httpHeader->chunkedData == true) {
                            file_download_context->is_success = true;
                        }
                    ...
                    if (file_download_context->httpHeader->chunkedData == false) {
...
static merr_t onReceivedData( struct _HTTPHeader_t * inHeader, uint32_t inPos, uint8_t * inData, size_t inLen, void * inUserContext )
{
    ...
    file_download_context->retry_count = 0; // got new data, reset retry count. 
    ...
    if (inHeader->chunkedData == false) {
        ...
        old_progress = file_download_context->file_info.download_len;
    }
    //TODO
    ...
    if (inHeader->chunkedData == false) {
        ...
        progress = file_download_context->file_info.download_len;
