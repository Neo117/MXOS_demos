 ...
 */
merr_t http_file_download_start( FILE_DOWNLOAD_CONTEXT *file_download_context_user, const char *file_url, FILE_DOWNLOAD_STATE_CB download_state_cb, FILE_DOWNLOAD_DATA_CB download_data_cb, uint32_t user_args);
 ...
 * @retval kNoErr is returned on success, otherwise, kXXXErr is returned
 */
merr_t http_file_download_pause( FILE_DOWNLOAD_CONTEXT *file_download_context );
 ...
 * @retval kNoErr is returned on success, otherwise, kXXXErr is returned
 */
merr_t http_file_download_continue( FILE_DOWNLOAD_CONTEXT *file_download_context );
 ...
 * @retval kNoErr is returned on success, otherwise, kXXXErr is returned
 */
merr_t http_file_download_stop( FILE_DOWNLOAD_CONTEXT *file_download_context, bool is_block);
...
//    require_noerr_string( err, exit, "ERROR: Unable to start the file download thread" );
    ...
    thread_id = mos_thread_new( MOS_APPLICATION_PRIORITY, "file download", http_file_download_thread, stack_size, file_download_context_user );
    require_string( thread_id != NULL, exit, "ERROR: Unable to start the file download thread" );
    ...
    if(err != kNoErr)
    {
        /* error happened, must release resource and set context to NULL */
...
merr_t http_file_download_stop( FILE_DOWNLOAD_CONTEXT *file_download_context, bool is_block)
{
    ...
    if(is_block == true)
    {
        ...
        {
            mos_msleep(20);
            app_log("------waitting download stop-------, state = %d", (*file_download_context)->control_state);
